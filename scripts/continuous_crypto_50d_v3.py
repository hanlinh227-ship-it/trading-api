...
TARGET=float(os.getenv('CRYPTO_TARGET_WR','80'));DAYS=int(os.getenv('CRYPTO_WINDOW_DAYS','50'));MIN_PER_WINDOW=max(DAYS,int(os.getenv('CRYPTO_V3_MIN_TRADES_WINDOW','50')));VAL_WINDOWS=max(3,int(os.getenv('CRYPTO_V3_VALIDATION_WINDOWS','3')));WORST=float(os.getenv('CRYPTO_V3_WORST_WR','70'));FEE=float(os.getenv('CRYPTO_FEE_BPS_RT','10'))/10000;SLIP=float(os.getenv('CRYPTO_SLIPPAGE_BPS_RT','4'))/10000;AI_META={'providers':['claude','openai','deepseek'],'rule':'DEV_ONLY','quorum':2}
...
def rsi(vals,n=14):
 ...
 return out
def run(rows,start,end,p,rr,cost_mult=1,delay=0):
 ...
 while True:
  ...
  try:
   ...
   if gate(dev):
    ...
    rng=random.Random(seed^0xA55A);rng.shuffle(pool);starts=sorted(pool[:VAL_WINDOWS]);vals=[run(rows,x,x+span,p,rr) for x in starts];aggN=sum(x['n'] for x in vals);aggW=sum(x['wins'] for x in vals);aggR=sum(x['netR'] for x in vals);aggWR=100*aggW/aggN if aggN else 0;baseok=len(vals)==VAL_WINDOWS and all(x['n']>=MIN_PER_WINDOW and x['netR']>0 for x in vals) and aggN>=MIN_PER_WINDOW*VAL_WINDOWS and aggWR>=TARGET and min(x['wr'] for x in vals)>=WORST and aggR>0
    if baseok:
     stress['cost1_5x']=[run(rows,x,x+span,p,rr,1.5) for x in starts];stress['cost2x']=[run(rows,x,x+span,p,rr,2) for x in starts];stress['delay1']=[run(rows,x,x+span,p,rr,1,1) for x in starts];stressok=all(sum(z['netR'] for z in a)>0 for a in stress.values());neigh=[sum(run(rows,x,x+span,pp,rr)['netR'] for x in starts) for pp in perturb(p)];stress['neighborNetR']=neigh;passed=stressok and sum(x>0 for x in neigh)>=max(1,int(len(neigh)*.6))
   ...
   if passed:lock={'symbol':s,'version':lin['version'],'profile':p,'profileHash':phash(p),'rr':rr,'lockedAt':now(),'validation':rec['validation'],'stress':rec['stress']};state['qualified'][s]=lock;atomic(ROOT/'profiles'/f'{s}.json',lock);lin['status']='LOCKED'
    ...
    package={'symbol':s,'round':state['round'],'profile':p,'rr':rr,'targetWR':TARGET,'minTrades':MIN_PER_WINDOW,'windowDays':DAYS,'metrics':rec['dev'],'long':{'n':dev['longN']},'short':{'n':dev['shortN']},'recentTrades':dev['trades'][-30:]}
