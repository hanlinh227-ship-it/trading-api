...
def run(rows,start,end,p,rr,cost_mult=1.0,delay=0):
 ...
 while i<len(rows)-3:
  ...
  if gross is None:
   xi=min(len(rows)-1,ei+95);exitpx=rows[xi][4];gross=side*(exitpx-entry)/risk
  costs=entry*(FEE+SLIP)*cost_mult/risk;net=gross-costs
  tr.append({'t':rows[ei][0],'side':'LONG' if side==1 else 'SHORT','entry':entry,'exit':exitpx,'sl':sl,'tp':tp,'grossR':gross,'costR':costs,'netR':net});i=xi+1
 wins=sum(x['netR']>0 for x in tr);loss=sum(x['netR']<=0 for x in tr);n=len(tr);eq=peak=dd=0;mxdd=0;streak=mxstreak=0
 for x in tr:
  eq+=x['netR'];peak=max(peak,eq);dd=peak-eq;mxdd=max(mxdd,dd);streak=streak+1 if x['netR']<=0 else 0;mxstreak=max(mxstreak,streak)
 pos=sum(x['netR'] for x in tr if x['netR']>0);neg=-sum(x['netR'] for x in tr if x['netR']<0)
 return {'trades':tr,'n':n,'wins':wins,'losses':loss,'wr':100*wins/n if n else 0,'netR':sum(x['netR'] for x in tr),'expectancy':sum(x['netR'] for x in tr)/n if n else 0,'profitFactor':pos/neg if neg else (999 if pos else 0),'maxDrawdownR':mxdd,'maxConsecutiveLosses':mxstreak,'entriesPerDay':n/DAYS}
...
def main():
 ...
 sp=ROOT/'state.json';state=json.loads(sp.read_text()) if sp.exists() else {'version':'CRYPTO-50D-V3','epochStartedAt':now(),'round':0,'qualified':{},'history':[]}
 while True:
  unresolved=[s for s in active if s not in state['qualified']]
  if not unresolved:state.update(status='TARGET_ACHIEVED_ALL_USABLE_SYMBOLS',updatedAt=now());atomic(sp,state);atomic(ROOT/'reports'/'final.json',state);time.sleep(3600);continue
  s=unresolved[state['round']%len(unresolved)];state['round']+=1;seed=random.SystemRandom().randrange(1,2**31);lin=loadlin(s);p=(lin.get('current') or {}).get('profile') or b.profile(seed);rr0=(lin.get('current') or {}).get('rr'); council=None
  try:
   ...
   first,last=rows[0][0],rows[-1][0];split=first+int((last-first)*.60);vstart=split;vmax=last-span
   if split-first<span or vmax-vstart<VAL_WINDOWS*span:raise RuntimeError('INSUFFICIENT_DISJOINT_HISTORY')
   ...
   if gate(dev):
    # deterministic, precommitted, non-overlapping unseen windows
    pool=[];x=vstart
    while x+span<=last:pool.append(x);x+=span
    rng=random.Random(seed^0xA55A);rng.shuffle(pool);starts=sorted(pool[:VAL_WINDOWS])
    vals=[run(rows,x,x+span,p,rr) for x in starts]
    ...
    if baseok:
     stress['cost1_5x']=[run(rows,x,x+span,p,rr,1.5) for x in starts];stress['cost2x']=[run(rows,x,x+span,p,rr,2.0) for x in starts];stress['delay1']=[run(rows,x,x+span,p,rr,1,1) for x in starts]
     stressok=all(sum(z['netR'] for z in arr)>0 for arr in stress.values())
     neigh=[]
     for pp in perturb(p):
      zs=[run(rows,x,x+span,pp,rr) for x in starts];neigh.append(sum(z['netR'] for z in zs))
     stress['neighborNetR']=neigh;robust=sum(x>0 for x in neigh)>=max(1,int(len(neigh)*.6));passed=stressok and robust
   rec={'round':state['round'],'at':now(),'symbol':s,'seed':seed,'profileVersion':lin['version'],'profileHash':phash(p),'profile':p,'rr':rr,'dataQuality':q,'dev':{k:v for k,v in dev.items() if k!='trades'},'validation':[{k:v for k,v in x.items() if k!='trades'} for x in vals],'stress':{k:([{kk:vv for kk,vv in z.items() if kk!='trades'} for z in v] if isinstance(v,list) and v and isinstance(v[0],dict) else v) for k,v in stress.items()},'pass':passed}
   append(ROOT/'trials.jsonl',rec);atomic(ROOT/'trades'/f'{s}-{state["round"]}-dev.json',dev['trades'])
   if passed:
    lock={'symbol':s,'version':lin['version'],'profile':p,'profileHash':phash(p),'rr':rr,'lockedAt':now(),'validation':rec['validation'],'stress':rec['stress']};state['qualified'][s]=lock;atomic(ROOT/'profiles'/f'{s}.json',lock);lin['status']='LOCKED'
   else:
   ...
   atomic(ROOT/'profiles'/f'{s}-lineage.json',lin);state['history']=(state['history']+[rec])[-500:];state.update(status='RESEARCH_RUNNING_V3',currentSymbol=s,updatedAt=now(),targetWR=TARGET,minTradesPer50d=MIN_PER_WINDOW,validationWindows=VAL_WINDOWS,worstWindowWR=WORST,rrAllowed=[1,2],aiLearning={'providers':['claude','openai','deepseek'],'rule':'DEV_ONLY','quorum':2});atomic(sp,state);atomic(ROOT/'reports'/'current.json',state)
  except Exception as e:
   er={'round':state['round'],'at':now(),'symbol':s,'error':repr(e),'pass':False};append(ROOT/'trials.jsonl',er);state['lastError']=er;state['updatedAt']=now();atomic(sp,state);time.sleep(5)
