...
def strength(frames,cut):
    ...
    result={h:{c:(statistics.mean(v) if v else 0.0) for c,v in ans[h].items()} for h in hs}; _STRENGTH_CACHE[key]=result; return result
...
def pair_features(pair,rows,cut,st):
    ...
    h4trend=1 if c>e4_20>e4_50 else -1 if c<e4_20<e4_50 else 0; h1trend=1 if c>e1_20>e1_50 else -1 if c<e1_20<e1_50 else 0
    h4slope=1 if e4_20>e4_prev else -1; h1slope=1 if e1_20>e1_prev else -1; m15dev=(c-e15_20)/a15; mom1=(c-mc[-5])/a15; mom4=(c-mc[-17])/a15 if len(mc)>=17 else 0.0
    ...
    if h1adx>=24:regime=1.20*d6+0.85*d24+0.40*d72+1.35*h4trend+1.15*h1trend+0.35*h1slope+0.30*max(-1.5,min(1.5,mom1))
    elif h1adx<=17:regime=0.25*d6+0.60*d24+0.45*d72+0.55*h4trend+0.45*h1trend-0.70*max(-1.5,min(1.5,m15dev))
    else:regime=0.80*d6+0.80*d24+0.45*d72+1.00*h4trend+0.90*h1trend+0.30*h1slope
    pf1=CURRENCY_PROFILE[b]; pf2=CURRENCY_PROFILE[q]; risk_floor=max(pf1["riskFloor"],pf2["riskFloor"]); chase=max(pf1["chase"],pf2["chase"]); adr=daily_range_proxy(pre) or 6.0*a1
    result={"symbol":pair,"entry":c,"atr15":a15,"atr1":a1,"adr":adr,"d6":d6,"d24":d24,"d72":d72,"h4trend":h4trend,"h1trend":h1trend,"h1adx":h1adx,"h1rsi":hrsi,"m15dev":m15dev,"mom1":mom1,"mom4":mom4,"ema15":e15_20,"riskFloorATR":risk_floor,"maxChaseATR":chase,"scores":{"BALANCED":balanced,"STRUCTURE":structure,"REGIME":regime},"pre":pre,"h1":h1}; _FEATURE_CACHE[key]=result; return result

def direction_for(f,model):return "BUY" if f["scores"][model]>=0 else "SELL"
def build_barriers(f,side):
    pre=f["pre"]; en=f["entry"]; a=f["atr15"]; adr=f["adr"]; sgn=1 if side=="BUY" else -1; recent=pre[-24:]; swing=min(r["low"] for r in recent) if side=="BUY" else max(r["high"] for r in recent)
    raw=(en-swing+0.12*a) if side=="BUY" else (swing-en+0.12*a); floor=f["riskFloorATR"]*a; cap=max(2.6*a,0.32*adr); risk=min(max(raw,floor),cap); sl=en-risk if side=="BUY" else en+risk
    p24=pre[-96:] if len(pre)>=96 else pre; p72=pre[-288:] if len(pre)>=288 else pre; ext24=max(r["high"] for r in p24) if side=="BUY" else min(r["low"] for r in p24); ext72=max(r["high"] for r in p72) if side=="BUY" else min(r["low"] for r in p72)
    ...
    target_dist=max(target_dist,0.90*a); tp=en+sgn*target_dist; return sl,tp,risk,(target_dist/risk if risk>0 else None)

def limit_price(f,side,risk):
    en=f["entry"]; ema15=f["ema15"]; a=f["atr15"]; sgn=1 if side=="BUY" else -1; favorable=(ema15<en) if side=="BUY" else (ema15>en); pull=abs(en-ema15) if favorable else 0.18*risk; pull=min(max(pull,0.12*risk),0.45*a); return en-sgn*pull
...
def horizon_dir(f,rows,cut,model,h):
    ...
    move=(end-f["entry"]) if side=="BUY" else (f["entry"]-end); return {"correct":move>0,"moveATR":(move/f["atr15"] if f["atr15"] else None),"end":end}

def summarize_market(items):
    res=[x for x in items if x["outcome"]["result"] in ("TP","SL")]; w=sum(x["outcome"]["result"]=="TP" for x in res); l=len(res)-w; rrs=[x["outcome"]["plannedRR"] for x in res if x["outcome"].get("plannedRR") is not None]; pnl=[x["outcome"].get("plannedRR",0.0) if x["outcome"]["result"]=="TP" else -1.0 for x in res]
...
def summarize_limit(items):
    fills=[x for x in items if x["outcome"].get("filled")]; res=[x for x in fills if x["outcome"]["result"] in ("TP","SL")]; w=sum(x["outcome"]["result"]=="TP" for x in res); l=len(res)-w; rrs=[x["outcome"].get("effectiveRR") for x in res if x["outcome"].get("effectiveRR") is not None]; pnl=[x["outcome"].get("effectiveRR",0.0) if x["outcome"]["result"]=="TP" else -1.0 for x in res]
...
def summarize_direction(items,key):
    ...
    c=sum(v["correct"] for v in vals); mv=[v["moveATR"] for v in vals if v.get("moveATR") is not None]; return {"tests":len(vals),"correct":c,"accuracy":round(100*c/len(vals),2),"avgMoveATR":round(statistics.mean(mv),3) if mv else None}
...
def evaluate_model_pair(pair,model,frames,dates):
    ...
    for s in dates:
        ...
        if not f:continue
        om=outcome_market(f,frames[pair],cut,model); items.append({"cutoff":s,"feature":f,"outcome":om,"d6":horizon_dir(f,frames[pair],cut,model,6),"d12":horizon_dir(f,frames[pair],cut,model,12),"d24":horizon_dir(f,frames[pair],cut,model,24)})
    sm=summarize_market(items); sd=summarize_direction(items,"d12"); acc=(sd["accuracy"] or 0)/100; exp=sm["expectancyR"] if sm["expectancyR"] is not None else -1; med=sm["medianPlannedRR"] if sm["medianPlannedRR"] is not None else 0; score=0.70*acc+0.30*max(-1,min(1,exp/1.5))
...
def evaluate_dates(frames,dates,chosen):
    ...
    for s in dates:
        ...
        for p in PAIRS:
            ...
            if not f:continue
            m=chosen[p]; om=outcome_market(f,frames[p],cut,m); ol=outcome_limit(f,frames[p],cut,m); rec={"symbol":p,"model":m,"side":direction_for(f,m),"score":round(f["scores"][m],4),"entry":f["entry"],"h1ADX":round(f["h1adx"],2),"h1RSI":round(f["h1rsi"],2),"d6Strength":round(f["d6"],3),"d24Strength":round(f["d24"],3),"d72Strength":round(f["d72"],3),"market":om,"limit":ol,"dir6h":horizon_dir(f,frames[p],cut,m,6),"dir12h":horizon_dir(f,frames[p],cut,m,12),"dir24h":horizon_dir(f,frames[p],cut,m,24)}; im={"symbol":p,"cutoff":s,"outcome":om,"d6":rec["dir6h"],"d12":rec["dir12h"],"d24":rec["dir24h"]}; il={"symbol":p,"cutoff":s,"outcome":ol}
    ...
    for p,recs in per_pair.items():
        ims=[{"outcome":r["market"],"d6":r["dir6h"],"d12":r["dir12h"],"d24":r["dir24h"]} for r in recs]; ils=[{"outcome":r["limit"]} for r in recs]; pair_summary[p]={"model":chosen[p],"tests":len(recs),"market":summarize_market(ims),"limit":summarize_limit(ils),"direction6h":summarize_direction(ims,"d6"),"direction12h":summarize_direction(ims,"d12"),"direction24h":summarize_direction(ims,"d24"),"trades":recs}
