...
def make_maps(data):
    mp={p:{r['dt']:(i,r) for i,r in enumerate(rows)} for p,rows in data.items()}
...
def execute(rows,i,side,par):
    ...
    if entry_mode=='MARKET':ei=i+1;entry=rows[ei]['open']
    else:
...
def evaluate(sym,days,data,par,a,c):
    ...
    for d in sorted(k for k in days if a<=k<=c):
        ...
        hs=days[d];hh=hour if hour in hs else max([x for x in hs if x<=hour],default=max(hs));i,m=hs[hh];o=execute(data[sym],i,side_for(fam,m),par)
        if o:z.append(o)
        ...
        for rf in RFS:
         for sw in SWS:
          for em,off,exp in ENTRIES:
           for ca,cr,prog in MGMTS:yield (fam,h,rr,rf,sw,em,off,exp,ca,cr,prog)
def main():
    data=load_data();mp,times=make_maps(data);mx=matrix(data,mp,times);ps=list(params());res={};print('PARAMS',len(ps),flush=True)
    for sym in b.PAIRS:
        hist=[];winner=None
