...
def main(src="data/v76_entry_methods.json",dst="data/v76_entry_summary.json"):
    ...
    out={
        "version":"V76-ENTRY-SUMMARY-R2","methodsVersion":d.get("version"),"generatedAt":d.get("generatedAt"),"selectionFrozen":d.get("selectionFrozen"),
        "retainedArchetypes":retained,"fallbackResearchArchetypes":fallback,
        "notRetainedArchetypes":sorted(setups-set(retained)),"fullyRejectedArchetypes":sorted(setups-set(retained)-set(fallback)),
        "counts":d.get("counts"),"promotionCriteria":d.get("promotionCriteria"),
        "promotedSymbols":[r["symbol"] for r in rows if r["liveEligible"]],
        "researchOnlySymbols":[r["symbol"] for r in rows if not r["liveEligible"]],"pairs":rows
    }
