...
def momentum_features(sym,fr,su,btc_fr):
    r6=v6.ret(fr['H1'],6);r24=v6.ret(fr['H1'],24);r72=v6.ret(fr['H4'],18)
    br24=v6.ret(btc_fr['H1'],24);br72=v6.ret(btc_fr['H4'],18)
    rs24=r24-br24;rs72=r72-br72
    st4=v6.structure(fr['H4']);st1=v6.structure(fr['H1'])
    ema50=su['H4']['ema50'];ema200=su['H4']['ema200'];cl=su['H4']['close']
    ema_bias=(1 if ema50 and cl>ema50 else -1 if ema50 else 0)+(0.5 if ema200 and cl>ema200 else -0.5 if ema200 else 0)
    # return magnitudes clipped so one spike cannot dominate all other evidence
...
def choose_side(policy,row,breadth):
    base=row['baseSide'];f=row['feat'];su=row['su'];fr=row['fr'];m=row['model'];score=f['momScore'];pos=m.get('rangePositionH1',.5)
    if policy=='v6_base':return base
    if policy=='tsmom':
        raw=1.0*sgn(f['r6'])+1.4*sgn(f['r24'])+1.7*sgn(f['r72'])
        return 'BUY' if raw>=0 else 'SELL'
    if policy=='tsmom_structure':return 'BUY' if score>=0 else 'SELL'
    if policy=='breadth_momentum':
        adj=score + (1.2 if breadth>=.65 else -1.2 if breadth<=.35 else 0)
        return 'BUY' if adj>=0 else 'SELL'
    # adaptive: short-horizon crypto evidence favors momentum; reversal only in low-ADX extreme ranges.
    rg=m['regime'];h4=su['H4'];h1=su['H1']
    low_adx=(h4.get('adx14') or 99)<16 and (h1.get('adx14') or 99)<16
    if rg=='range' and low_adx and pos>=.85:return 'SELL'
    if rg=='range' and low_adx and pos<=.15:return 'BUY'
    adj=score + (0.8 if breadth>=.67 else -0.8 if breadth<=.33 else 0)
    return 'BUY' if adj>=0 else 'SELL'
...
def load(cutoff):
    ...
    for sym in v6.COINS:
        try:
            ...
            base,sc,en,_,_,_,model=v6.choose(sym,su,fr,bf,bm)
            feat=momentum_features(sym,fr,su,bf);rows.append({'sym':sym,'source':source,'fr':fr,'su':su,'baseSide':base,'score':sc,'en':en,'model':model,'feat':feat,'future':future(source,sym,cut)})
        except Exception:pass
    breadth=sum(r['feat']['r24']>0 for r in rows)/len(rows) if rows else .5
    return rows,breadth
...
def main():
    ...
    for p,lb,sf,rr in itertools.product(POLICIES,LOOKBACKS,STOP_FACTORS,RRS):
        per={k:eval_cfg(v['rows'],v['breadth'],p,lb,sf,rr) for k,v in ds.items()};minwr=min(x['winRate'] for x in per.values());minexp=min(x['expectancyR'] for x in per.values());avgwr=sum(x['winRate'] for x in per.values())/2;avgexp=sum(x['expectancyR'] for x in per.values())/2
        robust=minwr+10*minexp+2.5*(rr-1.5)+.12*avgwr+3*avgexp
