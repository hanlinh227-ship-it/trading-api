...
// Persist only executable orders in Books. WATCH belongs to fresh scan output, never to durable order state.
const stateBlock=`function emptyGroup(){return {marketActive:[],limitActive:[],limitPending:[],watch:[]};}
...
function validExecutablePosition(p,group){
  ...
  if(group==="crypto")return true; // preserve legacy crypto positions opened by prior verified engine versions.
...
const uiBlock=`function summary(group,books,run=null){
  ...
  if(run){
    if(run.status==="RATE_BUDGET_WAIT"){L.push(`⏱ Không dùng WATCH cũ. Twelve Data còn ${run.diagnostics?.tdCreditsLeft??"?"} credit; cần ${run.diagnostics?.tdCreditsRequired??"?"}. Thử lại sau ~${run.retryAfterSec??60}s.`);return L.filter(Boolean).join("\\n");}
    if(run.status==="BUSY"){L.push("⏳ Một lượt quét khác đang chạy. Không hiển thị setup cũ thay cho kết quả mới.");return L.filter(Boolean).join("\\n");}
    L.push(`🔍 Coverage ${run.broadOk}/${run.requested} • Deep ${run.deepOk}/${run.deepRequested} • Lệnh mới ${run.newCount}`,`🆔 Scan ${run.scanId}`);
  }
  return L.filter(Boolean).join("\\n");
}
function currentPendingNews(run){return freshWatchFromRun(run).filter(w=>w.reason==="NEWS_CONTEXT_REQUIRED").slice(0,3);}
function groupKeyboard(group,run){const rows=baseKeyboard().inline_keyboard,pending=currentPendingNews(run);if(pending.length)rows.unshift(pending.map(w=>({text:`✅ Tin OK ${w.symbol}`,callback_data:`news:${group}:${w.symbol}`})));return {inline_keyboard:rows};}
function hubKeyboard(h){const rows=baseKeyboard().inline_keyboard,p=[];for(const [g,r] of Object.entries(h?.runs||{}))for(const w of currentPendingNews(r))if(p.length<3)p.push({text:`✅ Tin OK ${w.symbol}`,callback_data:`news:${g}:${w.symbol}`});if(p.length)rows.unshift(p);return {inline_keyboard:rows};}
...
async function runHub(env){
  const hubStarted=Date.now(),hubScanId=`hub-${hubStarted}-${Math.random().toString(36).slice(2,8)}`,runs={};for(const g of ["crypto","forex","metal"])runs[g]=await runGroup(g,env);
  const top=Object.entries(runs).flatMap(([group,r])=>(r.analyses||[]).map(a=>({...a,group,scanId:r.scanId,scannedAt:r.scannedAt}))).sort((a,b)=>hubRank(b)-hubRank(a)).slice(0,7);
  return {ok:true,version:CONFIG.version,hubScanId,scannedAt:new Date(hubStarted).toISOString(),runs,top};
}
...
`;
replaceRange('function summary(group,books,run=null){','async function lifecycle(env){',uiBlock+'async function lifecycle(env){','fresh scan UI');

// Record lifecycle outcomes, but never re-scan the strategy for already-open positions.
mustReplace('if(p.expiresAt&&Date.now()>p.expiresAt){changed=true;continue;}','if(p.expiresAt&&Date.now()>p.expiresAt){changed=true;await appendOrderHistory(env,{event:"LIMIT_EXPIRED",group:"crypto",position:p});continue;}','pending expiry history');
mustReplace('p.status="ACTIVE";p.openedAt=Date.now();b.limitActive.push(p);changed=true;await sendText(env,`🔵 LIMIT ĐÃ KHỚP\\n${p.symbol} ${sideText(p.side)}\\nEntry: ${p.entry}`).catch(()=>{});','p.status="ACTIVE";p.openedAt=Date.now();b.limitActive.push(p);changed=true;await appendOrderHistory(env,{event:"LIMIT_FILLED",group:"crypto",position:p,price:px});await sendText(env,`🔵 LIMIT ĐÃ KHỚP\\n${p.symbol} ${sideText(p.side)}\\nEntry: ${p.entry}`).catch(()=>{});','limit fill history');
...
// Direct/news rechecks may create executable orders, but WATCH remains ephemeral and is rendered from this exact recheck only.
mustReplace('await setNewsClearance(symbol,env);await sendText(env,`✅ Tin/context OK cho ${norm(symbol)} trong ${Math.round(CONFIG.newsClearanceTtlSec/60)} phút. Đang kiểm tra lại...`,chatId);\n      const a=await runSymbol(symbol,env);\n      const books=await getBooks(env);fillBooks(group,books,[a]);await saveBooks(env,books);await sendText(env,summary(group,books,{requested:1,broadOk:1,deepRequested:1,deepOk:a.ok===false?0:1,newCount:["MARKET","LIMIT"].includes(a.status)?1:0,analyses:[a]}),chatId,groupKeyboard(group,books));','await setNewsClearance(symbol,env);await sendText(env,`✅ Tin/context OK cho ${norm(symbol)} trong ${Math.round(CONFIG.newsClearanceTtlSec/60)} phút. Đang kiểm tra lại bằng dữ liệu mới...`,chatId);\n      const a=await runSymbol(symbol,env),books=await getBooks(env),added=fillBooks(group,books,[a]);if(added.length)await saveBooks(env,books);const rerun={version:CONFIG.version,group,scanId:`manual-${group}-${Date.now()}`,scannedAt:new Date().toISOString(),requested:1,broadOk:a.ok===false?0:1,deepRequested:1,deepOk:a.ok===false?0:1,newCount:added.length,analyses:[a]};await saveScanSnapshot(group,rerun,env);await sendText(env,summary(group,books,rerun),chatId,groupKeyboard(group,rerun));','news recheck fresh');
mustReplace('if(type!=="unknown"){const books=await getBooks(env);const added=fillBooks(type,books,[a]);if(added.length||a.status==="WATCH")await saveBooks(env,books);}','if(type!=="unknown"){const books=await getBooks(env);const added=fillBooks(type,books,[a]);if(added.length)await saveBooks(env,books);await saveScanSnapshot(type,{version:CONFIG.version,group:type,scanId:`manual-${type}-${Date.now()}`,scannedAt:new Date().toISOString(),requested:1,broadOk:a.ok===false?0:1,deepRequested:1,deepOk:a.ok===false?0:1,newCount:added.length,analyses:[a]},env);}','direct analysis ephemeral watch');

// Add endpoints to inspect persistent live orders and latest ephemeral scan separately.
mustReplace('if(p==="/books")return json(await getBooks(env));','if(p==="/books"||p==="/orders")return json(await getBooks(env));\n      if(p==="/latest-scan"){const g=u.searchParams.get("group");if(!GROUPS[g])return json({ok:false,error:"invalid group"},400);return json({ok:true,group:g,snapshot:await getScanSnapshot(g,env)});}','orders scan endpoints');
