...
def public(path,params=None):
    ...
    with urllib.request.urlopen(urllib.request.Request(url,headers={'User-Agent':'AUTO-FUTURES-PREFLIGHT'}),timeout=15) as r:return json.loads(r.read().decode())
def signed(method,path,params=None):
    p=dict(params or {});p['timestamp']=int(public('/fapi/v1/time')['serverTime']);p['recvWindow']=10000
    q=urllib.parse.urlencode(p);sig=hmac.new(SECRET.encode(),q.encode(),hashlib.sha256).hexdigest();payload=q+'&signature='+sig;url=BASE+path;data=None
...
def main():
    ...
    try:
        ...
        result['live_open_positions']=len(live);result['slots_left']=max(0,MAX_POSITIONS-len(live));result['available_balance']=available;result['open_order_count']=len(open_orders)
        ...
        for symbol,d in risk.get('decisions',{}).items():
            ...
            if info and not reasons[:1]:
                try:
                    ...
                    lot=flt(info,'MARKET_LOT_SIZE') or flt(info,'LOT_SIZE');pf=flt(info,'PRICE_FILTER');mn=flt(info,'MIN_NOTIONAL')
                    step=float(lot.get('stepSize',0) or 0);minq=float(lot.get('minQty',0) or 0);maxq=float(lot.get('maxQty',1e99) or 1e99);tick=float(pf.get('tickSize',0) or 0);min_notional=float(mn.get('notional',0) or 0)
                    lev=int(d.get('max_leverage',3) or 3);risk_pct=float(d.get('risk_pct',0.5) or .5)
                    qty=floor_step(min(available*risk_pct/100/risk_dist,available*lev/px),step) if risk_dist>0 else 0
                    notional=qty*px
                    ...
                    for name,val in [('stop_loss',stop),('tp1',tp1),('tp2',tp2),('tp3',tp3)]:
                        ...
                        if tick and abs(rv-float(val))>tick*0.501:reasons.append(f'{name.upper()}_PRECISION_INVALID')
                    max_algo=int((flt(info,'MAX_NUM_ALGO_ORDERS') or {}).get('limit',100) or 100)
                    algo_for_symbol=sum(1 for o in open_orders if o.get('symbol')==symbol and str(o.get('type','')).upper() in {'STOP','STOP_MARKET','TAKE_PROFIT','TAKE_PROFIT_MARKET','TRAILING_STOP_MARKET'})
            ...
            result['decisions'][symbol]={'eligible':not reasons,'reasons':reasons,'mark_price':px,'estimated_qty':qty,'estimated_notional':notional,'margin_mode':'ISOLATED','max_positions':MAX_POSITIONS}
