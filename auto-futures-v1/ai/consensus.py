...
AI_REVIEW_TIMEOUT_SECONDS=int(os.environ.get('AI_REVIEW_TIMEOUT_SECONDS','110'))
...
def run_ai(name,script,priority):
    ...
    try:
        ...
        if p.returncode!=0:return name,{'status':'ERROR','error':p.stderr[-1500:],'elapsed_seconds':round(elapsed,2),'reviews':{}}
        out=p.stdout.strip()
...
def main():
    ...
    print(f'Starting budget-governed Claude + DeepSeek + Codex reviewers | hard timeout {AI_REVIEW_TIMEOUT_SECONDS}s each...',flush=True)
    results={}
    ...
    out={'generated_at':datetime.now(timezone.utc).isoformat(),'policy':{'style':'SCALP_ONLY_24_7','decision_rule':'2_of_3_same_direction_no_opposition','three_ai_required':True,'token_mode':'EVENT_GATED_BUDGET_GOVERNED','cache_ttl_seconds':CACHE_TTL_SECONDS,'ai_review_timeout_seconds':AI_REVIEW_TIMEOUT_SECONDS,'priority':priority,'live_open_positions_hint':live_count,'paper_positions_ignored_for_ai_budget':True,'budget':budget_snapshot()},'claude_status':results.get('claude',{}).get('status','ERROR'),'deepseek_status':results.get('deepseek',{}).get('status','ERROR'),'codex_status':results.get('codex',{}).get('status','ERROR'),'reviewer_latency_seconds':{n:(results.get(n,{}) or {}).get('elapsed_seconds') for n in SCRIPTS},'symbols':decisions}
    ...
    print('='*48);print('V8 BUDGET-GOVERNED THREE-AI CONSENSUS');print('='*48);print('Claude:',out['claude_status'],'DeepSeek:',out['deepseek_status'],'Codex:',out['codex_status']);print('LATENCY:',json.dumps(out['reviewer_latency_seconds']));print('LIVE POSITIONS HINT:',live_count);print('BUDGET:',json.dumps(out['policy']['budget'],ensure_ascii=False))
