...
AI_REVIEW_TIMEOUT_SECONDS=int(os.environ.get('AI_REVIEW_TIMEOUT_SECONDS','60'))
...
def event_fingerprint(snapshot,live_count):
    ...
    raw=json.dumps({'candidates':rows,'live_open_count':live_count},sort_keys=True,separators=(',',':'))
...
def run_ai(name,script,priority,candidate_count):
    ...
    if not circuit_ok:
        r={'status':'CIRCUIT_OPEN','circuit_reason':circuit_reason,'reviews':{}}
        record_result(name,r['status'],review_count=0,candidate_count=candidate_count,error=circuit_reason);return name,r
    budget_ok,budget_reason=allow(name,priority)
    if not budget_ok:
        r={'status':'BUDGET_BLOCKED','budget_reason':budget_reason,'reviews':{}}
        record_result(name,r['status'],review_count=0,candidate_count=candidate_count,error=budget_reason);return name,r
    started=datetime.now(timezone.utc)
    try:
        p=subprocess.run(['python3',script],cwd=AI_DIR,capture_output=True,text=True,timeout=AI_REVIEW_TIMEOUT_SECONDS)
    ...
    record_result(name,r.get('status','ERROR'),elapsed=r.get('elapsed_seconds'),review_count=len(r.get('reviews') or {}),candidate_count=candidate_count,error=r.get('error') or r.get('budget_reason') or r.get('circuit_reason'))
    return name,r

def make_decisions(results):
    review_maps={n:(results.get(n,{}).get('reviews',{}) or {}) for n in SCRIPTS}
    symbols=sorted(set().union(*(set(x) for x in review_maps.values()))) if review_maps else []
    all_providers_ok=all(str((results.get(n) or {}).get('status','')).upper()=='OK' for n in SCRIPTS)
    decisions={}
    for symbol in symbols:
        ...
        longs,shorts,waits=actions.count('LONG'),actions.count('SHORT'),actions.count('WAIT');final_action,reason='WAIT','NO_EDGE';directional_conf=[]
        if not all_providers_ok or len(available)<3:reason='THREE_AI_HEALTH_REQUIRED'
        elif longs>=2 and shorts==0:
            directional_conf=[float(r.get('confidence',0) or 0) for r in available.values() if str(r.get('action','')).upper()=='LONG']
            if directional_conf and sum(directional_conf)/len(directional_conf)>=62:final_action,reason='LONG','TWO_OF_THREE_PLUS_NO_OPPOSITION'
            else:reason='LONG_CONFIDENCE_TOO_LOW'
        elif shorts>=2 and longs==0:
            directional_conf=[float(r.get('confidence',0) or 0) for r in available.values() if str(r.get('action','')).upper()=='SHORT']
            if directional_conf and sum(directional_conf)/len(directional_conf)>=62:final_action,reason='SHORT','TWO_OF_THREE_PLUS_NO_OPPOSITION'
        ...
        consensus_conf=round(sum(directional_conf)/len(directional_conf),2) if directional_conf else 0.0
        decisions[symbol]={'reviews':reviews,'available_reviewers':len(available),'actions':actions,'average_confidence':consensus_conf,'consensus_confidence':consensus_conf,'confidence_definition':'DIRECTIONAL_REVIEWERS_ONLY','final_action':final_action,'reason':reason}
    return decisions
...
def main():
    ...
    if not candidates and live_count<=0:
        out={'generated_at':datetime.now(timezone.utc).isoformat(),'policy':{'token_mode':'EVENT_GATED','reason':'NO_ACTIONABLE_SETUP_AND_NO_LIVE_POSITION','budget':budget_snapshot(),'provider_health':health_snapshot()},'claude_status':'SKIPPED','deepseek_status':'SKIPPED','codex_status':'SKIPPED','symbols':{}}
        OUT.write_text(json.dumps(out,indent=2,ensure_ascii=False),encoding='utf-8');build_coordination_policy();print('3-AI SKIPPED: no actionable setup and no LIVE position');return
    ...
    priority='POSITION' if live_count>0 else 'NORMAL';print(f'Starting coordinated 3-AI council | top {len(candidates)} | outer timeout {AI_REVIEW_TIMEOUT_SECONDS}s...',flush=True)
    results={}
    with ThreadPoolExecutor(max_workers=3) as pool:
        future_map={pool.submit(run_ai,n,s,priority,len(candidates)):n for n,s in SCRIPTS.items()}
        for f in as_completed(future_map):
            n,r=f.result();results[n]=r;print(f'AI reviewer finished: {n} | {r.get("status")} | {r.get("elapsed_seconds","-")}s',flush=True)
    decisions=make_decisions(results)
    out={'generated_at':datetime.now(timezone.utc).isoformat(),'policy':{'style':'SCALP_ONLY_24_7','decision_rule':'2_of_3_same_direction_no_opposition','three_ai_required':True,'confidence_rule':'directional_reviewers_only','token_mode':'COORDINATED_TOP3_EVENT_GATED','candidate_limit':3,'cache_ttl_seconds':CACHE_TTL_SECONDS,'ai_review_timeout_seconds':AI_REVIEW_TIMEOUT_SECONDS,'priority':priority,'budget':budget_snapshot(),'provider_health':health_snapshot()},'claude_status':results.get('claude',{}).get('status','ERROR'),'deepseek_status':results.get('deepseek',{}).get('status','ERROR'),'codex_status':results.get('codex',{}).get('status','ERROR'),'reviewer_latency_seconds':{n:(results.get(n,{}) or {}).get('elapsed_seconds') for n in SCRIPTS},'symbols':decisions}
    ...
    coord=build_coordination_policy(decisions)
    print('='*56);print('V9 COORDINATED THREE-AI COUNCIL');print('='*56);print('Claude:',out['claude_status'],'DeepSeek:',out['deepseek_status'],'Codex:',out['codex_status']);print('LATENCY:',json.dumps(out['reviewer_latency_seconds']));print('HEALTH:',json.dumps(coord.get('provider_health',{}),ensure_ascii=False))
