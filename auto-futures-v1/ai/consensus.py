...
def event_fingerprint(snapshot, positions):
    ...
    for x in snapshot.get('ai_candidates') or snapshot.get('setups') or []:
        ...
        if action not in {'LONG', 'SHORT'}:
            continue
        rows.append({
            'symbol': x.get('symbol'), 'action': action, 'strategy': x.get('strategy'), 'regime': x.get('regime'),
            'quality5': int(float(x.get('setup_quality', 0) or 0) // 5) * 5,
            'blockers': x.get('blockers') or [], 'warnings': (x.get('warnings') or [])[:3],
        })
    open_rows = []
    for p in positions.get('positions', []):
        if p.get('status') == 'OPEN':
            open_rows.append({'symbol':p.get('symbol'),'side':p.get('side'),'stop':round(float(p.get('stop_loss',0) or 0),8)})
...
def main():
    ...
    fp = event_fingerprint(snapshot, positions)
    cache = load(CACHE, {})

    if not should_call_ai(snapshot, positions):
        out = {
            'generated_at': datetime.now(timezone.utc).isoformat(),
            'policy': {'token_mode':'EVENT_GATED','reason':'NO_ACTIONABLE_SETUP_AND_NO_OPEN_POSITION'},
            'claude_status':'SKIPPED','deepseek_status':'SKIPPED','codex_status':'SKIPPED','symbols':{},
        }
        OUT.write_text(json.dumps(out, indent=2, ensure_ascii=False), encoding='utf-8')
        print('3-AI SKIPPED: no actionable setup and no open position')
        return
    ...
    if cache.get('fingerprint') == fp and age_seconds(cache.get('generated_at')) < CACHE_TTL_SECONDS and cache.get('consensus'):
        ...
        out['policy']['cache_ttl_seconds'] = CACHE_TTL_SECONDS
        OUT.write_text(json.dumps(out, indent=2, ensure_ascii=False), encoding='utf-8')
        print('3-AI CACHE REUSED | age', round(age_seconds(cache.get('generated_at')),1), 'sec')
        return

    print("Starting Claude + DeepSeek + Codex event-gated reviewers in parallel...", flush=True)
    results = {}
    ...
    out = {
        ...
        "policy": {
            ...
            "decision_rule":"2_of_3_same_direction_no_opposition",
            "token_mode":"EVENT_GATED_FULL_3AI",
            "cache_ttl_seconds":CACHE_TTL_SECONDS,
            "call_ai_only_when":"actionable_setup_or_open_position",
        },
    ...
    OUT.write_text(json.dumps(out, indent=2, ensure_ascii=False), encoding="utf-8")
    CACHE.write_text(json.dumps({'generated_at':out['generated_at'],'fingerprint':fp,'consensus':out}, indent=2, ensure_ascii=False), encoding='utf-8')
    with (LOG_DIR/"ai_consensus.jsonl").open("a", encoding="utf-8") as log:
        log.write(json.dumps(out, ensure_ascii=False)+"\n")
    print("="*48); print("V7 EVENT-GATED THREE-AI CONSENSUS"); print("="*48)
