...
AI_REVIEW_TIMEOUT_SECONDS=int(os.environ.get('AI_REVIEW_TIMEOUT_SECONDS','65'))
...
def actionable(snapshot):
    ...
    rows.sort(key=lambda x:float(x.get('setup_quality',x.get('setup_score',0)) or 0),reverse=True)
    return rows[:3]
...
def run_ai(name,script,priority):
    ...
    try:
        ...
        if p.returncode!=0:return name,{'status':'ERROR','error':p.stderr[-1000:],'elapsed_seconds':round(elapsed,2),'reviews':{}}
        out=p.stdout.strip()
        if not out:return name,{'status':'ERROR','error':'EMPTY_OUTPUT','elapsed_seconds':round(elapsed,2),'reviews':{}}
        obj=json.loads(out.splitlines()[-1]);obj['elapsed_seconds']=round(elapsed,2);return name,obj
    except subprocess.TimeoutExpired:
        elapsed=(datetime.now(timezone.utc)-started).total_seconds()
        if name in {'claude','codex'}:record_call(name,purpose='TIMEOUT',meta={'elapsed_seconds':round(elapsed,2)})
        return name,{'status':'TIMEOUT','error':f'AI_REVIEW_TIMEOUT_{AI_REVIEW_TIMEOUT_SECONDS}S','elapsed_seconds':round(elapsed,2),'reviews':{}}
        ...
        return name,{'status':'ERROR','error':repr(exc),'elapsed_seconds':round(elapsed,2),'reviews':{}}
...
def main():
    ...
    print(f'Starting FAST 3-AI review | top {len(candidates)} candidates | hard timeout {AI_REVIEW_TIMEOUT_SECONDS}s...',flush=True)
    results={}
    with ThreadPoolExecutor(max_workers=3) as pool:
        future_map={pool.submit(run_ai,n,s,priority):n for n,s in SCRIPTS.items()}
        for f in as_completed(future_map):
            n,r=f.result();results[n]=r
            print(f'AI reviewer finished: {n} | {r.get("status")} | {r.get("elapsed_seconds","-")}s',flush=True)
    ...
    out={'generated_at':datetime.now(timezone.utc).isoformat(),'policy':{'style':'SCALP_ONLY_24_7','decision_rule':'2_of_3_same_direction_no_opposition','three_ai_required':True,'token_mode':'FAST_TOP3_EVENT_GATED','candidate_limit':3,'cache_ttl_seconds':CACHE_TTL_SECONDS,'ai_review_timeout_seconds':AI_REVIEW_TIMEOUT_SECONDS,'priority':priority,'live_open_positions_hint':live_count,'paper_positions_ignored_for_ai_budget':True,'budget':budget_snapshot()},'claude_status':results.get('claude',{}).get('status','ERROR'),'deepseek_status':results.get('deepseek',{}).get('status','ERROR'),'codex_status':results.get('codex',{}).get('status','ERROR'),'reviewer_latency_seconds':{n:(results.get(n,{}) or {}).get('elapsed_seconds') for n in SCRIPTS},'symbols':decisions}
    ...
    print('='*48);print('V8 FAST THREE-AI CONSENSUS');print('='*48);print('Claude:',out['claude_status'],'DeepSeek:',out['deepseek_status'],'Codex:',out['codex_status']);print('LATENCY:',json.dumps(out['reviewer_latency_seconds']));print('BUDGET:',json.dumps(out['policy']['budget'],ensure_ascii=False))
