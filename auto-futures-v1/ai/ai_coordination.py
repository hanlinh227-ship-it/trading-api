...
def health_snapshot():
    ...
    for name in PROVIDERS:
        p=(s.get('providers') or {}).get(name,{});until=parse(p.get('circuit_until'));circuit='OPEN' if until and now()<until else 'CLOSED'
...
def build_coordination_policy(current_decisions=None):
    ...
    if CONS_LOG.exists():
        try:
            for line in CONS_LOG.read_text(encoding='utf-8').splitlines()[-120:]:
                try:rows.append(json.loads(line))
                except Exception:pass
        except Exception:pass
    conflicts=Counter();provider_wait=Counter();samples=0
    for row in rows:
        for _,d in (row.get('symbols') or {}).items():
            ...
            if len(set(vals))>1:
                strategy=str((reviews.get('claude') or {}).get('strategy') or (reviews.get('deepseek') or {}).get('strategy') or (reviews.get('codex') or {}).get('strategy') or 'UNKNOWN').upper();conflicts[f"{strategy}|{'/'.join(sorted(set(vals)))}"]+=1
            for p,a in acts.items():
                if a=='WAIT':provider_wait[p]+=1
            samples+=1
    repeated=[{'pattern':k,'count':v} for k,v in conflicts.most_common(5) if v>=2]
    policy={'generated_at':iso(),'method':'bounded coordination meta-learning; health/routing/conflict memory only; no autonomous source rewrite; no provider voting weight from paper PnL','provider_health':health_snapshot(),'repeated_conflicts':repeated,'observed_symbol_reviews':samples,'provider_wait_counts':dict(provider_wait),'guardrails':{'three_ai_required_for_new_entry':True,'circuit_failure_threshold':CIRCUIT_FAILURES,'circuit_minutes':CIRCUIT_MINUTES,'critical_probe_interval_seconds':60,'no_autonomous_prompt_rewrite':True,'no_live_source_rewrite':True,'do_not_suppress_real_disagreement':True}}
