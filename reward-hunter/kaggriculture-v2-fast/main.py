...
DEFAULT_PARAMS = {
    ...
    "max_quadrants": 2,
    "sell_floor_ratio": 0.76,
    "ahead_sell_floor_ratio": 0.84,
...
def _weights(day, params):
    if day <= int(params["phase_early_end"]):
        return params["weights_early"]
    if day <= int(params["phase_mid_end"]):
        return params["weights_mid"]
    if day <= int(params["phase_late_end"]):
...
def _choose_crop(day, tiles, seed_budget, params):
    ...
    for crop, w in weights.items():
        if w <= 0 or seed_budget.get(crop, 0) <= 0 or not _is_viable(crop, day):
            continue
        ratio = counts.get(crop, 0) / max(float(w), 1e-6)
...
def _task_for_tile(tile, day, can_plant):
    if tile == "LOCKED":
        return None
    if tile is None:
...
def _plan_units(obs, params):
    ...
    reserved = set()
    actions = []
    distance_penalty = float(params.get("distance_penalty", 10.0))

    for idx, pos in enumerate(units):
        x, y = pos
        tile = tiles[y][x]
        can_plant_here = sum(seed_budget.values()) > 0
        current = _task_for_tile(tile, day, can_plant_here)
        ...
        for ty, row in enumerate(tiles):
            for tx, t in enumerate(row):
                if (tx, ty) in reserved:
                    continue
                task = _task_for_tile(t, day, any_seed)
                if not task:
                    continue
                priority, op = task
                dist = abs(tx - x) + abs(ty - y)
                score = priority * 100 - dist * distance_penalty
...
def _market_actions(obs, params):
    ...
    gap = float(params.get("cash_gap_trigger", 2200))
    behind = money + gap < opp_money
    ahead = money > opp_money + gap
    if behind:
        floor_ratio = float(params["behind_sell_floor_ratio"])
    elif ahead:
        floor_ratio = float(params.get("ahead_sell_floor_ratio", params["sell_floor_ratio"]))
    else:
        floor_ratio = float(params["sell_floor_ratio"])
    liquidate = day >= int(params["late_liquidation_day"])
    batch = max(1, int(params.get("sell_batch_limit", 5)))
    ...
    for item, count in shed.items():
        ...
        if liquidate or price >= base * floor_ratio:
            qty = n if liquidate else min(n, batch)
            sellable.append((price * qty, item, qty))
    sellable.sort(reverse=True)
    for _, item, qty in sellable[:4]:
        orders.append(["SELL", item, qty])

    unlocked = len(me.get("unlocked_quadrants", ["NW"]))
    max_quadrants = max(1, min(4, int(params.get("max_quadrants", 2))))
    ...
    if unlocked < max_quadrants and unlocked < 4:
        idx = unlocked - 1
        if 0 <= idx < 3 and day <= int(deadlines[idx]) and money >= float(thresholds[idx]) and len(orders) < 10:
            orders.append(["BUY_LAND"])

    hands = len(me.get("hands", []))
    target_hands = int(params["target_hands"]) if day < int(params["late_hire_stop_day"]) else int(params["late_hands"])
    need_hires = max(0, target_hands - hands)
    ...
    if day < 28 and len(orders) < 10:
        ...
        desired_stock = min(int(params["max_seed_stock"]), max(5, int((1 + hands) * float(params["seed_buffer_per_unit"]))))
        current_stock = sum(int(seeds.get(c, 0) or 0) for c in SEED_COST)
        shortage = max(0, min(free + 4, desired_stock) - current_stock)
        weights = _weights(day, params)
        budget = max(0.0, money * float(params["seed_budget_ratio"]))
        buys = []
        if shortage > 0 and (hour < 8 or current_stock < max(3, hands // 2)):
            for crop, w in sorted(weights.items(), key=lambda kv: kv[1], reverse=True):
                if w <= 0 or not _is_viable(crop, day):
                    continue
                qty = max(0, int(round(shortage * float(w))) - int(seeds.get(crop, 0) or 0))
                if qty <= 0:
                    continue
