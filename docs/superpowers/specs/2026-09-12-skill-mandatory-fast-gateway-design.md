...
## Status
Approved in-chat design for implementation planning.

## Goal
Make every request handled by GitHub Brain pass through at least one real reasoning skill before answer generation, while reducing routing latency by compiling the stable routing/skill metadata into a hot exact-SHA snapshot that does not require a GitHub read on the FAST path.

## Scope
This design governs the GitHub Brain orchestration/runtime that this repository controls. It does not claim to alter or expose hidden reasoning inside the ChatGPT platform itself. The externally testable contract is the request path, selected profile, selected primary skill, optional supporting skills, authority/security decisions, snapshot revision, and response-quality gate.

## Non-Negotiable Request Contract
Every handled request must follow:

`request -> task_router -> runtime_profile -> primary_skill_required -> optional_context/tools -> execute -> response_quality_gate -> answer`
...
- if specialist routing confidence is insufficient, select `core_reasoning` rather than perform a blocking GitHub refresh.
...
- normalized skill metadata needed for selection: id, domain, triggers, excludes, priority, requires, conflicts, output contract, and tool/source capability names.

The snapshot must not contain credentials, private provider responses, trading account state, live market data, or user secrets.
...
- The compiler must be deterministic for the same source SHA and source files except for `generated_at`; content hashes must be stable.
- A snapshot is promoted only if schema validation, router validation, skill-reference validation, fallback-skill validation, security/authority hash validation, and regression tests all pass.
...
## Skill Selection
Selection order:
1. classify intent, risk, freshness, mutation, tool requirement, and project-authority requirement;
2. choose FAST/STANDARD/DEEP under existing escalation rules;
3. select exactly one primary domain;
4. select exactly one primary skill from the hot snapshot;
5. if no specialist clears the routing threshold, use `core_reasoning`;
6. add supporting skills only when allowed by the profile and materially useful;
7. resolve authority/security/tool gates;
8. execute.
...
- the final answer is non-empty and records the routing trace internally.
...
- snapshot lookup should be in-memory or equivalent edge-local hot state;
...
If Cloudflare platform constraints prevent the 25 ms target in a specific environment, CI/performance evidence must report the measured number rather than silently relaxing the contract; zero external routing RTT remains mandatory.
...
- skill-registry validation to reject missing/invalid fallback or primary-skill contracts;
...
- tests for FAST/STANDARD/DEEP routing, fallback, failure, stale snapshot, authority/security escalation, and no-network FAST behavior.
...
2. FAST specialist match tests across representative domains.
3. FAST unknown-domain fallback to `core_reasoning`.
4. FAST test that fails if GitHub/fetch/provider access occurs during routing.
5. STANDARD lazy-load tests.
6. DEEP escalation tests for architecture, deployment, live/trading, financial, destructive, and credential-sensitive intents.
7. deterministic snapshot compiler/hash tests.
8. invalid/missing skill and conflict validation tests.
9. atomic snapshot replacement and last-known-good fallback tests.
10. response-quality-gate rejection tests.
11. regression runs for existing V4 brain/router/authority/security validators.
12. performance benchmark capturing warm FAST routing p50/p95 and external-call count.
...
7. Run domain-routing smoke tests and confirm primary skill is present for every request class.
...
- no credentials, private provider payloads, live market state, or hidden chain-of-thought are stored in the snapshot or routing trace.
