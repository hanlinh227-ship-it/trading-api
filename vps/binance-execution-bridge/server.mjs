...
async function signed(path,params={}){if(!KEY||!SECRET)throw new Error('BINANCE_CREDENTIALS_MISSING');const usp=new URLSearchParams({...params,timestamp:String(Date.now()),recvWindow:'5000'});const q=usp.toString();const r=await fetch(`${BASE}${path}?${q}&signature=${sign(q)}`,{headers:{'X-MBX-APIKEY':KEY,accept:'application/json'}});const text=await r.text();let body;try{body=JSON.parse(text)}catch{body={raw:text.slice(0,500)}}if(!r.ok)throw new Error(`BINANCE_HTTP_${r.status}:${JSON.stringify(body).slice(0,500)}`);return body;}
async function pub(path,params={}){const usp=new URLSearchParams(params);const r=await fetch(`${BASE}${path}?${usp.toString()}`,{headers:{accept:'application/json'}});const text=await r.text();let body;try{body=JSON.parse(text)}catch{body={raw:text.slice(0,500)}}if(!r.ok)throw new Error(`BINANCE_PUBLIC_HTTP_${r.status}:${JSON.stringify(body).slice(0,500)}`);return body;}
...
const server=http.createServer(async(req,res)=>{
  ...
  }catch(e){return json(res,502,{ok:false,readOnly:true,error:String(e?.message||e),checkedAt:new Date().toISOString()});}
});
