...
if [[ ! -x /usr/bin/wine || -z "$(wine_major)" || "$(wine_major)" -lt 11 ]]; then
  ...
  if [[ -n "${VERSION_CODENAME:-}" ]]; then wget -qO "/etc/apt/sources.list.d/winehq-${VERSION_CODENAME}.sources" "https://dl.winehq.org/wine-builds/ubuntu/dists/${VERSION_CODENAME}/winehq-${VERSION_CODENAME}.sources"; fi
  apt_retry apt-get update -y >/dev/null
...
case "${FORCE_REBUILD,,}" in true|1|yes) stop_mt5_wine; stamp="$(date +%Y%m%d%H%M%S)"; [[ ! -d "$WINEPREFIX_DIR" ]] || mv "$WINEPREFIX_DIR" "${WINEPREFIX_DIR}.manual-backup-${stamp}" ;; esac
...
else echo 'MT5_PERSISTENT_PREFIX=PRESERVED'; fi

echo "MT5_WINE_STACK=$EXPECTED_STACK"; echo "MT5_WINE_VERSION=$WINE_VERSION_TEXT"; echo "MT5_WINE_STACK_CONSISTENCY=PASS"

find_terminal(){ local c found=""; for c in "$INSTALL_ROOT/terminal64.exe" "$WINEPREFIX_DIR/drive_c/Program Files/MetaTrader 5/terminal64.exe"; do [[ -f "$c" ]] && { printf '%s\n' "$c"; return 0; }; done; found="$(find "$WINEPREFIX_DIR/drive_c" -maxdepth 8 -type f -iname terminal64.exe -print -quit 2>/dev/null || true)"; [[ -n "$found" ]] && printf '%s\n' "$found"; }
TERMINAL="$(find_terminal || true)"
if [[ -z "$TERMINAL" ]]; then
  wget -q --https-only -O "$INSTALLER" "$INSTALLER_URL"; chown "$APP_USER:$APP_USER" "$INSTALLER"; chmod 0640 "$INSTALLER"; stop_mt5_wine
  run_as_mt5 timeout 900 xvfb-run -a -s "$SCREEN" "$WINE_BIN" "$INSTALLER" /auto /path:'C:\MT5Forex' >/tmp/mt5-install.log 2>&1 || true
  stop_mt5_wine; TERMINAL="$(find_terminal || true)"; echo 'MT5_PERSISTENT_TERMINAL=INSTALLED'
else echo 'MT5_PERSISTENT_TERMINAL=PRESERVED'; fi
[[ -n "$TERMINAL" && -f "$TERMINAL" ]] || { echo "ERROR: MT5 terminal64.exe not found" >&2; exit 6; }
MT5_DIR="$(dirname "$TERMINAL")"; chown -R "$APP_USER:$APP_USER" "$MT5_DIR"
...
rm -f "$EA_EX5" "$LOG_HOST"; stop_mt5_wine
...
if [[ -f "$LOG_HOST" ]]; then LOG_TEXT="$(iconv -f UTF-16LE -t UTF-8 "$LOG_HOST" 2>/dev/null || cat "$LOG_HOST")"; printf '%s' "$LOG_TEXT" | grep -Eq 'Result:[[:space:]]+0 errors' || { echo "ERROR: MetaEditor did not confirm 0 errors" >&2; printf '%s\n' "$LOG_TEXT" | tail -120 >&2; exit 74; }; fi

mkdir -p /opt/trading; ln -sfn "$MT5_DIR" /opt/trading/mt5-forex-terminal
{
  printf 'MT5_WINEPREFIX=%q\n' "$WINEPREFIX_DIR"; printf 'MT5_TERMINAL=%q\n' "$TERMINAL"; printf 'MT5_INSTALL_DIR=%q\n' "$MT5_DIR"; printf 'MT5_WINE_BIN=%q\n' "$WINE_BIN"; printf 'MT5_WINESERVER_BIN=%q\n' "$WINESERVER_BIN"; printf 'MT5_WINEPATH_BIN=%q\n' "$WINEPATH_BIN"; printf 'MT5_WINE_VERSION=%q\n' "$WINE_VERSION_TEXT"; printf 'MT5_WINE_STACK=%q\n' "$EXPECTED_STACK"
} >"$APP_HOME/runtime.env"
chown "$APP_USER:$APP_USER" "$APP_HOME/runtime.env"; chmod 0640 "$APP_HOME/runtime.env"
echo "MT5_EA_SOURCE_SHA256=$SRC_SHA"; echo 'MT5_FOREX_INSTALL=PASS'; echo 'MT5_FOREX_ARCHITECTURE=PERSISTENT_APPLIANCE'; echo 'MT5_FOREX_BROKER_STATE=PRESERVED'; echo "MT5_FOREX_TERMINAL=$TERMINAL"; echo "MT5_FOREX_EA_BINARY=$EA_EX5"; echo 'MT5_FOREX_EA_COMPILE=PASS'
