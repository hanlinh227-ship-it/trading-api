...
if not p.is_file():
    print('MT5_PULSE_DIAG=missing'); raise SystemExit
try: d=json.loads(p.read_text())
except Exception as e:
    print('MT5_PULSE_DIAG=json_error:'+repr(e)); raise SystemExit
...
fail_diag() {
  ...
  find "$MT5_INSTALL_DIR/Logs" "$MT5_INSTALL_DIR/logs" "$MT5_INSTALL_DIR/MQL5/Logs" "$MT5_INSTALL_DIR/MQL5/logs" -maxdepth 1 -type f 2>/dev/null -printf '%T@ %p\n' | sort -nr | head -n 8 | cut -d' ' -f2- | while IFS= read -r f; do
    echo "--- ${f##*/} ---" >&2
    iconv -f UTF-16LE -t UTF-8 "$f" 2>/dev/null | tail -n 160 | sed -E 's/[0-9]{6,}/[REDACTED_NUMBER]/g; s/(password|passwd|token|authorization)[=: ]+[^ ]+/\1=[REDACTED]/Ig' || tail -n 160 "$f" 2>/dev/null || true
  done
...
while true; do
  ...
  if MT5_ACCOUNT_LOGIN="$MT5_ACCOUNT_LOGIN" MT5_ACCOUNT_SERVER="$MT5_ACCOUNT_SERVER" MT5_LOCAL_PULSE_MAX_AGE_SECONDS=180 \
  ...
  if (( elapsed - last_report >= 30 )); then
    echo "MT5_LOCAL_WAIT_SECONDS=$elapsed" >&2
