...
class CommittedEvidenceTests(unittest.TestCase):
    ...
    def test_the_committed_evidence_is_rechecked_semantically(self):
        """The committed evidence must satisfy the oracle, not merely exist.

        This asserted the opposite, and was right to at the time: the committed
        evidence answered "What is the capital of France?" while the canonical
        golden request asks about Japan, so the verifier refused at the earlier
        gate - it will not grade an answer to a question it was not asked.

        The evidence has since been regenerated by a real run on a host whose
        engine executes, and the answer names Tokyo. So the expectation flips
        with the evidence. The verifier is untouched, and the tests that pin its
        refusals - an empty answer, a wrong answer, a non-canonical request -
        are all still asserting refusal above.
        """
        ...
        # A pass has to come from the oracle actually running, never from it
        # being unavailable and the check falling through.
