...
def _validator(root: Path) -> Draft202012Validator:
    ...
    schema = deepcopy(snapshot_schema)
    schema["properties"]["models"]["items"] = provider_schema
...
def _benchmarked(candidate: dict) -> bool:
    ...
    return any(
        ...
        and any(str(item).strip() for item in row["evidence"])
...
def validate_snapshot(root: Path, snapshot_path: Path, *, expected_source_sha: str | None = None) -> list[str]:
    ...
    if expected_source_sha is not None and snapshot.get("source_sha") != expected_source_sha:
        errors.append(f"source_sha mismatch: expected={expected_source_sha} actual={snapshot.get('source_sha')}")
    if snapshot.get("mode") != "FREE_ONLY":
        errors.append("snapshot mode must be FREE_ONLY")
    if snapshot.get("routing_authority") is not False or snapshot.get("reasoning_authority") is not False:
        errors.append("model mesh snapshot must have zero routing/reasoning authority")

    secret_paths = _secret_paths(snapshot)
    if secret_paths:
        errors.append("snapshot contains forbidden secret-like fields: " + ", ".join(secret_paths[:10]))
    ...
    if isinstance(models, list):
        ...
        for index, candidate in enumerate(models):
            ...
            if key in seen:
                errors.append(f"duplicate provider/model path at models[{index}]: {key[0]}:{key[1]}")
            seen.add(key)
            ordering = (key[0], key[1], str(candidate.get("model_family") or ""))
            if previous is not None and ordering < previous:
                errors.append("snapshot models must be deterministically sorted by provider_id, model_id, model_family")
                break
            previous = ordering
            if candidate.get("provider_class") not in {"F1", "F2", "F3"}:
                errors.append(f"models[{index}] provider_class is not active free capacity")
            if not eligible_free_candidate(candidate, data_class="PUBLIC"):
                errors.append(f"models[{index}] violates FREE_ONLY eligibility")
            if not candidate.get("source_evidence"):
                errors.append(f"models[{index}] missing source evidence")
            if not _benchmarked(candidate):
                errors.append(f"models[{index}] missing benchmark evidence")
    return errors


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Validate sanitized Adaptive Free Model Mesh snapshot")
    ...
    for item in errors:
        print(f"[ERROR] {item}")
    print(f"MODEL_MESH_SNAPSHOT_VALIDATE={'PASS' if not errors else 'FAIL'} errors={len(errors)}")
