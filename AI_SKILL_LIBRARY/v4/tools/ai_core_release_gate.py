...
#: The production Golden workflow writes this. It is the authoritative record of
#: the canonical run.
GOLDEN_PRIMARY = "B3_B4_GOLDEN_E2E_EVIDENCE.json"
#: Kept only so an older checkpoint still resolves. Never preferred.
GOLDEN_FALLBACK = "AI_CORE_E2E_EVIDENCE.json"


def load_golden(root: Path) -> tuple[Any, str]:
    """The golden evidence, and WHICH file it came from.

    Exported because three places used to decide this independently - this gate,
    the continuity tests and the fault-injection tests - and they drifted. The
    fault-injection tests ended up damaging the fallback while the gate read the
    primary, so every one of them passed vacuously: a deliberate break that the
    gate never saw. Anything that needs the golden evidence asks here, so that
    cannot happen again.
    """
    evidence = _load(root, GOLDEN_PRIMARY)
    if evidence is MISSING or _dig(evidence, "ai_core_e2e") is MISSING:
        return _load(root, GOLDEN_FALLBACK), GOLDEN_FALLBACK
    return evidence, GOLDEN_PRIMARY


def gate(root: Path) -> dict[str, Any]:
    """Every check, with what it required and why it did or did not hold."""
    b1 = _load(root, "B1_REAL_INFERENCE_EVIDENCE.json")
    e2e, _golden_source = load_golden(root)
