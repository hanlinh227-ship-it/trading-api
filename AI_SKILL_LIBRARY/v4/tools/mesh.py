...
def resolve_context_nodes(v4_root: Path, primary: str, requested_bridges: list[str], *, profile: str) -> list[str]:
    ...
    legal = {(row.get("from"), row.get("to")) for row in bridges.get("bridges", []) if isinstance(row, dict)}
    result = [primary]
