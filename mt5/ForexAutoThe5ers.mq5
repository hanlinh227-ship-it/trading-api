...
void HandleEntry(string resp){
 string action=Upper(JsonString(resp,"action"));if(action==""||action=="NO_TRADE"||action=="WAIT")return;string symbol=JsonString(resp,"symbol"),side=Upper(JsonString(resp,"side"));double sl=JsonNumber(resp,"sl"),tp=JsonNumber(resp,"tp"),riskPct=JsonNumber(resp,"riskPct"),rr=JsonNumber(resp,"rr"),decisionEntry=JsonNumber(resp,"entry");
 if(action=="PAPER_TRADE"||!InpAllowLiveTrading){Print("FOREX PURE AI PAPER ",symbol," ",side," SL=",sl," TP=",tp," RR=",rr," risk%=",riskPct);return;}if(action!="TRADE"||symbol==""||sl<=0||tp<=0||riskPct<=0)return;
 if(!AlternationOk(side)){Ack("REJECTED",0,symbol,"ALTERNATION_HARD_LOCK","ENTRY",0,side);return;}if(PositionSelect(symbol)){Ack("REJECTED",0,symbol,"DUPLICATE_SYMBOL_POSITION","ENTRY",0,side);return;}MqlTick tick;if(!SymbolSelect(symbol,true)||!SymbolInfoTick(symbol,tick))return;double liveEntry=side=="BUY"?tick.ask:tick.bid;
 if(side=="BUY"&&!(sl<liveEntry&&tp>liveEntry))return;if(side=="SELL"&&!(sl>liveEntry&&tp<liveEntry))return;double atr=FastAtr(symbol);if(decisionEntry>0&&atr>0&&MathAbs(liveEntry-decisionEntry)>atr*InpMaxEntryDriftAtr){Ack("REJECTED",0,symbol,"PRICE_DRIFT_RETHINK","ENTRY",0,side);return;}
