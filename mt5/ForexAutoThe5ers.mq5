...
void HandleEntry(string resp){
 ...
 if(!SideStreakOk(side)){Ack("REJECTED",0,symbol,"SIDE_STREAK_CAP_3","ENTRY",0,side);return;}if(PositionSelect(symbol)){Ack("REJECTED",0,symbol,"DUPLICATE_SYMBOL_POSITION","ENTRY",0,side);return;}if(HasPendingSymbol(symbol)){Ack("REJECTED",0,symbol,"DUPLICATE_SYMBOL_PENDING","ENTRY",0,side);return;}MqlTick tick;if(!SymbolSelect(symbol,true)||!SymbolInfoTick(symbol,tick))return;
