...
string AccountJson(){
  ...
  double riskPct=eq>0?openRiskMoney/eq*100.0:0;return "{\"balance\":"+D(bal,2)+",\"equity\":"+D(eq,2)+",\"margin\":"+D(margin,2)+",\"freeMargin\":"+D(freeMargin,2)+",\"marginLevelPct\":"+D(marginLevel,2)+",\"openRiskPct\":"+D(riskPct,3)+",\"openPositions\":"+(string)count+"}";
}
...
int OnInit(){TerminalId=StringFormat("%I64d-%s",AccountInfoInteger(ACCOUNT_LOGIN),AccountInfoString(ACCOUNT_SERVER));EventSetTimer(MathMax(10,InpPulseSeconds));Print("FOREX AUTO 0.401 dynamic-risk/margin ready: ",TerminalId," LIVE=",InpAllowLiveTrading);return INIT_SUCCEEDED;}
