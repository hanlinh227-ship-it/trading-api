#property strict
#property version   "0.402"
#property description "FOREX AUTO The5ers - independent 3AI bridge with dynamic risk and margin protection."
...
string BarsJson(string symbol,ENUM_TIMEFRAMES tf,int count){
  ...
  for(int i=got-1;i>=0;i--){
    ...
    out+="{\"time\":"+Epoch(r[i].time)+",\"open\":"+D(r[i].open)+",\"high\":"+D(r[i].high)+",\"low\":"+D(r[i].low)+",\"close\":"+D(r[i].close)+",\"volume\":"+IntegerToString((long)r[i].tick_volume)+"}";
  }
  return out+"]";
}
...
string SnapshotJson(string symbol){
  ...
  return "{\"symbol\":\""+Esc(symbol)+"\",\"bid\":"+D(t.bid)+",\"ask\":"+D(t.ask)+",\"last\":"+D(t.last)+",\"timestamp\":"+Epoch(t.time)+",\"newsBlocked\":"+JBool(newsBlocked)+",\"newsCalendarOk\":"+JBool(calendarOk)+",\"bars\":{\"M5\":"+BarsJson(symbol,PERIOD_M5,45)+",\"M15\":"+BarsJson(symbol,PERIOD_M15,45)+",\"H1\":"+BarsJson(symbol,PERIOD_H1,60)+",\"H4\":"+BarsJson(symbol,PERIOD_H4,60)+"}}";
}
...
string RiskKey(ulong ticket){return "FOREX_RISK_"+IntegerToString((long)ticket);}
string MfeKey(ulong ticket){return "FOREX_MFE_"+IntegerToString((long)ticket);}
string MaeKey(ulong ticket){return "FOREX_MAE_"+IntegerToString((long)ticket);}
...
int OnInit(){TerminalId=StringFormat("%I64d-%s",AccountInfoInteger(ACCOUNT_LOGIN),AccountInfoString(ACCOUNT_SERVER));EventSetTimer(MathMax(10,InpPulseSeconds));Print("FOREX AUTO 0.402 valid-json/dynamic-risk ready: ",TerminalId," LIVE=",InpAllowLiveTrading);Pulse();return INIT_SUCCEEDED;}
