...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        if self.path!='/bybit/private' and not self.authorized():return self.sendj(401,{'ok':False,'error':'UNAUTHORIZED'})
        if self.path=='/bybit/private' and not PRIVATE_PROXY_ONLY and not self.authorized():return self.sendj(401,{'ok':False,'error':'UNAUTHORIZED'})
