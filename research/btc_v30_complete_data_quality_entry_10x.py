...
def authority(i,b,I):
    ...
    sw=swing_dir(i,b,12)
    h1=sgn(b[i].c-b[i-12].c);h2=sgn(b[i-12].c-b[i-24].c)
    h4=sgn(b[i].c-b[i-48].c)
...
def signal(i,b,c,I):
    ...
    # HTF alignment must not conflict at entry.
    E=I['e']
    if d>0 and not(E[12][i]>E[36][i] and E[60][i]>E[150][i]):return 0
    if d<0 and not(E[12][i]<E[36][i] and E[60][i]<E[150][i]):return 0
    # Pullback toward value first, then closed M5 continuation confirmation.
    if d>0:
...
def run(b,start,c,I):
    bal=peak=START_BAL;dd=0.;lot=LOT0;tps=tr=0;pos=None
    st=b[start].ts;when=b[start].dt
    for i in range(max(start,WARM+2),len(b)):
        ...
        if hit:
            ...
            if L>=1.-1e-9:
                return R(tps,tps==TARGET,False,'PASS99' if tps==TARGET else 'CHAIN_ERROR',bal,dd*100,tr,L,(z.ts-st)/86400,z.dt)
            lot=round(L+.01,2);pos=None
            # no cooldown: next decision is earliest truthful next M5 open.
    return R(tps,False,False,'DATA_END',bal,dd*100,tr,lot,(b[-1].ts-st)/86400,when)

def idx_at(b,s):
    ts=int(datetime.strptime(s,'%Y-%m-%d').replace(tzinfo=timezone.utc).timestamp())
    lo=0;hi=len(b)
    while lo<hi:
        m=(lo+hi)//2
        if b[m].ts<ts:lo=m+1
...
def calibration_starts(b):
    return [idx_at(b,x) for x in ('2023-02-01','2023-04-01','2023-06-01','2023-08-01')]

def fresh_starts(b,seed=300831):
    lo=idx_at(b,'2023-10-01');hi=idx_at(b,'2024-08-01')
    cand=list(range(lo,hi,12*24)) # one candidate per day
...
def main():
    b=data.load('2023-01-01 00:00:00')
    I=v21.prep(b)
    cs=list(cfgs());cal=calibration_starts(b)
