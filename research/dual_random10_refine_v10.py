...
import random
from research.mt5_progressive_tp_backtest_v3 import load,DATA,cfgs,run,rank,Cfg
from research.mt5_progressive_tp_backtest_v8 import load as load_btc, prep as prep_btc, run as run_btc, cfgs as cfgs_btc, rank as rank_btc
...
def frozen_starts(n:int, count=10):
    ...
    hi=max(1,int(n*0.78))
    xs=sorted(rnd.sample(range(0,hi),count))
    return xs
...
def main():
    ...
    # broad but finite: reuse V3 grid; optimize across all 10 frozen starts, not per-window.
    for c in cfgs():
    ...
    bb=load_btc(); bs=frozen_starts(len(bb)); I=prep_btc(bb)
    print('BTC starts:', [bb[s].dt for s in bs])
    # V8 candidate grid, evaluated on each subwindow independently with recalculated indicators.
    best=None;bestc=None;tested=0
    for c in cfgs_btc():
        tested+=1;rs=[]
        for s in bs:
