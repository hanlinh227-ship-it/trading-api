...
INTERVAL_MS=300_000
# Binance Spot has a verified historical 5m outage on 2023-03-24 12:40..13:55 UTC.
# Research hard-locks the dataset to the first clean bar after that outage; no synthetic fill.
CLEAN_START='2023-03-25 00:00:00'
...
def _get(params,retries=6):
    ...
    for n in range(retries):
        try:
            req=urllib.request.Request(url,headers={'User-Agent':'btc-research-complete-m5/1.1'})
...
def load(start=CLEAN_START, end=None):
    st=int(datetime.strptime(start,'%Y-%m-%d %H:%M:%S').replace(tzinfo=timezone.utc).timestamp()*1000)
    if end is None:
        now=int(time.time()*1000);en=(now//INTERVAL_MS)*INTERVAL_MS-1
    else:
    ...
    while cur<=en:
        ...
        cur=nxt;time.sleep(0.015)
    uniq={int(x[0]):x for x in rows if st<=int(x[0])<=en};xs=[uniq[k] for k in sorted(uniq)]
    ...
    audit(bars,hard=True);return bars
...
def audit(bars,hard=True):
    ...
    expected=(bars[-1].ts-bars[0].ts)//300+1;coverage=100*len(bars)/expected
    ...
    if hard and (len(bars)!=expected or abs(coverage-100)>1e-9):raise RuntimeError('dataset grid is not exactly complete')
