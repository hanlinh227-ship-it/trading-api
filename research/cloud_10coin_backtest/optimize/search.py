...
class CoinResearchResult:
    ...
    def to_dict(self) -> dict:
        if self.locked_profile is None:
            ...
            locked = {"params": self.locked_profile.params, "profile_hash": self.locked_profile.profile_hash}
        return {
            "symbol": self.symbol,
            "status": self.status,
...
def passes_g2_gate(
    *,
    ...
    return completed_trades >= min_trades and rr2_wr >= target_wr and expectancy_r > 0 and validation_ok and holdout_ok
...
def _rank_g2(metrics: Metrics, complexity: int = 1) -> float:
    ...
    return 0.62 * metrics.rr2_wr + 0.23 * metrics.wilson_low + 0.10 * breadth + 0.05 * expectancy_term - complexity_penalty
...
def _validation_row_score(row: dict) -> float:
    ...
    return 0.70 * float(m.get("rr2_wr", 0.0)) + 0.20 * float(m.get("wilson_low", 0.0)) + 0.05 * breadth + 0.05 * expectancy


def choose_validation_members(rows: list[dict], max_members: int = 3) -> list[dict]:
    ranked = sorted(rows, key=lambda r: (_validation_row_score(r), r.get("family", "")), reverse=True)
...
def search_coin_g2(symbol: str, features: pd.DataFrame, config: BacktestConfig) -> CoinResearchResult:
    ...
    for module_name in G2_FAMILIES:
        ...
        family_rows.sort(key=lambda x: (x["score"], x["metrics"]["rr2_wr"], x["metrics"]["completed_trades"]), reverse=True)
    ...
    for row in frontier:
        ...
        local = val.reset_index(drop=True)
        candidates = _with_family_quality(module, row["params"], local)
        trades = _simulate_fast(candidates, local, config)
        vm = summarize_outcomes(trades)
        enriched = {**row, "validation": vm.to_dict()}
        validation_rows.append(enriched)
        val_candidate_cache.append(candidates)

    order = sorted(range(len(validation_rows)), key=lambda i: _validation_row_score(validation_rows[i]), reverse=True)[:6]
    best_combo = None
    ...
    for k in range(1, min(3, len(order)) + 1):
        for combo in combinations(order, k):
            ...
            trades = _simulate_fast(merged, val.reset_index(drop=True), config)
            metrics = summarize_outcomes(trades)
            row_score = 0.70 * metrics.rr2_wr + 0.20 * metrics.wilson_low + 0.05 * min(1.0, metrics.completed_trades / 100.0) + 0.05 * max(-1.0, min(1.0, metrics.expectancy_r / 2.0)) - 0.01 * (k - 1)
    ...
    locked = lock_ensemble([{"family": r["family"], "params": r["params"]} for r in selected_rows])
    dev_metrics, _ = _evaluate_member_rows(locked.members, dev, config)
    hold_metrics, hold_trades = _evaluate_member_rows(locked.members, hold, config)
    ...
    for row in sorted(validation_rows, key=_validation_row_score, reverse=True):
        candidate_hash = lock_profile({"family": row["family"], **row["params"]}).profile_hash
        if candidate_hash not in selected_hashes:
            rejected.append({"family": row["family"], "params": row["params"], "metrics": row["metrics"], "validation": row["validation"], "score": row["score"]})
        if len(rejected) >= 8:
            break
