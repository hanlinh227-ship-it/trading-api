...
# Ten alternate starts spread through the first 80% of the six-month sample.
# Each run gets only data from its start forward; indicators cannot see bars before start.
N=len(bars); starts=[int((N*0.80)*k/10) for k in range(10)]
passed=0
for j,s in enumerate(starts,1):
 sub=bars[s:]
 r=run('XAUUSD',sub,fixed)
 passed+=int(r.finished)
