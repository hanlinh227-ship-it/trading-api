...
class GoogleFlowVideoProvider:
    ...
    def preflight(self) -> ProviderStatus:
        ...
        return ProviderStatus(
            available=ready,
            mode=self.mode,
            reason="ready" if ready else "No authenticated supported Flow video connector/API is available",
    ...
    def submit(self, job: StagedJob) -> ProviderJob:
        self._require_ready()
        raise ProviderExecutionUnavailable("Flow video execution connector has not been bound to this adapter yet")
