...
function sanitize(proposed={}){const out=structuredClone(DEFAULT_TUNING);for(const group of ["signal","hyro"]){for(const [k,r] of Object.entries(RANGE[group]))out[group][k]=clamp(proposed?.[group]?.[k],r,DEFAULT_TUNING[group][k]);}out.version="V77.18.38";return out;}
...
export async function applyAdaptiveTuning(env,proposed,{source="AI_ARBITER",reviewId=null}={}){const values=sanitize(proposed),state={version:"V77.18.38",source,reviewId,values,updatedAt:Date.now(),guardrails:{hardNews:true,freshness:true,executionAuthority:true,structuralSL:true,hyroRiskUntouched:true,tradeAuthority:false,deployAuthority:false,marketEntryBalanced:true}};if(env.TRADING_STATE)await env.TRADING_STATE.put(KEY,JSON.stringify(state));return state;}
export async function getAdaptiveTuningState(env){const x=await readState(env);return x?{...x,values:sanitize(x.values||x),version:"V77.18.38"}:{version:"V77.18.38",source:"DEFAULT",values:structuredClone(DEFAULT_TUNING),updatedAt:null};}
