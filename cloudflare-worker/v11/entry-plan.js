...
export function buildEntryPlan(market,c={}){
 ...
 const forward=nearestForward(side,entry,[c.forwardLiquidity,c.targetStructure,...liquidityLevels,...structureLevels]);
 const preferredRoom=Number(sp.minRR||1.05),executionRoom=Math.max(.95,preferredRoom-.10),naturalRoom=forward?Math.abs(forward-entry)/risk:0;
 const fallbackTarget=Math.max(atr*sp.targetAtr,risk*executionRoom),maxTarget=atr*2.60;let tp=null,reason='WIDE_SCALP_ATR_FALLBACK',targetMode='ATR_FALLBACK';
