...
const PROVIDER_FAILURE=new Set(['V11_QUOTE_NOT_VERIFIED_FRESH','QUOTE_NOT_VERIFIED_FRESH']);
...
export async function scanMarketV11(env,market){const r=await engine(`/run-now?group=${market}`,env);if(!r?.ok||['BUSY','RATE_BUDGET_WAIT'].includes(String(r?.status||'')))return {status:r?.status||'SCAN_FAILED',market,approved:0,providerFailure:0};let approved=0,watch=0,rejected=0,notified=0,providerFailure=0;for(const raw of r.analyses||[]){if(!ACTIONABLE.has(String(raw?.status||'').toUpperCase())||!raw?.symbol)continue;try{const {analysis,quote}=await resolveFreshAnalysis(raw,env),upstreamStatus=String(analysis?.status||raw?.status||'').toUpperCase(),c=normalizeCandidate({...raw,...analysis},quote),plan=buildEntryPlan(market,c);if(!plan.ok||!plan.executable){await recordV11(env,{status:plan.ok?'WATCH':'REJECTED',market,symbol:raw.symbol,plan});plan.ok?watch++:rejected++;continue;}if(!MARKET_READY.has(upstreamStatus)){await recordV11(env,{status:'WATCH',market,symbol:raw.symbol,reason:'UPSTREAM_'+upstreamStatus+'_NOT_MARKET_READY',plan});watch++;continue;}const candidate={...c,symbol:plan.symbol,entry:plan.entry,sl:plan.sl,tp:plan.tp,rr:plan.rr,setup:plan.setup,upstreamStatus};const gate=evaluateMarketCandidate(market,candidate);if(!gate.pass){await recordV11(env,{status:'REJECTED',market,symbol:plan.symbol,reason:gate.reasons?.join('|')||'MARKET_POLICY_REJECTED',plan,gate});rejected++;continue;}const saved=await recordV11(env,{status:'APPROVED_SIGNAL',version:'V11',market,symbol:plan.symbol,candidate,plan,gate,approvedAt:Date.now(),expiresAt:Date.now()+V11_CONFIG.markets[market].horizonMin*60000});approved++;if(saved.stored&&await notify(env,signalText(saved)))notified++;}catch(e){const msg=String(e?.message||e);const isProviderFailure=PROVIDER_FAILURE.has(msg);await recordV11(env,{status:isProviderFailure?'PROVIDER_FAILURE':'REJECTED',market,symbol:raw.symbol,plan:{reason:msg}});isProviderFailure?providerFailure++:rejected++;}}
 return {status:'SCANNED',market,approved,watch,rejected,notified,providerFailure,coverage:{requested:r.requested,broadOk:r.broadOk,deepOk:r.deepOk}};}
