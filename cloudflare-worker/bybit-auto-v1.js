...
export async function runBybitAutoV1(env,{forceScan=false,entryBlockReason=null}={}){
  ...
  for(const candidate of candidateQueue){
    ...
    riskPreflight=bybitRiskPreflight({cfg,equityUsd:equity,state:riskState,candidateRiskUsd:sizing.riskUsd,candidateInitialMarginUsd:sizing.initialMarginUsd,candidateSide:setup.side});
  ...
  const actualRiskGuard=bybitRiskPreflight({cfg,equityUsd:equity,state:riskState,candidateRiskUsd:actualRisk,candidateInitialMarginUsd:sizing.initialMarginUsd,candidateSide:setup.side});if(!actualRiskGuard.ok){await emergencyFlat(api,setup,f.executedQty);return {ok:true,executed:false,mode,reason:"ACTUAL_"+actualRiskGuard.reason,risk:actualRiskGuard,actualRisk,fill:f,preparation,setup,scan,state};}
