...
export async function runBybitAutoV1(env,{forceScan=false}={}){
  ...
  // Freshen only the winning candidate once. This prevents stale scan-entry sizing while also
  // preventing an infinite refresh/re-anchor loop. A second and final quote check happens only
  // after AI review.
  ...
  state.trades=Number(state.trades||0)+1;state.lastTradeAt=now();state.lastFingerprint=fp;state.openPlans={...(state.openPlans||{}),[setup.symbol]:actual};await learn(env,{stage:"LIVE_ACCEPT",mode,symbol:actual.symbol,side:actual.side,strategy:actual.strategy,score:actual.score,rr:actual.rr,riskUsd:actual.riskUsd,rewardUsd:actual.rewardUsd,entry:actual.entry,sl:actual.sl,tp:actual.tp,preparation,ai,postAi,reason:"ORDER_SUBMITTED_PROTECTED_VERIFIED_TRAILING_ARMED"});await put(env,state);
