...
async function reconcileLivePnl(api,state){
  const p=await api.closedPnl(dayStartMs(),now()),list=p?.result?.list||[];
  state.realizedUsd=list.reduce((s,x)=>s+Number(x.closedPnl||0),0);
  state.lastPnlReconcile={at:iso(),closedTrades:list.length,realizedUsd:state.realizedUsd};
}
async function manageLivePositions(env,api,state,positions,cfg){
  ...
  state.lastPositionManagement={at:iso(),results};
  return results;
}
async function verifyProtection(api,symbol,expectedSl,expectedTp){
  ...
  return {ok:true,stopLoss:sl,takeProfit:tp};
}
...
export async function runBybitAutoV1(env,{forceScan=false}={}){
  ...
  if(mode==="LIVE"){
    ...
    try{await reconcileLivePnl(api,state);}catch(e){return {ok:true,executed:false,mode,reason:"DAILY_PNL_RECONCILIATION_FAILED",error:String(e?.message||e),state};}
  ...
  const rewardTp=tpForReward(setup.side,setup.entry,sizing.qty,sizing.rewardUsd),plan={
    mode,symbol:setup.symbol,side:setup.side,qty:sizing.qty,entry:setup.entry,sl:setup.sl,initialSl:setup.sl,tp:rewardTp||setup.tp,structureTp:setup.tp,tickSize:Number(setup.filters?.tickSize||0),filters:setup.filters,
    rr:sizing.targetRR,strategy:setup.strategy,score:setup.score,riskUsd:sizing.riskUsd,rewardUsd:sizing.rewardUsd,riskPreflight,
    ...
    postAiQuote:{px:postAi.px,spreadBps:postAi.spreadBps,driftBps:postAi.driftBps,checkedAt:postAi.checkedAt},createdAt:iso(),createdAtMs:now()
  };
  ...
  if(mode==="PAPER"){
    ...
    await learn(env,{stage:"PAPER_ACCEPT",mode,symbol:setup.symbol,side:setup.side,strategy:setup.strategy,score:setup.score,rr:plan.rr,riskUsd:plan.riskUsd,rewardUsd:plan.rewardUsd,entry:plan.entry,sl:plan.sl,tp:plan.tp,ai,postAi,reason:"PAPER_ORDER_ACCEPTED_AFTER_AI"});await put(env,state);
    return {ok:true,executed:true,paper:true,mode,reason:"PAPER_ORDER_ACCEPTED_AFTER_AI",plan,ai,postAi,risk:riskPreflight,scan,state};
  }
  ...
  const geometry=validateProtectionGeometry({side:setup.side,entry:f.avgPrice,sl,tp});
  if(!geometry.ok||!(actualRR>=cfg.risk.minRR)){await emergencyFlat(api,setup,f.executedQty);return {ok:true,executed:false,mode,reason:geometry.ok?"ACTUAL_RR_INVALID":geometry.reason,actualRR,fill:f,setup,scan,state};}
  const actualRiskGuard=bybitRiskPreflight({cfg,equityUsd:equity,state,candidateRiskUsd:actualRisk});
  if(!actualRiskGuard.ok){await emergencyFlat(api,setup,f.executedQty);return {ok:true,executed:false,mode,reason:"ACTUAL_"+actualRiskGuard.reason,risk:actualRiskGuard,actualRisk,fill:f,setup,scan,state};}
  ...
  let protection;
  try{protection=await verifyProtection(api,setup.symbol,sl,tp);}catch(e){protection={ok:false,reason:"PROTECTION_VERIFY_FAILED",error:String(e?.message||e)};}
  ...
  const actual={...plan,entry:f.avgPrice,qty:f.executedQty,sl,initialSl:sl,tp,rr:actualRR,orderId:f.orderId,riskUsd:actualRisk,rewardUsd:actualReward,riskPreflight:actualRiskGuard,protectionVerified:true,managementPhase:"INITIAL",execution:{status:f.status}};
  ...
  await learn(env,{stage:"LIVE_ACCEPT",mode,symbol:actual.symbol,side:actual.side,strategy:actual.strategy,score:actual.score,rr:actual.rr,riskUsd:actual.riskUsd,rewardUsd:actual.rewardUsd,entry:actual.entry,sl:actual.sl,tp:actual.tp,ai,postAi,reason:"ORDER_SUBMITTED_PROTECTED_VERIFIED"});await put(env,state);
  return {ok:true,executed:true,mode,reason:"ORDER_SUBMITTED_PROTECTED_VERIFIED",plan:actual,ai,postAi,risk:actualRiskGuard,protection,scan,lifecycles,state};
}
