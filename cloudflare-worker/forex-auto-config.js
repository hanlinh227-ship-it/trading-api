export const FOREX_AUTO_VERSION="FOREX-AUTO-0.7.0-FREE-SIDE-2AI";
export const FOREX_AUTO_MODE="AUTONOMOUS_2AI";

export const FOREX_USER_ACTIVE_CAMPAIGN={
  enabled:true,source:"USER_COMMAND_20260827",cycleId:"USER_20260827_510USD_3TRADINGDAYS",
  targetUsd:510,targetPct:null,targetDays:3,tradingDaysOnly:true,excludeWeekend:true,dailyMinProfitPct:1.00
};
...
export const FOREX_AUTO_CONFIG={
  ...
  rules:{
    ...
    prohibitNewsBracketing:true,prohibitHft:true,prohibitTickScalping:true,prohibitArbitrage:true,prohibitLatencyArbitrage:true,
    prohibitReverseArbitrage:true,prohibitHedgeArbitrage:true,prohibitEmulators:true,prohibitMartingale:true,prohibitGridRecovery:true,
    prohibitCopyTrading:true,requireVisibleBrokerStop:true,requireOwnedSource:true,
    alternateTradeSide:false,sidePolicy:"FREE_WITH_ANTI_BIAS_STREAK_CAP",maxConsecutiveSameSide:3,sideBalanceLookback:8,maxSideShareInLookback:0.75
  },
  ...
  ai:{authority:"PURE_AI_ENTRY_AND_POSITION_MANAGEMENT",requireAllTwo:true,consensusPassesRequired:2,discussionMode:"INDEPENDENT_THEN_OPEN_RECONCILIATION",claudeRole:"INDEPENDENT_DISCRETIONARY_TECHNICAL_AND_MACRO_TRADER",chatgptRole:"INDEPENDENT_DISCRETIONARY_TECHNICAL_AND_MACRO_TRADER",chatgptWebResearch:true,claudeWebResearch:true,requireCurrentEconomicContext:true,requireTechnicalReasoning:true,economicResearchIsContextOnly:true,brokerQuoteRemainsExecutionAuthority:true,rawMt5CandlesArePrimaryEvidence:true,ruleBasedSignalAuthority:false,precomputedScoreAuthority:false,indicatorGateAuthority:false,confidenceGateAuthority:false,automatedSetupRankingAuthority:false,requireIndependentReviews:true,noAiOverrideOfHardRules:true,noForcedTrade:true,enforceAlternatingFilledSide:false,antiBiasOnly:true,learningIsContextOnly:true,maximizeReasoningBudget:true,quotaCooldownHours:5,openAiMaxOutputTokens:16000,claudeMaxOutputTokens:16000,parallelProviderCalls:true,macroContextCacheSec:120,avoidRepeatedResearchWithinCache:true,maxEntryCandidatesPerAi:5},
  ...
  execution:{liveEnabled:false,requireBridgeToken:true,decisionTtlSec:8,maxClockSkewSec:4,duplicateCooldownSec:8,defaultSlippagePoints:15,maxEntryDriftAtr:.10,magicNumber:560602}
};
...
export function forexAutoConfig(env={}){
  ...
  c.ai.openAiMaxOutputTokens=Math.max(1000,Math.min(32000,Number(env.FOREX_OPENAI_MAX_OUTPUT_TOKENS||16000)));
  c.ai.claudeMaxOutputTokens=Math.max(1000,Math.min(32000,Number(env.FOREX_CLAUDE_MAX_OUTPUT_TOKENS||16000)));
  c.dailyObjective.enabled=true;c.dailyObjective.minProfitPct=Math.max(1.0,Number(env.FOREX_DAILY_OBJECTIVE_PCT||FOREX_USER_ACTIVE_CAMPAIGN.dailyMinProfitPct||1.0));
  const enabled=String(env.FOREX_USER_TARGET_ENABLED||"").toLowerCase()==="true";
  if(enabled){const targetUsd=Number(env.FOREX_USER_TARGET_USD||0),targetPct=Number(env.FOREX_USER_TARGET_PCT||0),targetDays=Number(env.FOREX_USER_TARGET_DAYS||0),cycleId=String(env.FOREX_USER_TARGET_CYCLE_ID||"USER_SET");if((targetUsd>0||targetPct>0)&&targetDays>0){c.target.enabled=true;c.target.targetUsd=targetUsd>0?targetUsd:null;c.target.targetPct=targetPct>0?targetPct:null;c.target.targetDays=targetDays;c.target.cycleId=cycleId;c.target.source="USER_RUNTIME";}}
  return c;
}
