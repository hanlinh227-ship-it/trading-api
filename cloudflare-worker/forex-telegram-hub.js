...
async function snap(e){
 ...
 const allAi=!!ai?.chatgpt?.configured&&!!ai?.claude?.configured;
 return {cfg,ai,last,state,allAi,mode:cfg.execution.liveEnabled?"LIVE":"PAPER"};
}
function dash(s){const st=s.state||{},a=st.account||{},d=st.dailyObjective||{},t=st.target||{};return [
 ...
 `🖥️ MT5 ${s.last?.connected?"CONNECTED":"NOT CONNECTED"}`,
 ...
 `🕒 ${st?.receivedAt||s.last?.receivedAt||"chưa có pulse"}`,
...
export async function handleForexTelegramHub(req,e){
 ...
 const cb=String(body?.callback_query?.data||"");if(!(cb==="hub:forex"||cb.startsWith("forex:")))return null;
 ...
 if(cb==="hub:forex"||cb==="forex:dashboard")await send(e,chatId,dash(s));
 ...
 return json({ok:true,view:cb,forexStateStore:e.FOREX_STATE?"FOREX_STATE":"TRADING_STATE",twoAi:true});
}
