...
const snapshot={
  schema_version:1,source_sha:'b'.repeat(40),release_id:'4.5.0',generated_at:'2026-09-13T00:00:00Z',fallback_primary_skill:'core_reasoning',
  ...
  skills:{
    core_reasoning:{id:'core_reasoning',display_name:'Giải thích',domain:'core',aliases:['giải thích'],triggers:['explain'],excludes:[],priority:80,requires:['task_router'],conflicts_with:[],tools:[],sources:[],output_contract:'Explain precisely.',primary_selectable:true},
    advertising_copy:{id:'advertising_copy',display_name:'Viết quảng cáo',domain:'writing',aliases:['viết quảng cáo'],triggers:['ad copy'],excludes:[],priority:80,requires:['task_router'],conflicts_with:[],tools:[],sources:[],output_contract:'Write audience-fit advertising copy.',primary_selectable:true},
  },
  capsules:{
...
assert.equal(body.plainLanguagePresentation,true);
assert.equal(body.presentationLocale,'vi');
...
assert.equal(body.route.primarySkillName,'Viết quảng cáo');
