...
export async function runHyroAutoCycle(env,opts={}){
  ...
  try{
    ...
    const management=await manageHyroPositions(env,telemetry).catch(e=>({ok:false,reason:String(e?.message||e),managed:[]}));
    if(control.manualPaused){if(telemetry.openOrders?.length)await cancelHyroPending(env).catch(()=>{});return done(env,base,{reason:"MANUAL_PAUSED",telemetry:telemetryView,management});}
    if(!cfg.autoExecutionRequested)return done(env,base,{reason:"AUTO_EXECUTION_DISABLED",telemetry:telemetryView,management});
    const targetUsd=Number(profile.accountSize||0)*.05;
    if((telemetry.day?.pnlFromDayStart||0)>=targetUsd){if(telemetry.openOrders?.length)await cancelHyroPending(env).catch(()=>{});return done(env,base,{reason:"DAILY_PROFIT_TARGET_REACHED",targetUsd,telemetry:telemetryView,management});}
    const hard=Number(profile.internal?.dailyHardStopUsd||0);
    if(hard>0&&(telemetry.day?.drawdownFromPeak||0)>=hard){if(telemetry.openOrders?.length)await cancelHyroPending(env).catch(()=>{});return done(env,base,{reason:"DAILY_HARD_STOP",hardStopUsd:hard,telemetry:telemetryView,management});}
    const active=new Set([...(telemetry.positions||[]).map(x=>x.symbol),...(telemetry.openOrders||[]).map(x=>x.symbol)]);
    if(active.size>=2)return done(env,base,{reason:"MAX_ACTIVE_SLOTS_REACHED",telemetry:telemetryView,management});
    ...
    const candidates=[...allEligible].sort((a,b)=>(Number(b.rr||0)-Number(a.rr||0))||(Number(b.context?.turnover24h||0)-Number(a.context?.turnover24h||0)));
    const preview=(scan.results||[]).slice(0,3).map(x=>({symbol:x.symbol,status:x.status,side:x.side,rr:Number(x.rr||0),strategy:x.strategy,profile:x.profile,entry:x.entry,sl:x.sl,tp1:x.tp1,tp2:x.tp2,tp3:x.tp3??x.tp,funding:x.context?.funding||null}));
    if(!candidates.length)return done(env,base,{reason:marketOnly?"NO_MARKET_CANDIDATE":"NO_ELIGIBLE_CANDIDATE",candidateCount:0,scanSummary:{broadCount:scan.broadCount,deepCount:scan.deepCount,profile:scan.profile},preview,telemetry:telemetryView,management});
    ...
    if(execution?.executed)return done(env,base,{reason:"ORDER_SUBMITTED",executed:true,execution:execution.order,candidateCount:candidates.length,scanSummary:{broadCount:scan.broadCount,deepCount:scan.deepCount,profile:scan.profile},preview,telemetry:telemetryView,management});
    return done(env,base,{reason:lastError?"EXECUTION_REJECTED":"CANDIDATES_BLOCKED",lastError,lastGate,candidateCount:candidates.length,scanSummary:{broadCount:scan.broadCount,deepCount:scan.deepCount,profile:scan.profile},preview,telemetry:telemetryView,management});
