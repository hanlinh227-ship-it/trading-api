...
export async function runHyroAutoCycle(env,opts={}){
  const marketOnly=!!opts.marketOnly,excluded=new Set((opts.excludedSignatures||[]).map(String));
  const startedAt=Date.now(),cfg=hyroExecutionConfig(env),base={ok:true,executed:false,mode:cfg.mode,startedAt,silentTelegram:true,source:marketOnly?"HYRO_QUICK_ABC_MICRO_PORTFOLIO":"HYRO_INDEPENDENT_SCANNER_MICRO_PORTFOLIO",marketOnly,accountId:opts.accountId||env.HYRO_ACCOUNT_ID||"A"};
  try{
    ...
    const rawScan=await hyroDynamicScan({maxBroad:100,maxDeep:Number(adaptive?.hyro?.maxDeep||14),minTurnover:Number(adaptive?.hyro?.minTurnover||6000000),tune:adaptive?.hyro||{}}),enriched=await enrichHyroPlans(rawScan.results||[],{limit:6}),scan={...rawScan,results:enriched,tiers:enriched.reduce((a,x)=>(a[x.tier||"C"]=(a[x.tier||"C"]||0)+1,a),{})};
