...
export async function runHyroAutoCycle(env,opts={}){
  ...
  try{
    ...
    const allEligible=(scan.results||[]).filter(x=>{if(active.has(x.symbol)||excluded.has(sig(x)))return false;if(marketOnly)return ["MARKET_PLAN","NEAR_MARKET_PLAN"].includes(x.status);if(["MARKET_PLAN","LIMIT_PLAN"].includes(x.status))return true;return x.status==="NEAR_MARKET_PLAN"&&microScore(x)>=.58;});
