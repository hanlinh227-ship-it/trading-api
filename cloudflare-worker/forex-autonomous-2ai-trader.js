...
function entriesOf(x={}){const out=[];if(Array.isArray(x?.entryCandidates))for(const e of x.entryCandidates.slice(0,5)){const z=entryOf(e);if(z.decision==="ENTER"&&z.symbol!=="NONE"&&z.side)out.push(z);}const best=entryOf(x?.entry||{});if(best.decision==="ENTER"&&best.symbol!=="NONE"&&best.side&&!out.some(z=>z.symbol===best.symbol&&z.side===best.side&&z.orderType===best.orderType))out.unshift(best);return out.slice(0,5);}
...
function entryConsensus(g,c,snap,minRR,requiredSide){
  const gs=entriesOf(g),cs=entriesOf(c);
