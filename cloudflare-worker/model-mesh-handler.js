...
export function createModelMeshHandler({skillSnapshot,modelSnapshot,activeIndex,routeSkill,executeWorkers=null,probeProviders=null}={}){
  if(!skillSnapshot||!modelSnapshot||!activeIndex||typeof routeSkill!=='function')throw new Error('MODEL_MESH_HANDLER_CONFIG_REQUIRED');
  if(activeIndex.source_sha!==modelSnapshot.source_sha)throw new Error('MODEL_MESH_SOURCE_SHA_MISMATCH');
  return async function handleModelMesh(request,env={},ctx={}){
    ...
    if(url.pathname==='/brain/mesh/health'){
      ...
      return json({
        ...
        capabilityEvidence:capabilityEvidenceDiagnostics(activeIndex),
    ...
    if(url.pathname==='/brain/mesh/plan'){
      ...
      const plan=await buildModelMeshPlan({...body,dataClass,profile:route.profile,route},{skillSnapshot,modelSnapshot,activeIndex,env});
    ...
    if(url.pathname==='/brain/mesh/execute'){
      ...
      return executeWorkers(request,env,{skillSnapshot,modelSnapshot,activeIndex,routeSkill,ctx});
    }
    return null;
  };
}
