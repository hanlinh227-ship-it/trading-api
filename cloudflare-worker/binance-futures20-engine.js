...
function analyze(symbol,r1,r5,bid,ask,cfg,metrics){
  ...
  if(trend>0&&near&&impUp){side="BUY";strategy=`SCALP:${profile.family}:TREND_CONTINUATION`;score=78;}
  else if(trend<0&&near&&impDn){side="SELL";strategy=`SCALP:${profile.family}:TREND_CONTINUATION`;score=78;}
  else{const look=r1.slice(-26,-3),hi=Math.max(...look.map(x=>x[2])),lo=Math.min(...look.map(x=>x[3]));if(c[4]>hi&&c[4]-hi<=a1*profile.maxChaseAtr){side="BUY";strategy=`SCALP:${profile.family}:BREAKOUT`;score=75;}else if(c[4]<lo&&lo-c[4]<=a1*profile.maxChaseAtr){side="SELL";strategy=`SCALP:${profile.family}:BREAKOUT`;score=75;}}
  if(!side)return null;score+=clamp(Math.abs(ctxFast-ctxSlow)/Math.max(a5,1e-9)*8,0,10);if(score<profile.minScore)return null;
  const entry=side==="BUY"?ask:bid,exitPlan=buildScalpExitPlan({side,entry,atr:a1,r1,rrFloor:Math.max(1.15,Number(cfg.risk.minRR||1.15)),rrCap:Math.min(2.0,Number(profile.rr||1.6)+.25)});if(!exitPlan)return null;
  return {symbol,side,strategy,profile:profile.family,score:Math.round(score),entry,sl:exitPlan.sl,tp:exitPlan.tp,rr:exitPlan.rr,atr1:a1,spreadBps,riskWeight:profile.riskWeight,exitPlan,liquidity:{quoteVolume:Number(metrics?.quoteVolume||0),universeSpreadBps:Number(metrics?.spreadBps||spreadBps)}};
}
