...
async function notifyPendingLiveEntries(env,out,state={}){
  ...
  for(const p of pending){
    ...
    const text=[reconciled?`🔄 BTCUSDT ${side} • RECONCILED`:`${icon} BTCUSDT ${side} • LIVE`,`Entry ${compactPrice(p.entry,tick)} • ${Number(p.qty||0).toFixed(6)} BTC • ${Number(p.leverage||0)}x`,`TP ${compactPrice(p.tp,tick)} • +$${Math.max(0,net).toFixed(2)} net`,`SL ${compactPrice(p.sl,tick)} • -${usd(p.riskUsd)}`,cost>0?`Cost reserve ~${usd(cost)} • RR ${Number(p.rr||0).toFixed(2)}`:`RR ${Number(p.rr||0).toFixed(2)}`,`${p.setup||"BTC_STRUCTURE_FLOW"} • ${p.regime||"REGIME"}`,route?`Route ${route}`:null,`${BYBIT_AUTO_VERSION}`].filter(Boolean).join("\n");
    try{await telegramApiRequest(env,"sendMessage",{chat_id:env.TELEGRAM_CHAT_ID,text,disable_web_page_preview:true});notified.add(orderId);results.push({sent:true,orderId,reconciled});}
...
async function notifyClosedPnl(env,mode,previous={},walletAfter=0){
  ...
  try{const api=bybitV5(env),start=Math.max(now-6*3600000,last-120000),r=await api.closedPnl(start,now),rows=(r?.result?.list||[]).filter(x=>String(x.symbol)==="BTCUSDT"&&Number(x.updatedTime||x.createdTime||0)>last-1000),sent=[];for(const x of rows){const id=String(x.orderId||x.execId||`${x.updatedTime}:${x.avgExitPrice}:${x.closedPnl}`),pnl=Number(x.closedPnl||0),cause=classifyClose(x,previous),side=String(x.side||""),entry=Number(x.avgEntryPrice||0),exit=Number(x.avgExitPrice||0),qty=Number(x.qty||x.closedSize||0);sent.push(await sendOnce(env,`BTC:CLOSED:${id}`,[pnl>=0?`✅ BTC CLOSED ${signedUsd(pnl)}`:`❌ BTC CLOSED ${signedUsd(pnl)}`,`Cause ${cause}`,`${side.toUpperCase()} ${qty?qty.toFixed(6):"—"} BTC • ${compactPrice(entry)} → ${compactPrice(exit)}`,`Balance ≈ $${Number(walletAfter||0).toFixed(2)}`,`${BYBIT_AUTO_VERSION}`].join("\n"),{symbol:"BTCUSDT",action:"CLOSED",cause,pnlUsd:pnl}));}return {checkedAtMs:now,baseline:false,notifications:sent,rows:rows.length};}
