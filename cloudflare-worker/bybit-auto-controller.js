...
export async function runBybitAutoControlled(env,opts={}){
  const balanceReconcile=await reconcileBtcAccountBalance(env);
  ...
  const previous=await kvGet(env,CONTROL_KEY,{}),controller={...previous,executionMode:out?.mode||mode,requestedLive,btcLiveAck:btcAck,liveAuthority:requestedLive&&btcAck?"BTC_ONLY":"PAPER_SAFE_MIGRATION",entrySpacingSec:spacingMs/1000,entryBlockReason,unlimitedDailyEntries:true,frequencyAuthority:"ACTIVE_RISK_MARGIN_DRAWDOWN_NOT_TRADE_COUNT",managementAlwaysOn:true,legacyMultiCoinDisabled:true,symbol:"BTCUSDT",strategyAuthority:"MARKET_STRUCTURE_ORDERFLOW_DERIVATIVES_MICROSTRUCTURE",balanceAuthority:"BYBIT_WALLET_PLUS_TRANSACTION_LOG",depositWithdrawalAware:true,balanceReconcile:{ok:!!balanceReconcile?.ok,reason:balanceReconcile?.reason||null,netExternalCashFlowUsd:Number(balanceReconcile?.netExternalCashFlowUsd||0),walletBalanceUsd:Number(balanceReconcile?.snapshot?.walletBalanceUsd||finalState?.lastWalletBalanceUsd||0)||null,equityUsd:Number(balanceReconcile?.snapshot?.equityUsd||out?.equity||finalState?.lastEquityUsd||0)||null,availableUsd:Number(balanceReconcile?.snapshot?.availableUsd||finalState?.lastAvailableUsd||0)||null},telegramNotification,lifecycleNotifications,equityUsd:Number(out?.equity||finalState?.lastEquityUsd||0)||null,walletBalanceUsd:Number(finalState?.lastWalletBalanceUsd||balanceReconcile?.snapshot?.walletBalanceUsd||0)||null,availableUsd:Number(finalState?.lastAvailableUsd||balanceReconcile?.snapshot?.availableUsd||0)||null,highWaterUsd:Number(finalState?.highWaterUsd||0)||null,activeTranches:Array.isArray(finalState?.tranches)?finalState.tranches.filter(x=>String(x.status||"OPEN")==="OPEN").length:0,lastCycleAt:iso(),lastCycleReason:String(out?.reason||"UNKNOWN"),lastCycleExecuted:!!out?.executed,lastScan:scanTelemetry(out?.scan||{}),consecutiveSchedulerErrors:0,lastSchedulerError:null,runtimeRevision:String(env.RUNTIME_REVISION||"UNKNOWN")};await kvPut(env,CONTROL_KEY,controller);return {...out,balanceReconcile,controller};
}
