...
function entryCandidateFromTranche(t={},state={}){return {orderId:String(t.orderId||t.id||""),side:t.side,qty:Number(t.qty||0),entry:Number(t.entry||0),sl:Number(t.managedSl||t.sl||0),tp:Number(t.tp||state.virtualTarget||0),riskUsd:Number(t.riskUsd||t.initialRiskUsd||0),rr:Number(t.rr||0),leverage:Number(t.leverage||0),setup:t.setup||"BTC_STRUCTURE_FLOW",regime:t.regime||state.lastRegime||"REGIME",tickSize:Number(t.tickSize||0),capitalBaseUsd:Number(t.capitalBaseUsd||state.lastCapitalBaseUsd||0),scaleRiskMult:Number(t.scaleRiskMult||0),reconciledExternalPosition:!!t.reconciledExternalPosition,createdAtMs:Number(t.createdAt||0)};}
...
async function notifyPendingLiveEntries(env,out,state={}){
  ...
  for(const p of pending){
    const orderId=String(p.orderId),side=String(p.side||"").toUpperCase()==="BUY"?"BUY":"SELL",icon=side==="BUY"?"🟢":"🔴",tick=Number(p.tickSize||0),tranches=openTranches(state).length||1,capital=Number(p.capitalBaseUsd||state.lastCapitalBaseUsd||0),scale=Number(p.scaleRiskMult||p.riskMult||0),reconciled=!!p.reconciledExternalPosition;
    const text=[reconciled?`🔄 BTCUSDT ${side} • LIVE POSITION RECONCILED`:`${icon} BTCUSDT ${side} • tranche ${tranches}`,`Entry ${compactPrice(p.entry,tick)} • qty ${Number(p.qty||0).toFixed(6)} BTC`,`SL ${compactPrice(p.sl,tick)} • risk ${usd(p.riskUsd)}`,`Virtual target ${compactPrice(p.tp,tick)} • RR ${Number(p.rr||0).toFixed(2)}`,`${Number(p.leverage||0)}x • ${p.setup||"BTC_STRUCTURE_FLOW"}`,`${p.regime||"REGIME"} • winner-pyramid / risk-recycle`,reconciled?`Nguồn: vị thế Bybit được đồng bộ lại vào tranche ledger`:null,capital>0?`Capital base $${capital.toFixed(2)}${scale>0?` • scale ×${scale.toFixed(3)}`:""}`:null,`${BYBIT_AUTO_VERSION} • LIVE`].filter(Boolean).join("\n");
