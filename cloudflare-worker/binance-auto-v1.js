...
import {dailyTargetPolicy} from "./binance-daily-target.js";
...
export async function runBinanceAutoV1(env){
  ...
  if(stop){await put(env,state);return {ok:true,executed:false,mode,reason:stop,target:dailyTargetPolicy(env,state,0),state};}
  ...
  const target=dailyTargetPolicy(env,state,equity);
  if(target.reached){await put(env,state);return {ok:true,executed:false,mode,reason:"DAILY_TARGET_REACHED",target,state};}
  if(mode!=="PAPER"&&positions.length>=cfg.maxOpenPositions)return {ok:true,executed:false,mode,reason:"MAX_OPEN_POSITIONS",positions:positions.length,target,state};

  const scan=await scanBinance20(env),setup=scan.best;
  if(!setup)return {ok:true,executed:false,mode,reason:scan.reason||"NO_SETUP",scan,target,state};
  if(Number(setup.rr||0)<cfg.risk.minRR)return {ok:true,executed:false,mode,reason:"RR_TOO_LOW",setup,scan,target,state};

  const sizing=sizeBinance20(setup,setup.filters,cfg,equity);
  if(!sizing.ok)return {ok:true,executed:false,mode,reason:sizing.reason,setup,sizing,scan,target,state};

  const fingerprint=`${setup.symbol}:${setup.side}:${setup.strategy}:${Math.round(Number(setup.entry||0)*1e6)}`;
  if(state.lastFingerprint===fingerprint&&now()-Number(state.lastTradeAt||0)<cfg.execution.cooldownSec*1000)return {ok:true,executed:false,mode,reason:"DUPLICATE_COOLDOWN",setup,scan,target,state};
  ...
  if(mode==="PAPER"){
    ...
    return {ok:true,executed:true,paper:true,mode,reason:"PAPER_ORDER_ACCEPTED",plan,scan,target,state};
  }
  ...
  return {ok:true,executed:true,mode,reason:"ORDER_SUBMITTED_AND_PROTECTED",plan:{...plan,orderId:entry.orderId},scan,target,state};
}
