...
const fetchImpl=async (url,init={})=>{
  ...
  if(u.pathname==='/brain/project/state'&&init.method==='GET'){
    if(!projectState)return new Response(JSON.stringify({ok:false,error:'project_not_initialized'}),{status:404});
  ...
  if(u.pathname==='/brain/project/state'&&init.method==='PUT'){
    const body=JSON.parse(init.body);
    const currentVersion=projectState?.version||0;
  ...
  if(u.pathname==='/brain/bootstrap'){
    if(!projectState)return new Response(JSON.stringify({ok:true,clientId:client,sourceSha:expectedSha,project:{initialized:false,state:null},jobRefs:[],nextAction:null}),{status:200});
...
assert.equal(seen.filter(x=>x.path==='/brain/project/state'&&x.method==='GET').length,1);
