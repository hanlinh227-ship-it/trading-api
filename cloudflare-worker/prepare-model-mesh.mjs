...
// M7: endpoint_family was owned by two registries that could drift
// independently. The binding registry is the single owner (it is what actually
// dispatches); the snapshot must agree, or the health fingerprint silently
// invalidates itself on the next change.
...
// Compile the canonical policy into the runtime contract so `policy.yaml` is
// the single authority for parallelism and selection filters, instead of the
// same numbers being hard-coded in contracts.js, the health response and CI.
...
console.log(`MODEL_MESH_ESM=PASS source_sha=${snapshot.source_sha} models=${(snapshot.models||[]).length} bindings=${Object.keys(bindingRegistry.bindings).length} max_parallel=${JSON.stringify(runtimePolicy.max_parallel)} filters=${runtimePolicy.selection_filters.length}`);
