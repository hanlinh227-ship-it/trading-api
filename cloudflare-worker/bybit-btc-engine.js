...
function leverageFor(cfg,setup,ddMult=1,scale={}){
  let x=setup?.strength==="A_PLUS"?num(cfg.leverage?.aPlus||11):setup?.strength==="STRONG"?num(cfg.leverage?.strong||8):num(cfg.leverage?.normal||6);
  x+=Math.max(0,Math.round(num(scale?.leverageBonus)));
  if(setup?.regime==="RANGE"||setup?.regime==="SQUEEZE")x=Math.min(x,6);
  if(ddMult<.8)x=Math.min(x,5);
  if(ddMult<.6)x=Math.min(x,4);
  return Math.max(num(cfg.leverage?.min)||1,Math.min(num(cfg.leverage?.max)||15,x));
}
...
async function manageCluster(env,api,state,position,market,filters){
  ...
  const side=String(position.side),mark=num(position.markPrice||market.mark||market.price),currentStop=num(position.stopLoss||state.aggregateStop),t=openTranches(state),latest=t.at(-1);
...
export async function runBtcHyperscale(env,{entryBlockReason=null}={}){
  ...
  state.lastCapitalBaseUsd=num(risk.capital?.capitalBaseUsd||preRisk.capital?.capitalBaseUsd);state.lastScaleTierUsd=num(risk.scale?.tierEquityUsd||preRisk.scale?.tierEquityUsd);
...
export const BTC_HYPERSCALE_ENGINE_VERSION="BYBIT-BTC-HYPERSCALE-2.5";
