...
function analyze(symbol,r1,r5,bid,ask,cfg,metrics,learningSummary={}){
 ...
 const exitSelection=selectExitProfile(edge,regimeState.regime),edgeConfidence=Number(threshold.confidence||0),edgeNetR=Number(edge?.avgNetR??edge?.avgR??0),executionQuality=liquidityQualityBonus(metrics),rankingScore=score-threshold.threshold+clamp(edgeNetR*3,-3,3)*edgeConfidence+Math.min(3,Number(exitPlan.rr||0))+executionQuality;
 return {symbol,side,strategy,strategyBase,setupType,profile:profile.family,score:Math.round(score),adaptiveThreshold:threshold.threshold,rankingScore,executionQuality,entry,sl:exitPlan.sl,tp:exitPlan.tp,rr:exitPlan.rr,atr1:a1,atrPct,minAtrPct,spreadBps,exitPlan,regime:regimeState.regime,regimeState,betaCluster:betaCluster(symbol),exitProfile:exitSelection.profile,exitProfileReason:exitSelection.reason,edge:{trades:Number(edge?.trades||0),winRate:edge?.winRate??null,avgR:edge?.avgR??null,avgNetR:edge?.avgNetR??null,avgMfeR:edge?.avgMfeR??null,avgMaeR:edge?.avgMaeR??null,confidence:edgeConfidence},returns1m:returnsFromRows(r1),context:{...ctx,vwapAligned,confluence:conf.reasons},liquidity:{turnover24h:metrics.turnover24h,spreadBps:metrics.spreadBps}};
}
...
export function sizeBybitAuto(setup,cfg,equityUsd=50){
  ...
  const reservePct=clamp(Number(cfg.risk.minFreeReservePct??0),0,20),feeBufferPct=clamp(Number(cfg.risk.feeBufferPct||4),2,12),slotCeilingPct=Math.min(Number(cfg.risk.maxMarginPerPositionPct||100),100-reservePct);
  const grossMarginBudgetUsd=equity*slotCeilingPct/100,marginBudgetUsd=grossMarginBudgetUsd;

  const targetRiskPct=equityRiskCurvePct(equity,cfg),configuredMaxRiskPct=Math.max(.1,Number(cfg.risk.maxRiskPctOfEquity||8)),maxRiskPct=Math.min(configuredMaxRiskPct,Math.max(targetRiskPct,targetRiskPct*1.35));
  ...
  const riskUsd=qty*dist,structureRewardUsd=structureTp>0?qty*Math.abs(structureTp-entry):Infinity,minRR=Math.max(1,Number(cfg.risk.minRR||1.5)),minRiskUtilizationPct=minimumRiskUtilizationPct(equity,cfg),minAcceptedRiskUsd=targetRiskUsd*minRiskUtilizationPct/100;
