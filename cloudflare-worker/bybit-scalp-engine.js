...
function analyze(symbol,r1,r5,bid,ask,cfg,metrics,learningSummary={}){
 ...
 const entry=side==="Buy"?ask:bid,exitPlan=buildScalpExitPlan({side:sideUpper,entry,atr:a1,r1,rrFloor:Number(cfg.risk.minRR||1),preferredRR:Number(cfg.risk.preferredRR||2),rrCap:Number(cfg.risk.maxRR||3)});if(!exitPlan||Number(exitPlan.rr||0)<cfg.risk.minRR)return null;
