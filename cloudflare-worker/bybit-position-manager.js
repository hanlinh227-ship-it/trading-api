...
function adaptiveTrailDistanceR(r,base){if(r>=2.4)return Math.min(base,.30);if(r>=1.8)return Math.min(base,.36);if(r>=1.4)return Math.min(base,.44);return base;}
...
export async function manageBybitScalpPosition(env,api,plan,position,cfg){
  ...
  const plannedBe=num(plan?.exitPlan?.breakEvenTriggerR),plannedTrailAt=num(plan?.exitPlan?.positiveTrailTriggerR),plannedLockR=num(plan?.exitPlan?.positiveTrailLockR),plannedTrailAtr=num(plan?.exitPlan?.trailAtr),atr1=num(plan?.atr1),beAt=Math.max(.45,Number(env.BYBIT_BE_TRIGGER_R||plannedBe||.60)),trailAt=Math.max(beAt+.20,Number(env.BYBIT_TRAIL_TRIGGER_R||plannedTrailAt||1.15)),defaultLockAt=Math.max(beAt+.15,Math.min(trailAt-.05,(beAt+trailAt)/2)),lockAt=Math.max(beAt+.15,Number(env.BYBIT_PROFIT_LOCK_TRIGGER_R||defaultLockAt)),beOffsetR=clamp(Number(env.BYBIT_BE_OFFSET_R||.05),0,.20),lockR=clamp(Number(env.BYBIT_PROFIT_LOCK_R||plannedLockR||.35),.10,.85),plannedTrailR=plannedTrailAtr>0&&atr1>0?plannedTrailAtr*atr1/initialRisk:.50,baseTrailR=clamp(Number(env.BYBIT_TRAIL_DISTANCE_R||plannedTrailR||.50),.25,1.0),trailDistanceR=adaptiveTrailDistanceR(r,baseTrailR);let phase="INITIAL",nextSl=currentSl,trailingStop=0;
  if(r>=beAt){phase="BREAKEVEN";nextSl=side==="Buy"?entry+initialRisk*beOffsetR:entry-initialRisk*beOffsetR;}if(r>=lockAt){phase="PROFIT_LOCK";let dynamicLock=lockR;if(r>=1.5)dynamicLock=Math.max(dynamicLock,.65);if(r>=2)dynamicLock=Math.max(dynamicLock,1.0);const lock=side==="Buy"?entry+initialRisk*dynamicLock:entry-initialRisk*dynamicLock;if(betterStop(side,lock,nextSl))nextSl=lock;}if(r>=trailAt){phase="TRAIL";trailingStop=initialRisk*trailDistanceR;}
  nextSl=roundTick(nextSl,tick);trailingStop=roundTick(trailingStop,tick);const stopTighter=betterStop(side,nextSl,currentSl),trailTighter=phase==="TRAIL"&&trailingStop>0&&(!(currentTrailing>0)||trailingStop<currentTrailing-Math.max(tick/2,1e-12)),shouldTighten=stopTighter||trailTighter,verdict=shouldTighten?"TIGHTEN":"HOLD",reason=shouldTighten?(trailTighter?"ADAPTIVE_TRAILING":"PROTECTIVE_STOP_ADVANCE"):(phase==="TRAIL"&&currentTrailing>0?"TRAIL_ALREADY_TIGHT":cut.candidate?"SMART_CUT_CONFIRMING":momentum.adverseTrend?"HOLD_NO_CUT_CONFIRMATION":"HOLD_THESIS_INTACT");
