...
function adaptiveTrailDistanceR(r,base){if(r>=3)return clamp(base*.78,.55,.90);if(r>=2.4)return clamp(base*.88,.60,.95);return base;}
function dynamicCutThresholdR(ageSec,env){const early=Math.abs(clamp(Number(env.BYBIT_SMART_CUT_EARLY_R||.72),.62,.85)),mature=Math.abs(clamp(Number(env.BYBIT_SMART_CUT_MATURE_R||.62),.52,.78)),stale=Math.abs(clamp(Number(env.BYBIT_SMART_CUT_STALE_R||.56),.48,.72));if(ageSec<300)return -early;if(ageSec<600)return -mature;return -stale;}
...
function pendingCut(plan,cfg,env){
  ...
  const ageSec=(now()-ts)/1000,baseWait=Math.max(60,Math.min(300,Number(env.BYBIT_SMART_CUT_REISSUE_SEC||cfg?.risk?.smartCutReissueSec||120)));
...
export async function manageBybitScalpPosition(env,api,plan,position,cfg){
  ...
  const r=favorableR(side,entry,mark,initialRisk),peakR=Math.max(num(plan?.peakR),r),createdAtMs=num(plan?.createdAtMs)||now(),ageSec=Math.max(0,(now()-createdAtMs)/1000);plan.peakR=peakR;
  const pending=pendingCut(plan,cfg,env);if(pending){plan.lastReview={at:new Date().toISOString(),verdict:"CUT_PENDING",reason:"SMART_CUT_ORDER_PENDING",r,peakR,ageSec,pending};return {managed:false,verdict:"CUT_PENDING",reason:"SMART_CUT_ORDER_PENDING",r,peakR,ageSec,markPrice:mark,pending};}
  let momentum={available:false,aligned:null,adverseTrend:false,adverseBars:0,momentumR:0,structureBroken:false};try{momentum=momentumReview(side,parseKlines(await api.kline(plan.symbol,"1",14)),initialRisk);}catch{}
  const cut=smartCutAssessment({env,cfg,r,ageSec,momentum,plan});plan.smartCutCandidateCount=cut.candidate?cut.confirmations:0;plan.lastSmartCutAssessment={at:new Date().toISOString(),...cut,r,ageSec,momentum};
  if(cut.eligible){
    const closeSide=side==="Buy"?"Sell":"Buy",positionIdx=Number(position?.positionIdx??cfg.execution.positionIdx??0),order=await api.order({symbol:plan.symbol,side:closeSide,orderType:"Market",qty:String(qty),reduceOnly:true,positionIdx,timeInForce:"IOC"}),orderId=String(order?.result?.orderId||""),cutReason=cut.emergency?"SMART_CUT_EMERGENCY_INVALIDATION":"SMART_CUT_CONFIRMED_INVALIDATION";
    ...
    plan.lastReview={at:plan.cutRequestedAt,verdict:fill.ok?"CUT":"CUT_PENDING",reason:fill.ok?cutReason:(fill.partial?"SMART_CUT_PARTIAL_FILL":"SMART_CUT_ORDER_PENDING"),r,peakR,ageSec,momentum,smartCut:cut,closeFill:fill,discretionaryCutEnabled:true};
    return {managed:true,verdict:fill.ok?"CUT":"CUT_PENDING",cutExecuted:fill.ok,cutRequested:true,reason:fill.ok?cutReason:(fill.partial?"SMART_CUT_PARTIAL_FILL":"SMART_CUT_ORDER_PENDING"),r,peakR,ageSec,markPrice:mark,orderId,momentum,smartCut:cut,closeFill:fill};
  }
  const plannedBe=num(plan?.exitPlan?.breakEvenTriggerR),plannedTrailAt=num(plan?.exitPlan?.positiveTrailTriggerR),plannedLockR=num(plan?.exitPlan?.positiveTrailLockR),plannedTrailAtr=num(plan?.exitPlan?.trailAtr),atr1=num(plan?.atr1),warmupSec=Math.max(120,Number(env.BYBIT_PROTECTION_WARMUP_SEC||120));
  if(ageSec<warmupSec){plan.lastReview={at:new Date().toISOString(),verdict:"HOLD",reason:"PROTECTION_WARMUP",r,peakR,ageSec,warmupSec,momentum,smartCut:cut};return {managed:false,verdict:"HOLD",reason:"PROTECTION_WARMUP",phase:"INITIAL",r,peakR,ageSec,warmupSec,currentSl,currentTrailing,markPrice:mark,momentum,smartCut:cut};}
  const beAt=Math.max(1.30,Number(env.BYBIT_BE_TRIGGER_R||plannedBe||1.30)),trailAt=Math.max(beAt+.40,Number(env.BYBIT_TRAIL_TRIGGER_R||plannedTrailAt||1.85)),defaultLockAt=Math.max(beAt+.25,Math.min(trailAt-.15,(beAt+trailAt)/2)),lockAt=Math.max(beAt+.25,Number(env.BYBIT_PROFIT_LOCK_TRIGGER_R||defaultLockAt)),beOffsetR=clamp(Number(env.BYBIT_BE_OFFSET_R||.02),0,.10),lockR=clamp(Number(env.BYBIT_PROFIT_LOCK_R||plannedLockR||.30),.15,.70),plannedTrailR=plannedTrailAtr>0&&atr1>0?plannedTrailAtr*atr1/initialRisk:.70,baseTrailR=clamp(Number(env.BYBIT_TRAIL_DISTANCE_R||plannedTrailR||.70),.45,1.10),trailDistanceR=adaptiveTrailDistanceR(r,baseTrailR),protectionConfirmed=momentum.available&&momentum.aligned&&momentum.momentumR>=-.02;let phase="INITIAL",nextSl=currentSl,trailingStop=0;
  if(r>=beAt&&protectionConfirmed){phase="BREAKEVEN";nextSl=side==="Buy"?entry+initialRisk*beOffsetR:entry-initialRisk*beOffsetR;}if(r>=lockAt&&protectionConfirmed){phase="PROFIT_LOCK";let dynamicLock=lockR;if(r>=2)dynamicLock=Math.max(dynamicLock,.55);if(r>=2.6)dynamicLock=Math.max(dynamicLock,.80);const lock=side==="Buy"?entry+initialRisk*dynamicLock:entry-initialRisk*dynamicLock;if(betterStop(side,lock,nextSl))nextSl=lock;}if(r>=trailAt&&protectionConfirmed){phase="TRAIL";trailingStop=initialRisk*trailDistanceR;}
  ...
  plan.lastReview={at:new Date().toISOString(),verdict,reason,r,peakR,ageSec,phase,currentSl,nextSl:stopTighter?nextSl:currentSl,currentTrailing,trailingStop:trailTighter?trailingStop:currentTrailing||null,momentum,smartCut:cut,discretionaryCutEnabled:cut.enabled,protectionConfirmed,thresholds:{warmupSec,beAt,lockAt,trailAt,lockR,trailDistanceR}};
  if(!shouldTighten)return {managed:false,verdict,reason,phase,r,peakR,ageSec,currentSl,currentTrailing,markPrice:mark,momentum,smartCut:cut,protectionConfirmed,thresholds:{warmupSec,beAt,lockAt,trailAt,lockR,trailDistanceR}};
