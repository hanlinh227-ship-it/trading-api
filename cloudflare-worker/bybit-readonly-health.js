...
export async function handleBybitReadonlyHealth(req,env){
  ...
  const creds=bybitCredentials(env),mode=bybitExecutionMode(env),api=bybitV5(env);
  if(!(creds.apiKey&&creds.apiSecret))return json({ok:false,readOnly:true,authenticated:false,exchange:"BYBIT",mode,reason:"BYBIT_CREDENTIALS_MISSING",credentialsPresent:false,credentialSource:creds.source},503);
  try{
    ...
    return json({ok:true,readOnly:true,authenticated:true,exchange:"BYBIT",mode,credentialSource:creds.source,account:{totalEquity:Number(acct.totalEquity||coin.equity||0),walletBalance:Number(acct.totalWalletBalance||coin.walletBalance||0),availableBalance:Number(acct.totalAvailableBalance||coin.availableToWithdraw||0)},positions:{openCount:pos.length,items:pos.slice(0,20).map(x=>({symbol:x.symbol,side:x.side,size:Number(x.size),avgPrice:Number(x.avgPrice),markPrice:Number(x.markPrice),unrealisedPnl:Number(x.unrealisedPnl),leverage:Number(x.leverage)}))},openOrdersCount:open.length,serverTime:time?.time||null,checkedAt:new Date().toISOString(),guarantees:["NO_ORDER_SUBMISSION","NO_LEVERAGE_CHANGE","NO_CANCEL"]});
  }catch(e){const b=e?.bybit||{};return json({ok:false,readOnly:true,authenticated:false,exchange:"BYBIT",mode,credentialSource:creds.source,reason:"BYBIT_READONLY_HEALTH_FAILED",bybit:{path:b.path||null,httpStatus:b.httpStatus??null,retCode:b.retCode??null,retMsg:b.retMsg||String(e?.message||e).slice(0,300)},checkedAt:new Date().toISOString()},502);}
}
