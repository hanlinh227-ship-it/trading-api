...
export async function handleBybitReadonlyHealth(req,env){
  ...
  try{
    // Health must verify the actual authenticated LIVE path. Do not couple readiness to
    // Bybit public REST from the Cloudflare edge because that route can be region-blocked
    // even while the VPS private transport is healthy and trading normally.
    ...
    return json({ok:true,readOnly:true,authenticated:true,exchange:"BYBIT",mode,runtimeRevision,privateTransport,credentialSource:creds.source,execution:{liveAck,scheduled,ready},account:{totalEquity:Number(acct.totalEquity||coin.equity||0),walletBalance:Number(acct.totalWalletBalance||coin.walletBalance||0),availableBalance:Number(acct.totalAvailableBalance||coin.availableToWithdraw||0)},positions:{openCount:pos.length,items:pos.slice(0,20).map(x=>({symbol:x.symbol,side:x.side,size:Number(x.size),avgPrice:Number(x.avgPrice),markPrice:Number(x.markPrice),unrealisedPnl:Number(x.unrealisedPnl),leverage:Number(x.leverage)}))},openOrdersCount:open.length,healthTransportVerified:privateTransport,serverTime:null,checkedAt:new Date().toISOString(),guarantees:["THIS_HEALTH_REQUEST_DOES_NOT_SUBMIT_ORDERS","THIS_HEALTH_REQUEST_DOES_NOT_CHANGE_LEVERAGE","THIS_HEALTH_REQUEST_DOES_NOT_CANCEL","HEALTH_USES_AUTHENTICATED_VPS_PRIVATE_TRANSPORT_ONLY"]});
