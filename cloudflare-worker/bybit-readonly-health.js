...
export async function handleBybitReadonlyHealth(req,env){
  ...
  if(!(creds.apiKey&&creds.apiSecret))return json({ok:false,readOnly:true,authenticated:false,exchange:"BYBIT",mode,runtimeRevision,execution:{liveAck,scheduled,ready:false},reason:"BYBIT_CREDENTIALS_MISSING",credentialsPresent:false,credentialSource:creds.source},503);
  try{
    ...
    return json({ok:true,readOnly:true,authenticated:true,exchange:"BYBIT",mode,runtimeRevision,credentialSource:creds.source,execution:{liveAck,scheduled,ready},account:{totalEquity:Number(acct.totalEquity||coin.equity||0),walletBalance:Number(acct.totalWalletBalance||coin.walletBalance||0),availableBalance:Number(acct.totalAvailableBalance||coin.availableToWithdraw||0)},positions:{openCount:pos.length,items:pos.slice(0,20).map(x=>({symbol:x.symbol,side:x.side,size:Number(x.size),avgPrice:Number(x.avgPrice),markPrice:Number(x.markPrice),unrealisedPnl:Number(x.unrealisedPnl),leverage:Number(x.leverage)}))},openOrdersCount:open.length,serverTime:time?.time||null,checkedAt:new Date().toISOString(),guarantees:["THIS_HEALTH_REQUEST_DOES_NOT_SUBMIT_ORDERS","THIS_HEALTH_REQUEST_DOES_NOT_CHANGE_LEVERAGE","THIS_HEALTH_REQUEST_DOES_NOT_CANCEL"]});
  }catch(e){const b=e?.bybit||{};return json({ok:false,readOnly:true,authenticated:false,exchange:"BYBIT",mode,runtimeRevision,execution:{liveAck,scheduled,ready:false},credentialSource:creds.source,reason:"BYBIT_READONLY_HEALTH_FAILED",bybit:{path:b.path||null,httpStatus:b.httpStatus??null,retCode:b.retCode??null,retMsg:b.retMsg||String(e?.message||e).slice(0,300)},checkedAt:new Date().toISOString()},502);}
}
