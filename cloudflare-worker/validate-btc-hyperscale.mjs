...
import {BYBIT_TRADE_UNIVERSE,BYBIT_PORTFOLIO_POLICY,coinProfileForSymbol,isCoreTradeSymbol,maxConcurrentForEquity} from './bybit-coin-profiles.js';
...
assert.equal(BYBIT_AUTO_VERSION,'BYBIT-MULTI-STATEFLOW-4.8.0');
assert.equal(BYBIT_RUNTIME_CONTRACT.version,'BYBIT_MULTI_ASSET_RUNTIME_V26_CAPITAL_INTELLIGENCE_FAST_SCALE');
...
for(const k of ['infiniteLeverage','realizedProfitGuarantee'])assert.equal(BYBIT_RUNTIME_CONTRACT[k],false);
for(const k of ['exchangeMaxLeverageCap','dynamicBybitScalpUniverse','cryptoOnlyDynamicUniverse','allActiveCryptoEligible','executionUnsafeNoNewRisk','perSymbolRegimeSideCoherence','crossMarketDirectionBreadthGuard','strictContrarianException','realizedPnlPerformanceGovernor','perSymbolExpectancyGate','lossStreakRequalification','performanceAdaptiveRisk','smallProfitEarlyHarvest','capitalPreservationRecoveryMode','continuousTimeCapitalScale','capitalIntelligenceV4','separateCapitalState','instantDepositRecognition','instantWithdrawalRiskReduction','paginatedTransactionReconciliation','capitalHighWaterDoubleCountFixed','fastScaleControlled','adaptiveLeverageExpanded','freshWsRequiredForNewRisk'])assert.equal(BYBIT_RUNTIME_CONTRACT[k],true,`RUNTIME ${k}`);
...
assert.ok(maxConcurrentForEquity(60)>maxConcurrentForEquity(50));assert.ok(maxConcurrentForEquity(500)>maxConcurrentForEquity(100));assert.equal(BYBIT_PORTFOLIO_POLICY.forcedOpportunityReplacement,false);assert.equal(BYBIT_PORTFOLIO_POLICY.protectedRiskSlotReuse,true);assert.ok(BYBIT_PORTFOLIO_POLICY.deepScanCount>=12);
...
for(const x of ['4.8.0-CAPITAL-INTELLIGENCE-FAST-SCALE','positiveLockReady','exchangeMaxLeverage','lastContinuousCapitalUsd','performanceRiskMult','reduceOnly:true'])assert.ok(engine.includes(x),`ENGINE ${x}`);
for(const x of ['continuousCapacityCapitalUsd','capitalIntelligence','CROSS_MARKET_DIRECTION_CONFLICT','buildBybitPerformanceGovernor','performanceRiskMult'])assert.ok(controller.includes(x),`CONTROLLER ${x}`);
...
for(const x of ['BYBIT_MULTI_ASSET_RUNTIME_V26_CAPITAL_INTELLIGENCE_FAST_SCALE','capitalIntelligenceV4:true','instantDepositRecognition:true','capitalHighWaterDoubleCountFixed:true','fastScaleControlled:true'])assert.ok(runtime.includes(x),`RUNTIME ${x}`);
...
console.log('BYBIT_MULTI_ASSET_VALIDATION=PASS');
console.log(JSON.stringify({version:BYBIT_AUTO_VERSION,contract:BYBIT_RUNTIME_CONTRACT.version,coreSymbols:BYBIT_TRADE_UNIVERSE.length,capitalIntelligenceV4:true,instantDepositRecognition:true,fastScaleControlled:true,baseRiskPct:cfg.risk.baseEntryRiskPct,strongRiskPct:cfg.risk.strongEntryRiskPct,aPlusRiskPct:cfg.risk.aPlusEntryRiskPct,maxActiveRiskPct:cfg.risk.maxActiveRiskPct,maxConfiguredLeverage:cfg.leverage.max,freshWsRequired:true,martingale:false,addToLoser:false,monitorReadOnly:true},null,2));
