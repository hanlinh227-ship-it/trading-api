...
const engine=read('bybit-symbol-engine.js'),aiLegion=read('bybit-ai-legion.js'),cloudStream=read('bybit-market-stream.js'),microClient=read('bybit-btc-microstructure-client.js'),control=read('bybit-control-plane.js'),client=read('bybit-v5-client.js'),dynamic=read('bybit-dynamic-universe.js'),runtime=read('bybit-runtime-contract.js'),balance=read('bybit-btc-balance-reconciler.js'),monitor=read('bybit-android-monitor.js'),capital=read('bybit-capital-state.js'),sync=read('bybit-capital-sync-handler.js'),bridge=read('../bybit-live-bridge/bybit_live_bridge.py');
for(const x of ['BYBIT_EXECUTION_SYMBOL','BYBIT_NON_BTC_EXECUTION_ERROR','positiveLockReady','exchangeMaxLeverage','reduceOnly:true','nearerTarget','scalpCapTarget','floorRelaxedForFastScalp','resolveBybitAiLegionDecision','BTC_DEMO_ENTRY_EXECUTED'])assert.ok(engine.includes(x),`ENGINE ${x}`);for(const x of ['structure_regime_agent','flow_liquidity_agent','derivatives_risk_agent','independent_checker','ADVISORY_EVIDENCE_ONLY','noMajorityVote:true'])assert.ok(aiLegion.includes(x),`AI_LEGION ${x}`);
...
for(const x of ['BYBIT_BTC_RUNTIME_2_1','researchOnlyDynamicUniverse:true','researchProductionExecutionAuthority:false','aiLegionIntegrated:true','demoExecutionSupported:true','aiLegionMayPlaceOrders:false','vpsRequired:false','cloudNativeMarketStream:true','directBybitRest:true'])assert.ok(runtime.includes(x),`RUNTIME ${x}`);
