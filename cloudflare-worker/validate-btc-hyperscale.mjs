...
const engine=read('bybit-symbol-engine.js'),aiLegion=read('bybit-ai-legion.js'),control=read('bybit-control-plane.js'),client=read('bybit-v5-client.js'),dynamic=read('bybit-dynamic-universe.js'),runtime=read('bybit-runtime-contract.js'),balance=read('bybit-btc-balance-reconciler.js'),monitor=read('bybit-android-monitor.js'),capital=read('bybit-capital-state.js'),sync=read('bybit-capital-sync-handler.js'),bridge=read('../bybit-live-bridge/bybit_live_bridge.py');
...
for(const x of ['BYBIT_BTC_RUNTIME_2_1','researchOnlyDynamicUniverse:true','researchProductionExecutionAuthority:false','aiLegionIntegrated:true','demoExecutionSupported:true','aiLegionMayPlaceOrders:false'])assert.ok(runtime.includes(x),`RUNTIME ${x}`);
