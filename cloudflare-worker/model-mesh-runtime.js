...
export async function buildModelMeshPlan(input,{skillSnapshot,modelSnapshot,env={},fetchImpl=fetch}={}){
  void fetchImpl;
  ...
  const liveModels=profile==='FAST'||dataClass==='SECRET'?[]:await resolveLiveModels(modelSnapshot,env);
  const workers=profile==='FAST'||dataClass==='SECRET'?[]:selectModelWorkers({profile,domain:capsule.domain||'core',dataClass,models:liveModels});
