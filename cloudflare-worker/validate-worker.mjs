...
const required=['index.js','bybit-runtime-contract.js','bybit-auto-config.js','bybit-auto-controller.js','bybit-auto-hub.js','bybit-control-plane.js','bybit-ai-legion.js','bybit-market-stream.js','bybit-readonly-health.js','bybit-v5-client.js','bybit-btc-balance-reconciler.js','bybit-btc-microstructure-client.js','bybit-btc-market-state.js','bybit-btc-strategy.js','bybit-btc-risk-engine.js','bybit-btc-engine.js','providers/bybit-signed-client.js','providers/telegram-client.js','universal-entry-active.js','universal-entry-handler.js','universal-entry-contract.js','universal-auth.js','universal-state.js','universal-telemetry.js','memory-candidate.js'];
for(const f of required)if(!fs.existsSync(path.join(root,f)))errors.push(`MISSING ${f}`);

const indexSource=fs.existsSync(path.join(root,'index.js'))?fs.readFileSync(path.join(root,'index.js'),'utf8'):'';
for(const requiredWiring of [
  "import {handleUniversalEntry} from './universal-entry-active.js'",
...
])if(!indexSource.includes(requiredWiring))errors.push(`UNIVERSAL_FABRIC_WIRING_MISSING ${requiredWiring}`);
const universalCall=indexSource.indexOf('handleUniversalEntry(req,env,ctx)');
const memoryCall=indexSource.indexOf('handleMemoryCandidate(req,env,ctx)');
const brainCall=indexSource.indexOf('handleSkillGateway(req,env)');
if(universalCall<0||memoryCall<0||brainCall<0||!(universalCall<memoryCall&&memoryCall<brainCall))errors.push('UNIVERSAL_FABRIC_HANDLER_ORDER_INVALID');
