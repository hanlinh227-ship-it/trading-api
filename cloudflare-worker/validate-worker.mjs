...
walk(root);
const required=['index.js','bybit-auto-hub.js','meme-auto-design.js','meme-paper-engine.js','bybit-control-plane.js','bybit-readonly-health.js','bybit-auto-v1.js','bybit-auto-controller.js','bybit-scalp-engine.js','bybit-adaptive-edge.js','bybit-risk-guard.js','bybit-position-manager.js','bybit-ai-scalp-gate.js','bybit-learning-engine.js','multi-ai-control-plane.js','providers/telegram-client.js'];
...
for(const needle of ['adaptive:{','minScore:68','maxScore:85','correlationSoft:0.80','correlationHard:0.90','autoPromote:false'])if(!config.includes(needle))errors.push(`BYBIT adaptive config invariant missing ${needle}`);
for(const needle of ['SCALED_TRADE_BAND_ALLOCATOR','capitalQty','slotMarginPct','capitalLimited','marginUtilizationPct','ladderMaxLossUsd','ladderMinRewardUsd','ladderMaxRewardUsd','STRUCTURE_REWARD_BELOW_LADDER_MIN','adaptiveThreshold','assessPortfolioCorrelation','loadAdaptiveLearning','rankingScore'])if(!engine.includes(needle))errors.push(`BYBIT sizing/scanner invariant missing ${needle}`);
for(const needle of ['REGIMES','classifyRegime','learningConfidence','bounded:[68,85]','CORRELATION_HARD_CAP','CORRELATION_CLUSTER_CAP','selectExitProfile','loadAdaptiveLearning'])if(!adaptive.includes(needle))errors.push(`BYBIT Adaptive Edge invariant missing ${needle}`);
for(const needle of ['bySymbolStrategyRegime','avgNetR','netExpectancyAfterCosts:true','minSampleGuard:true','autoPromote:false','BOUNDED_ADAPTIVE_LEARNING'])if(!learning.includes(needle))errors.push(`BYBIT learning invariant missing ${needle}`);
for(const needle of ['SINGLE_TRADE_RISK_CAP','TOTAL_OPEN_RISK_CAP','computeOpenInitialMarginUsd','PORTFOLIO_MARGIN_HEADROOM','OPEN_PLAN_INITIAL_MARGIN_AWARE','dailyTargetEnabled:false','continuousTrading:true'])if(!risk.includes(needle))errors.push(`BYBIT risk guard invariant missing ${needle}`);
for(const needle of ['smartCutEnabled:true','smartCutMinAgeSec:180','smartCutScore:7','smartCutConfirmations:2'])if(!config.includes(needle))errors.push(`BYBIT Smart CUT config invariant missing ${needle}`);
for(const needle of ['smartCutAssessment','dynamicCutThresholdR','STRUCTURE_BREAK','SMART_CUT_CONFIRMING','SMART_CUT_CONFIRMED_INVALIDATION','reduceOnly:true'])if(!manager.includes(needle))errors.push(`BYBIT Smart CUT manager invariant missing ${needle}`);
