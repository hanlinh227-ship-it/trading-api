...
walk(root);
const required=['index.js','bybit-auto-hub.js','bybit-control-plane.js','bybit-readonly-health.js','bybit-auto-v1.js','bybit-auto-controller.js','bybit-scalp-engine.js','bybit-adaptive-edge.js','bybit-risk-guard.js','bybit-position-manager.js','bybit-ai-scalp-gate.js','bybit-learning-engine.js','multi-ai-control-plane.js','providers/telegram-client.js'];
...
for(const needle of ['BYBIT-AUTO-1.4.0','CONTINUOUS_CAPITAL_ALLOCATOR','baseRiskUsd:1.5','maxRiskPctOfEquity:4','maxTotalOpenRiskPct:10','maxMarginPerPositionPct:20','minFreeReservePct:30','feeBufferPct:5','maxPortfolioMarginPct:65'])if(!config.includes(needle))errors.push(`CAPITAL config invariant missing ${needle}`);
...
for(const needle of ['Adaptive Edge ON','regime + coin edge + correlation','auto-promote OFF'])if(!hub.includes(needle))errors.push(`AUTO HUB adaptive telemetry missing ${needle}`);
...
console.log('Worker AUTO preflight PASS: Bybit Auto 1.4.0 + Adaptive Edge regime/per-coin/correlation + bounded learning + continuous capital allocation + Smart CUT + 3AI + hard protection locked.');
