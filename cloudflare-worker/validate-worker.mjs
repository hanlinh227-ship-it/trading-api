...
walk(root);
const required=['index.js','hub-v11.js','v11/native-runtime.js','v11/manual-market-hunter.js','v11/entry-plan.js','v11/market-policies.js','v11/store.js','engine-v77168.js','bybit-control-plane.js','bybit-readonly-health.js','bybit-auto-v1.js','bybit-scalp-engine.js','bybit-risk-guard.js','bybit-position-manager.js','bybit-ai-scalp-gate.js','bybit-learning-engine.js','multi-ai-control-plane.js','providers/telegram-client.js'];
for(const f of required)if(!fs.existsSync(path.join(root,f)))errors.push(`REQUIRED missing ${f}`);
const index=fs.readFileSync(path.join(root,'index.js'),'utf8'),hub=fs.readFileSync(path.join(root,'hub-v11.js'),'utf8'),hunter=fs.readFileSync(path.join(root,'v11/manual-market-hunter.js'),'utf8'),multi=fs.readFileSync(path.join(root,'multi-ai-control-plane.js'),'utf8'),auto=fs.readFileSync(path.join(root,'bybit-auto-v1.js'),'utf8');
...
if(!index.includes('runBybitAutoV1'))errors.push('BYBIT Auto V1 scheduler must remain wired');
...
console.log('Worker V11 preflight PASS: Signal V11 + private 5AI + Bybit hard risk/protection/position management locked.');
