...
if(!native.includes('PROVIDER_FAILURE'))errors.push('V11 native runtime must record PROVIDER_FAILURE');
if(!store.includes('getV11ProviderFailures'))errors.push('V11 store must expose provider failure query');
if(!hub.includes('providerFailureRows'))errors.push('V11 hub must separate provider failures from WATCH');
