export function dailyTargetPolicy(env,state={},equityUsd=0){
  ...
  const modeRaw=String(env.BINANCE_DAILY_TARGET_MODE||"FLEXIBLE").toUpperCase();
  const mode=modeRaw==="HARD_ATTEMPT"?"HARD_ATTEMPT":"FLEXIBLE";
  ...
  return {
    mode,targetUsd,realizedUsd:realized,remainingUsd:remaining,reached,
    progressPct:pct*100,targetPctOfEquity:targetPct,
    keepScanningUntilTarget:mode==="HARD_ATTEMPT"&&!reached,
    allowQualityRelaxation:false,
    forceTrade:false,
    forceRiskIncrease:false,
    stopReason:reached?"DAILY_TARGET_REACHED":null
  };
}
