...
function minimumQualityRR(intel){const mode=profileMode(intel),type=intel?.type||"unknown";let x=mode==="MEAN_REVERSION"?1.10:mode==="TREND"?1.30:mode==="RELATIVE"?1.25:1.30;if(type==="future")x=Math.max(x,1.50);else if(type==="metal")x=Math.max(x,mode==="MEAN_REVERSION"?1.25:1.45);else if(type==="crypto")x=Math.max(x,mode==="MEAN_REVERSION"?1.15:1.35);else if(type==="forex")x=Math.max(x,mode==="MEAN_REVERSION"?1.15:1.30);if(Number(intel?.methodFit)>=90)x-=.05;return Math.max(1.1,Number(x.toFixed(2)));}
