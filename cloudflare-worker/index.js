...
async function deepAnalyze(symbol,env,candles=null,reference=null,source=null,context={}){
  ...
  if(!locPolicy.pass){const planned=indicativePlan(side,M5,M15,H1,H4,D1,locPolicy,prior)||rescuePlan(side,M15.close,M5,M15,H1,H4,D1,prior,intel,locPolicy,{})||conditionalPlanFromRoute(intel,M5,M15,H1,H4,D1,prior);if(planned)return planSignal(s,type,planned.side||side,"M15_LOCATION_REQUIRED",planned,{...base,score:Math.max(base.score,44),entryPolicy:{profile:profileMode(intel),location:locPolicy},canonical:{...base.canonical,m15Location:loc}});return watch(s,type,side,"M15_LOCATION_REQUIRED",{...base,entryPolicy:{profile:profileMode(intel),location:locPolicy},canonical:{...base.canonical,m15Location:loc}});}
  ...
  if(!trigPolicy.pass){const planned=indicativePlan(side,M5,M15,H1,H4,D1,locPolicy,prior)||rescuePlan(side,M5.close,M5,M15,H1,H4,D1,prior,intel,locPolicy,trigPolicy)||conditionalPlanFromRoute(intel,M5,M15,H1,H4,D1,prior);if(planned)return planSignal(s,type,planned.side||side,"M5_MSS_DISPLACEMENT_RETEST_REQUIRED",planned,{...base,score:Math.max(base.score,48),entryPolicy:{profile:profileMode(intel),location:locPolicy,trigger:trigPolicy},canonical:{...base.canonical,m15Location:loc,m5Trigger:trig}});return watch(s,type,side,"M5_MSS_DISPLACEMENT_RETEST_REQUIRED",{...base,entryPolicy:{profile:profileMode(intel),location:locPolicy,trigger:trigPolicy},canonical:{...base.canonical,m15Location:loc,m5Trigger:trig}});}
