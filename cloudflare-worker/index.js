...
export default {
  async fetch(req,env,ctx){
    ...
    if(["/mcp","/mcp/health","/gpt-5ai/action","/internal/multi-ai/review"].includes(url.pathname))return json({ok:false,error:"RETIRED_CONFLICTING_AI_ROUTE",replacement:"/bybit/ai/latest-review",authority:"BYBIT_AUTO_CANONICAL_3AI_GATE"},410);
    ...
    if(url.pathname==="/status")return json({
      ...
      bybit:{version:BYBIT_AUTO_VERSION,autoEnabled:envBool(env.BYBIT_AUTO_ENABLED),live:envBool(env.BYBIT_AUTO_LIVE),executionAuthority:true,readonlyHealth:BYBIT_HEALTH_ROUTE,runtimeContract:"/runtime/contract",threeAiAuthority:"BYBIT_AUTO_CANONICAL_3AI_GATE",latestAiReview:"/bybit/ai/latest-review"},
