...
function actionableRank(a){let x=Number(a?.score)||0;if(a?.status==="MARKET")x+=1000;else if(a?.status==="LIMIT")x+=900;else if(a?.status==="MARKET_PLAN")x+=700;else if(a?.status==="LIMIT_PLAN")x+=650;else if(a?.planned?.mode==="MARKET")x+=500;else if(a?.planned?.mode==="LIMIT"||a?.planned?.mode==="INDICATIVE_LIMIT")x+=420;else if(a?.setupReady)x+=150;const p=a?.planned||a,rr=Number(p?.targetRR),g=p?.geometry||p?.marketGeometry||{};if(Number.isFinite(rr))x+=Math.min(80,rr*22);if(Number.isFinite(g.distanceATR))x+=Math.max(0,24-Math.abs(g.distanceATR-.30)*30);if(Number.isFinite(p?.confluence))x+=Math.min(20,Number(p.confluence)*5);if(a?.contextOnly)x-=600;return x;}
...
function allLivePositions(books){const out=[];for(const g of ["crypto","forex","metal","index"]){const b=books?.[g];if(!b)continue;for(const p of b.marketActive||[])out.push({group:g,...p});for(const p of b.limitActive||[])out.push({group:g,...p});}return out;}
...
function booksSummary(books){const L=["📚 LIVE ORDERS — LƯU BỀN QUA UPDATE BOT"];for(const g of ["forex","crypto","metal","index"]){const b=books[g];L.push("",groupTitle(g)+" • MARKET "+b.marketActive.length+" • LIMIT ACTIVE "+b.limitActive.length+" • LIMIT CHỜ "+b.limitPending.length);for(const p of b.marketActive)L.push("• "+livePositionLine(p));for(const p of b.limitActive)L.push("• "+livePositionLine(p));for(const p of b.limitPending)L.push("• "+livePositionLine(p));}L.push("","WATCH/SETUP không nằm trong Books; Hub quét setup mới nhưng LIVE positions luôn được hiển thị riêng.");return L.join("\n");}
