...
import {buildEntryIntelligenceShadow,recordEntryIntelligenceShadow,getEntryIntelligenceShadow,entryIntelligenceRankAdjustment,entryIntelligencePromotionAllowed} from "./providers/entry-intelligence.js";

const CONFIG = {
  version: "V77.16.20",
  service: "Trading V77.16.20 Signal Lifecycle Guard R7",
...
async function tdIndexQuoteVisibilityFallback(symbol,env){const s=norm(symbol),ps=INDEX_PROVIDER[s]||s,p=await tdFetch("quote",{symbol:ps,type:"Index"},env),price=num(p?.close)??num(p?.price)??num(p?.previous_close);if(!(price>0)||!validIndexPrice(s,price))throw new Error(`Twelve Data index quote fallback invalid ${s}/${ps}: ${price}`);const ts=parseTs(p?.last_quote_at??p?.timestamp??p?.datetime),age=ts==null?null:Math.max(0,nowSec()-ts);return {source:"Twelve Data Index Quote Fallback",requestedSymbol:s,providerSymbol:ps,price,providerTimestamp:ts,quoteAgeSec:age,fresh:false,bid:null,ask:null,executionVerified:false,analysisOnly:true,fallback:true,fallbackInterval:"quote",assetType:"Index"};}
const YAHOO_INDEX_CASH_PROVIDER={NAS100:"^NDX",US30:"^DJI",US500:"^GSPC",DEX:"^GDAXI",JP225:"^N225"};
...
async function tdQuote(symbol,env){
  if(marketType(symbol)==="index"){const s=norm(symbol);if(env.MASSIVE_API_KEY){try{return await massiveIndexQuote(s,env);}catch{}}if(env.TWELVE_DATA_API_KEY){try{return await tdIndexQuote(s,env);}catch{}try{return await tdIndexQuoteVisibilityFallback(s,env);}catch{}}try{return await yahooIndexCashVisibilityQuote(s);}catch{}throw new Error("No native index quote available");}
...
function actionableRank(a){let x=Number(a?.score)||0;if(a?.status==="MARKET")x+=1000;else if(a?.status==="LIMIT")x+=900;else if(a?.status==="MARKET_PLAN")x+=700;else if(a?.status==="LIMIT_PLAN")x+=650;else if(a?.planned?.mode==="MARKET")x+=500;else if(a?.planned?.mode==="LIMIT"||a?.planned?.mode==="INDICATIVE_LIMIT")x+=420;else if(a?.setupReady)x+=150;const p=a?.planned||a,rr=Number(p?.targetRR),g=p?.geometry||p?.marketGeometry||{};if(Number.isFinite(rr))x+=Math.min(80,rr*22);if(Number.isFinite(g.distanceATR))x+=Math.max(0,24-Math.abs(g.distanceATR-.30)*30);if(Number.isFinite(p?.confluence))x+=Math.min(20,Number(p.confluence)*5);if(a?.contextOnly)x-=600;x+=entryIntelligenceRankAdjustment(a);return x;}
...
async function deepAnalyze(symbol,env,candles=null,reference=null,source=null,context={}){
  ...
  const base={source,method:intel,knowledge:knowledge?{canonicalSymbol:knowledge.canonicalSymbol,allowedModes:knowledge.allowedModes,riskATR:knowledge.riskATR,newsProfile:knowledge.newsProfile,calibration:knowledge.calibration}:null,context,timeframes:{M5:{atr14:M5.atr14,close:M5.close},M15:{atr14:M15.atr14,close:M15.close},H1:{atr14:H1.atr14,close:H1.close},H4:{atr14:H4.atr14,close:H4.close},D1:{atr14:D1.atr14,close:D1.close}},canonical:{v73Prior:prior},score:setupScore({methodFit:intel.methodFit,htf,htfVotes:Math.max(votes.bull,votes.bear),contextScore:context.score??5})};
...
async function liveBookQuote(g,p,env){try{if(g==="crypto")return await cryptoExecutionQuote(p.symbol);if(g==="forex"||g==="metal")return await tdQuote(p.symbol,env);if(g==="index")return await currentQuote(p.symbol,env);}catch{}return null;}
...
function baseKeyboard(){return {inline_keyboard:[[{text:"📡 SIGNAL",callback_data:"signal"}],[{text:"🔎 SYMBOL",callback_data:"symbols"}],[{text:"📊 STATUS",callback_data:"status"},{text:"📚 LIVE ORDERS",callback_data:"live"}]]};}
...
function singleAnalysisText(a){
  ...
  const eiShadow=buildEntryIntelligenceShadow(a);if(eiShadow)L.push(`Quality: ${eiShadow.quality.grade} (${eiShadow.quality.score}/100)${eiShadow.promotion.allowed===false?" • ⚠️ "+eiShadow.promotion.reasons.join(","):""}`);
  L.push(`WHY NOW: ${whyNowVi(a)}`);
...
async function handleTelegram(req,env){
  ...
  else if(cb==="hub"||text==="/hub")await sendText(env,"🏠 Global scan đã tắt. Chọn từng thị trường trong Hub mới.",chatId,baseKeyboard());
  else if(cb==="symbols")await sendText(env,"🔎 Symbol chung đã tắt. Chọn đúng thị trường:",chatId,symbolMarketKeyboard());
