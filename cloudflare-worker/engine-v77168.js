...
async function notifySignalPlans(group,run,env){
  ...
  for(const a of plans){
    ...
    const rr=Number(a.planned?.targetRR||0).toFixed(2),txt=["📡 SIGNAL PLAN • "+groupTitle(group),"━━━━━━━━━━━━━━",(mode==="MARKET"?"🟢":"🟡")+" "+a.symbol+" • "+sideText(a.side)+" • "+mode,"🎯 Entry "+fmtPx(a.planned.entry),"🛡 SL "+fmtPx(a.planned.sl),"💰 TP1 "+fmtPx(a.planned.tp1)+" • TP2 "+fmtPx(a.planned.tp2||a.planned.tp1)+" • RR "+rr,"⭐ Score "+(Number(a.score)||0)+"/100"+(a.method?.profile?" • "+a.method.profile:""),"ℹ️ Plan kỹ thuật • chờ execution authority thật"].join("\n");
