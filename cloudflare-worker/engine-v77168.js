...
const CONFIG = {
  version: "V77.16.11",
  service: "Trading V77.16.11 Forex Opportunity Routing R3",
...
function mirrorFutureAnalysis(a,target){if(!a)return null;const k=symbolKnowledge(target),out={...a,symbol:target,market:"future",source:(a.source||"Massive Futures")+" • leader mirror",knowledge:k?{canonicalSymbol:k.canonicalSymbol,allowedModes:k.allowedModes,riskATR:k.riskATR,calibration:k.calibration}:a.knowledge,executionLeader:a.symbol,basisReference:true};const fs=futureSizing(target,out.planned||out);if(fs)out.microSizing=fs;if(fs&&!fs.tradable&&["MARKET","LIMIT","MARKET_PLAN","LIMIT_PLAN"].includes(out.status)){out.rejectedPlan=out.planned||{entry:out.entry,sl:out.sl,tp1:out.tp1,tp2:out.tp2,targetRR:out.targetRR};out.planned=null;out.status="WATCH";out.action="WATCH";out.reason="FUTURE_RISK_LIMIT_REQUIRED";out.canonicalStage=out.reason;out.executionReady=false;}return out;}
...
async function broadScan(group,env){
  ...
  let h1Map=new Map();try{h1Map=await tdBatchCandles(symbols,"1h",env,60);}catch(e){return {requested:symbols.length,rows,errors:symbols.map(symbol=>({symbol,reason:"H1_BATCH_UNAVAILABLE",error:e?.message||String(e)})),h1Map};}const fx=group==="forex"?forexStrengthMap(h1Map):{};let metalMoves={};if(group==="metal")for(const sym of symbols)metalMoves[sym]=changeFromCandles(h1Map.get(sym)||[]);if(group==="forex")await saveMarketContext("forex",{strength:fx},env);else if(group==="metal")await saveMarketContext("metal",{moves:metalMoves},env);
  for(const sym of symbols){const T=tf(h1Map.get(sym)||[]);if(!T.ready){errors.push({symbol:sym,reason:"H1_UNAVAILABLE"});continue;}let context={score:5};if(group==="forex"){const base=sym.slice(0,3),quote=sym.slice(3),d=(fx[base]||0)-(fx[quote]||0),sess=forexSessionBias(sym);context={baseStrength:fx[base]||0,quoteStrength:fx[quote]||0,strengthDiff:d,session:sess.label,sessionScore:sess.score,score:Math.min(10,4.6+Math.abs(d)*4.4+sess.score*.7)};}else{const other=sym==="XAUUSD"?"XAGUSD":"XAUUSD",rel=(metalMoves[sym]||0)-(metalMoves[other]||0);context={relativeStrength:rel,benchmark:other,score:Math.min(10,5+Math.abs(rel)*3)};}const raw=Math.abs((T.close-(T.ema20??T.close))/(T.atr14||1)),prior=v73Prior(sym,group),sweet=Math.max(0,1-Math.abs(raw-.35)/.85),sessionBoost=group==="forex"?Number(context.sessionScore||.35):sessionFit(prior),directionBoost=group==="forex"?Math.min(1.4,Math.abs(Number(context.strengthDiff||0))*3.2):0,discoveryScore=sweet*2.2+Number(context.score||5)*.48+sessionBoost*.75+directionBoost*.35+Math.min(raw,1.2)*.08;rows.push({symbol:sym,quote:{source:"Twelve Data H1",price:T.close,fresh:true},strength:discoveryScore,rawStrength:raw,context});}rows.sort((a,b)=>b.strength-a.strength);return {requested:symbols.length,rows,errors,h1Map};
}
...
async function runSymbol(symbol,env){
  ...
  if(type==="future"){const recent=await recentFutureScanAnalysis(s,env);if(recent)return recent;try{return await runFutureSymbol(s,env);}catch(e){return {ok:false,status:"DATA_BLOCK",symbol:s,market:"future",reason:"MASSIVE_RATE_OR_DATA_UNAVAILABLE",error:e?.message||String(e),engine:CONFIG.version};}}
  ...
  if(type==="forex"){let cached=await loadMarketContext("forex",env),fx=cached?.strength||null,h1;if(fx){h1=await tdBatchCandles([s],"1h",env,60);}else{h1=await tdBatchCandles(FOREX,"1h",env,60);fx=forexStrengthMap(h1);await saveMarketContext("forex",{strength:fx},env);}const base=s.slice(0,3),quote=s.slice(3),d=(fx[base]||0)-(fx[quote]||0),context={baseStrength:fx[base]||0,quoteStrength:fx[quote]||0,strengthDiff:d,score:Math.min(10,5+Math.abs(d)*4),contextCached:!!cached},extra=await Promise.all(["5min","15min","4h","1day"].map(i=>tdBatchCandles([s],i,env,CONFIG.candleOutputSize))),candles=[extra[0].get(s)||[],extra[1].get(s)||[],h1.get(s)||[],extra[2].get(s)||[],extra[3].get(s)||[]];let q=null;try{q=await tdQuote(s,env);}catch{}const a=await deepAnalyze(s,env,candles,q,"Twelve Data direct + cached 28-pair strength",context);if(q)a.analysisQuote=q;return a;}
...
async function sendGroup(group,env,chatId){await sendText(env,"⏳ Đang quét MỚI "+group.toUpperCase()+"...",chatId);const run=await runGroup(group,env,"GROUP_MANUAL"),books=await getBooksLive(env);return sendText(env,summary(group,books,run),chatId,groupKeyboard(group,run));}
