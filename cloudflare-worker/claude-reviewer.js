...
function safeHealth(h){
  ...
  return {
    ok:!!h.ok,version:h.version||null,errors:num(h.errors),warnings:num(h.warnings),signature:h.signature||null,
    issues:(h.issues||[]).filter(x=>x?.level!=="OK").slice(0,12).map(x=>({level:x.level,code:x.code,msg:x.msg})),
    account:h.account?{configured:!!h.account.configured,connected:!!h.account.connected,equity:num(h.account.equity),positions:num(h.account.positions),orders:num(h.account.orders),autoRequested:!!h.account.autoRequested,paused:!!h.account.paused,runtimeReason:h.account.runtimeReason||null}:null
  };
}
...
function normalizeReview(x){
  ...
  const findings=Array.isArray(x?.findings)?x.findings.slice(0,10).map(f=>({severity:String(f?.severity||"WARN").toUpperCase(),area:String(f?.area||"GENERAL"),issue:clampText(f?.issue,420),evidence:clampText(f?.evidence,420),recommendation:clampText(f?.recommendation,520)})):[];
  const tuning=Array.isArray(x?.tuning)?x.tuning.slice(0,8).map(v=>typeof v==="object"&&v?{priority:clampText(v.priority||"MEDIUM",40),area:clampText(v.area||"GENERAL",80),current_problem:clampText(v.current_problem||"",420),proposed_change:clampText(v.proposed_change||"",520),expected_effect:clampText(v.expected_effect||"",420),risk:clampText(v.risk||"",360)}:{priority:"MEDIUM",area:"GENERAL",proposed_change:clampText(v,520)}):[];
  return {verdict,confidence:Math.max(0,Math.min(100,num(x?.confidence))),summary:clampText(x?.summary||"",900),findings,tuning,must_fix:Array.isArray(x?.must_fix)?x.must_fix.slice(0,8).map(v=>clampText(typeof v==="string"?v:JSON.stringify(v),520)):[]};
}
...
Output ONLY valid compact JSON with keys: verdict (PASS|WARN|FAIL), confidence (0-100), summary, findings (array severity,area,issue,evidence,recommendation), tuning (array of objects priority,area,current_problem,proposed_change,expected_effect,risk), must_fix (array).`;
  const payload={version,kind,repo:REPO,github,health,runtime};
...
async function callClaude(env,{version,kind,github,health,runtime}){
  if(!env.ANTHROPIC_API_KEY)throw new Error("ANTHROPIC_API_KEY missing");
  const model=String(env.ANTHROPIC_REVIEW_MODEL||DEFAULT_MODEL),maxTokens=Math.max(500,Math.min(2200,num(env.ANTHROPIC_REVIEW_MAX_TOKENS,1400))),p=reviewerPrompt({version,kind,github,health,runtime});
...
export function formatClaudeReviewText(state){
  ...
  const r=state.review||{},icon=r.verdict==="PASS"?"✅":r.verdict==="FAIL"?"❌":"⚠️",must=(r.must_fix||[]).slice(0,3),find=(r.findings||[]).slice(0,3).map(x=>`${x.severity==="FAIL"||x.severity==="ERROR"?"❌":"⚠️"} ${x.area}: ${x.issue}`),tune=(r.tuning||[]).slice(0,2).map(x=>`💡 ${x.area||"SYSTEM"}: ${x.proposed_change||""}`);
  return [`${icon} Claude Reviewer • ${r.verdict||"WARN"} ${r.confidence||0}%`,`${state.version||"—"} • ${state.kind||"REVIEW"} • ${state.model||"—"}`,r.summary||"",...find,...tune,...(must.length?["🔧 Must-fix:",...must.map(x=>`• ${x}`)]:[]),`Usage in/out: ${num(state.usage?.input_tokens)}/${num(state.usage?.output_tokens)}`].filter(Boolean).join("\n").slice(0,3900);
}
export async function runClaudeReview(env,{version="UNKNOWN",kind="MANUAL",force=false,notify=true,health=null}={}){
  ...
  try{
    const h=safeHealth(health||await kvGet(env,HEALTH_KEY,null)),[github,runtime]=await Promise.all([githubContext(),safeRuntimeSnapshot(env)]),resp=await callClaude(env,{version,kind,github,health:h,runtime}),budget=await saveBudget(env,gate.budget,resp.usage),out={ok:true,version,kind,startedAt,finishedAt:now(),githubHead:github.head,model:resp.model,messageId:resp.id,stopReason:resp.stopReason,usage:resp.usage,budget,review:resp.review};
...
export async function runClaudeAutoReview(env,{version="UNKNOWN",health=null}={}){
  ...
  const rel=await kvGet(env,RELEASE_KEY,null);if(rel?.version!==version){const r=await runClaudeReview(env,{version,kind:"RELEASE_FINAL_SYSTEM_REVIEW",force:false,notify:true,health});if(r.ok&&!r.skipped)await kvPut(env,RELEASE_KEY,{version,at:now(),verdict:r.review?.verdict||null},2592000);return r;}
...
export async function getClaudeReviewerStatus(env){
  const [last,budget,release,dailyAudit]=await Promise.all([kvGet(env,STATE_KEY,null),kvGet(env,BUDGET_KEY,null),kvGet(env,RELEASE_KEY,null),kvGet(env,DAILY_AUDIT_KEY,null)]);
  return {configured:!!env.ANTHROPIC_API_KEY,model:String(env.ANTHROPIC_REVIEW_MODEL||DEFAULT_MODEL),dailyLimit:Math.max(1,Math.min(20,num(env.CLAUDE_REVIEW_DAILY_LIMIT,4))),cooldownMin:Math.max(5,Math.min(720,num(env.CLAUDE_REVIEW_COOLDOWN_MIN,45))),last,budget,release,dailyAudit,automation:{releaseFinalReview:true,incidentReview:true,dailySystemAuditHours:Math.max(12,Math.min(72,num(env.CLAUDE_SYSTEM_AUDIT_HOURS,24)))},permissions:{trade:false,closePosition:false,deploy:false,kvMutationExceptOwnReviewState:true,reviewOnly:true}};
}
