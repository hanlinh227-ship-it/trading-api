...
export function equityScaleState(equityUsd,cfg){
  ...
  if(!s.enabled)return {riskMult:1,marginCapPct:num(cfg?.risk?.maxPortfolioMarginPct||100),leverageBonus:0,tierEquityUsd:equity,continuous:true,lowerReferenceUsd:0,upperReferenceUsd:null,progressPct:100};
  const steps=[...(s.steps||[])].sort((a,b)=>num(a.equityUsd)-num(b.equityUsd));
  if(!steps.length)return {riskMult:1,marginCapPct:num(cfg?.risk?.maxPortfolioMarginPct||100),leverageBonus:0,tierEquityUsd:equity,continuous:true,lowerReferenceUsd:0,upperReferenceUsd:null,progressPct:100};
  const first=steps[0];
  if(equity<=num(first.equityUsd))return {riskMult:clamp(num(first.riskMult)||1,.5,num(s.maxRiskMult||1.4)),marginCapPct:clamp(num(first.marginCapPct)||num(cfg?.risk?.maxPortfolioMarginPct||100),30,num(s.maxMarginCapPct||100)),leverageBonus:Math.max(0,num(first.leverageBonus)),tierEquityUsd:equity,continuous:true,lowerReferenceUsd:num(first.equityUsd),upperReferenceUsd:steps[1]?num(steps[1].equityUsd):null,progressPct:0};
  for(let i=0;i<steps.length-1;i++){const a=steps[i],b=steps[i+1],lo=num(a.equityUsd),hi=Math.max(lo+1e-9,num(b.equityUsd));if(equity>=lo&&equity<hi){const t=(equity-lo)/(hi-lo);return {riskMult:clamp(lerp(a.riskMult,b.riskMult,t),.5,num(s.maxRiskMult||1.4)),marginCapPct:clamp(lerp(a.marginCapPct,b.marginCapPct,t),30,num(s.maxMarginCapPct||100)),leverageBonus:Math.max(0,lerp(a.leverageBonus,b.leverageBonus,t)),tierEquityUsd:equity,continuous:true,lowerReferenceUsd:lo,upperReferenceUsd:hi,progressPct:t*100};}}
  ...
  return {riskMult:clamp(lerp(last.riskMult,num(s.maxRiskMult||last.riskMult||1.4),t),.5,num(s.maxRiskMult||1.4)),marginCapPct:clamp(lerp(last.marginCapPct,num(s.maxMarginCapPct||last.marginCapPct||84),t),30,num(s.maxMarginCapPct||100)),leverageBonus:Math.max(0,num(last.leverageBonus)),tierEquityUsd:equity,continuous:true,lowerReferenceUsd:tailStart,upperReferenceUsd:t<1?tailEnd:null,progressPct:t*100};
}
...
export function btcRiskDecision({cfg,equityUsd,state={},setup,markPrice,candidateInitialMarginUsd=0,candidateActualRiskUsd=0,externalActiveRiskUsd=0,externalMarginUsd=0,performanceRiskMult=1}){
  ...
  const marginCap=capital.capitalBaseUsd*scale.marginCapPct/100,openMargin=(tranches||[]).filter(t=>String(t.status||'OPEN')==='OPEN').reduce((s,t)=>s+Math.max(0,num(t.initialMarginUsd)),0)+Math.max(0,num(externalMarginUsd)),candidateMargin=Math.max(0,num(candidateInitialMarginUsd));
...
export function closeAllTranches(state={},meta={}){const tranches=(state.tranches||[]).map(t=>String(t.status||'OPEN')==='OPEN'?{...t,status:'CLOSED',closedAt:Date.now(),...meta}:t);return {...state,tranches,previousAdaptiveProtection:state.lastAdaptiveProtection||state.previousAdaptiveProtection||null,lastAdaptiveProtection:null,aggregateStop:0,virtualTarget:0,positionPeakR:0,invalidationCount:0,hardInvalidationCount:0,profitHarvestWeakCount:0,earlyProfitHarvestWeakCount:0,profitFloorHit:false,profitFloorHitAt:null,profitFloorPeakNetUsd:0,currentPositionMarginUsd:0,currentPositionLeverage:0,openPlans:{}};}

export const BTC_RISK_ENGINE_VERSION='BYBIT_RISK_RECYCLE_V8_PORTFOLIO_FEE_AWARE_SIZING';
