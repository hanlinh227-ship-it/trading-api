...
const META_KEY='bybit:dynamic:universe:meta:v1';
...
async function loadInstrumentMeta(env,api){
  ...
  try{
    for(let page=0;page<4;page++){
      ...
      for(const x of list){
        ...
        rows.push({symbol,status:String(x.status||''),contractType:String(x.contractType||''),settleCoin:String(x.settleCoin||''),quoteCoin:String(x.quoteCoin||''),launchTime:num(x.launchTime),maxLeverage:num(x.leverageFilter?.maxLeverage),minLeverage:num(x.leverageFilter?.minLeverage),leverageStep:num(x.leverageFilter?.leverageStep)});
      }
  ...
  const out={at:now,rows,source:'BYBIT_INSTRUMENTS_INFO'};await put(env,META_KEY,out);return out;
}
...
export async function buildBybitDynamicUniverse(env,api){
  ...
  return {authority:'BYBIT_DYNAMIC_LINEAR_SCALP_UNIVERSE_V1',at:now,coreSymbols:BYBIT_TRADE_UNIVERSE,tradeSymbols:trade.map(x=>x.symbol),ranked:rows,watchNew,watchOnly,blocked,summary:{authority:'BYBIT_DYNAMIC_LINEAR_SCALP_UNIVERSE_V1',totalLinearUsdt:rows.length,tradeableNow:trade.length,watchNew:watchNew.length,watchOnly:watchOnly.length,doNotTrade:blocked.length,counts,topTrade:trade.slice(0,20).map(x=>({symbol:x.symbol,class:x.classification,score:Number(x.score.toFixed(4)),spreadBps:Number(x.spreadBps.toFixed(3)),turnover24h:x.turnover,maxLeverage:x.maxLeverage,style:x.style})),newListings:watchNew.slice(0,20).map(x=>({symbol:x.symbol,ageDays:x.ageDays===null?null:Number(x.ageDays.toFixed(2)),turnover24h:x.turnover,spreadBps:Number(x.spreadBps.toFixed(3)),reason:x.reason})),watch:watchOnly.slice(0,20).map(x=>({symbol:x.symbol,class:x.classification,reason:x.reason,turnover24h:x.turnover,spreadBps:Number(x.spreadBps.toFixed(3))})),metaFresh:!metaState.stale,metaAt:metaState.at||null}};
}

export const BYBIT_DYNAMIC_UNIVERSE_VERSION='BYBIT_DYNAMIC_LINEAR_SCALP_UNIVERSE_V1';
