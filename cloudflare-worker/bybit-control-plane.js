...
export async function handleBybitControlApi(req,env){const u=new URL(req.url);if(u.pathname==="/bybit/scan"&&req.method==="GET"){try{return json({ok:true,exchange:"BYBIT",...(await scanBybitAuto(env))});}catch(e){return json({ok:false,exchange:"BYBIT",reason:"BYBIT_SCAN_FAILED",error:String(e?.message||e)},502);}}
