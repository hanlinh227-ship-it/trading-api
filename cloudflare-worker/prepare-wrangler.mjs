...
const config={$schema:'./node_modules/wrangler/config-schema.json',name:'trading-v77-scanner',main:'index.js',compatibility_date:'2026-08-21',keep_vars:true,vars,kv_namespaces:kvNamespaces,durable_objects:{bindings:[{name:'TINYFISH_CIRCUIT',class_name:'TinyFishCircuit'},{name:'MODEL_MESH_HEALTH',class_name:'ModelMeshHealthState'}]},migrations:[{tag:'tinyfish-circuit-v1',new_sqlite_classes:['TinyFishCircuit']},{tag:'model-mesh-health-v1',new_sqlite_classes:['ModelMeshHealthState']}],vpc_services:[{binding:'AI_BRIDGE',service_id:proxy.id,remote:true}],triggers:{crons:[HEALTH_REFRESH_CRON]}};
assertHealthOnlyCrons(config.triggers.crons);
fs.writeFileSync('wrangler.jsonc',`${JSON.stringify(config,null,2)}\n`,'utf8');
console.log(`Prepared BTC-only wrangler.jsonc: TRADING_STATE=${kv.source}, BRAIN_STATE=${brainKv?.source||'TRADING_STATE_NAMESPACED_FALLBACK'}, MODEL_MESH_HEALTH=DURABLE_OBJECT_STRONG, BYBIT_VPS_PROXY=${proxy.source}, RUNTIME_REVISION=${revision}, MODEL_MESH_EXECUTION=ENABLED_AUTHENTICATED, IMAGE_RENDER_EXECUTION=ENABLED_AUTHENTICATED_PUBLIC_ONLY, FINANCIAL_RUNTIME_SWITCHES=PRESERVE_EXISTING, LIVE_ACK=PRESERVE_EXISTING, CRON=${config.triggers.crons.join(',')}_MODEL_MESH_HEALTH_ONLY`);
