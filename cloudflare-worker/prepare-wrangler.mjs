...
console.log(`Prepared BTC-only wrangler.jsonc: TRADING_STATE=${kv.source}, BRAIN_STATE=${brainKv?.source||'TRADING_STATE_NAMESPACED_FALLBACK'}, BYBIT_VPS_PROXY=${proxy.source}, RUNTIME_REVISION=${revision}, MODEL_MESH_EXECUTION=ENABLED_AUTHENTICATED, FINANCIAL_RUNTIME_SWITCHES=PRESERVE_EXISTING, LIVE_ACK=PRESERVE_EXISTING, CRON=${config.triggers.crons.join(',')}_MODEL_MESH_HEALTH_ONLY`);
