...
const loadBridgeHealth=env=>cached('bridge',350,()=>bridgeFetch(env,'/health'));
...
function botState(env,controller={},bridge={}){
  const mode=bybitExecutionMode(env),enabled=on(env.BYBIT_AUTO_ENABLED),liveAck=on(env.BYBIT_BTC_LIVE_ACK),ws=bridge.wsTelemetry||{},accountReady=num(controller.equityUsd)>0||num(controller.walletBalanceUsd)>0,ready=enabled&&mode!=='BLOCKED'&&(mode!=='LIVE'||liveAck)&&accountReady&&bridge.ok===true&&num(ws.connectedCount)>0,status=!enabled?'DISABLED':mode==='LIVE'&&ready?'LIVE_RUNNING':mode==='DEMO'&&ready?'DEMO_RUNNING':mode==='PAPER'&&ready?'PAPER_RUNNING':mode==='BLOCKED'?'MODE_CONFLICT':'DEGRADED';
...
async function openWebSocket(req,env,ctx){
  ...
  const pair=new WebSocketPair(),[client,server]=Object.values(pair);server.accept();let closed=false,timer=null,inflight=false,intervalMs=750,seq=0;
