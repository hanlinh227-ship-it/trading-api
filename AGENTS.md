...
- For any multi-tool, build, deployment, or device-operation request, emit a one-line acknowledgement immediately before slow work starts; never block the first visible response on research, CI, build, deployment, or device completion.
- For Android Agent commands, create the `[ANDROID_AGENT]` bridge issue and acknowledge immediately after issue creation; do not wait for GitHub Actions or device completion unless the user explicitly asks to check status.
- When reporting Android Agent work, separate three states precisely: `QUEUED`, `DEVICE_EXECUTED`, and `VERIFIED`; never wait for `VERIFIED` before acknowledging `QUEUED`.
...
7. Only after routing, lazy-load project authority, bounded memory, sources, provider metadata, and tools that are materially required. Resolve them through the checkpoint-declared retrieval index (`retrieval_index_path`, exact lookup first; HOT tier only for `FAST`, WARM for `STANDARD`/`DEEP`, COLD only on explicit request/rollback/audit) and stay inside the checkpoint-declared budgets (`stable_budgets_path`). Never scan the repository to answer a request.
...
## Skill registry
- Catalog rows with `alias_of` are aliases: never primary, never routed; their triggers fold into the canonical skill at compile time. One purpose has exactly one primary skill and one trigger term has exactly one owner.
...
## Engineering
Behavior changes use test-first discipline: RED -> minimum GREEN -> regression suite -> validators -> CI -> merge -> post-merge production verification. Validation has one entrypoint (`ci_validate_path`); release manifests, hashes, history and the retrieval index are generated by tools (`release_tool_path`, `retrieval_index_builder_path`), never edited by hand.
