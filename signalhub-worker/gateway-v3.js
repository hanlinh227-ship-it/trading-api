...
async function maybeCreateV31(env,market,style,setups,maxNew=1){
  if(!env?.SIGNALS_KV)return[];const current=await getV31Signals(env,market,style),active=current.filter(x=>x.status==='PENDING'||x.status==='OPEN');if(active.length>=6)return[];const lastKey=`v31:lastnew:${market}:${style}`,raw=await env.SIGNALS_KV.get(lastKey),last=raw?Date.parse(raw):0,minGap=style==='SCALP'?10*60*1000:30*60*1000;if(Number.isFinite(last)&&Date.now()-last<minGap)return[];const made=[];
  for(const setup of setups){if(made.length>=maxNew)break;if(active.some(x=>x.symbol===setup.symbol))continue;const ptr=await env.SIGNALS_KV.get(activePointer(market,style,setup.symbol));if(ptr)continue;const issuedAt=nowIso(),id=`V31-${market}-${style}-${setup.symbol}-${Date.now().toString(36)}`,s={...setup,id,issuedAt,lastCheckedAt:issuedAt,outcome:null,resultR:null,engineVersion:V3_VERSION,checkpoint:CHECKPOINT};await writeV31Signal(env,s);made.push(s);active.push(s);await env.SIGNALS_KV.put(lastKey,issuedAt,{expirationTtl:SIGNAL_TTL});}
  return made;
}
...
async function legacyScalpSignals(env){
  if(!env?.SIGNALS_KV)return[];const listing=await env.SIGNALS_KV.list({prefix:'signal:',limit:1000}),out=[];for(let i=0;i<listing.keys.length;i+=50){const raws=await Promise.all(listing.keys.slice(i,i+50).map(k=>env.SIGNALS_KV.get(k.name)));for(const raw of raws){if(!raw)continue;try{const s=JSON.parse(raw);s.market='FOREX';s.style='SCALP';out.push(s)}catch{}}}out.sort((a,b)=>Date.parse(b.issuedAt||0)-Date.parse(a.issuedAt||0));return out;
}
async function unifiedSignals(url,env){
  const market=String(url.searchParams.get('market')||'FOREX').toUpperCase()==='CRYPTO'?'CRYPTO':'FOREX',style=String(url.searchParams.get('style')||'SCALP').toUpperCase()==='SWING'?'SWING':'SCALP',status=String(url.searchParams.get('status')||'active').toLowerCase(),limit=Math.min(300,Math.max(1,Number(url.searchParams.get('limit')||120)));let rows=market==='FOREX'&&style==='SCALP'?await legacyScalpSignals(env):await getV31Signals(env,market,style);
  rows=rows.filter(s=>status==='all'||(status==='active'&&(s.status==='PENDING'||s.status==='OPEN'))||(status==='closed'&&s.status==='CLOSED')||String(s.status||'').toLowerCase()===status).slice(0,limit);return json({ok:true,version:V3_VERSION,market,style,status,count:rows.length,signals:rows});
}
