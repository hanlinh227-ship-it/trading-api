...
LEDGER='/var/lib/meme-alpha-signer/order-ledger.json'
ENABLE='/etc/meme-alpha/signer-enabled'
...
def b58e(b):
    ...
    z=len(b)-len(b.lstrip(b'\0'));return '1'*z+s
...
def pub(seed):
    p=subprocess.run(['openssl','pkey','-inform','DER','-pubout','-outform','DER'],input=der(seed),stdout=subprocess.PIPE,stderr=subprocess.PIPE,check=True,timeout=3)
    return p.stdout[-32:]

def sign(seed,msg):
    d='/run/meme-alpha-signer' if os.access('/run/meme-alpha-signer',os.W_OK) else tempfile.gettempdir()
    f=tempfile.NamedTemporaryFile(prefix='.ma-ed25519-',dir=d,delete=False);f.write(der(seed));f.close();os.chmod(f.name,0o600)
    try:
        p=subprocess.run(['openssl','pkeyutl','-sign','-inkey',f.name,'-keyform','DER','-rawin'],input=msg,stdout=subprocess.PIPE,stderr=subprocess.PIPE,check=True,timeout=3)
...
def limits(p,buy_lamports):
    now=time.time();l=load_ledger();recent=[x for x in l.get('orders',[]) if now-float(x.get('ts',0))<86400]
    hour=[x for x in recent if now-float(x.get('ts',0))<3600]
    if len(hour)>=int(p['maxOrdersPerHour']):raise PermissionError('HOURLY_ORDER_LIMIT')
    spent=sum(int(x.get('buyLamports',0)) for x in recent)
    if spent+buy_lamports>int(p['dailyBuyLamports']):raise PermissionError('DAILY_BUY_LIMIT')
    return l,recent

def record(l,recent,buy,request_id):
    recent.append({'ts':time.time(),'buyLamports':buy,'requestId':request_id});l['orders']=recent;save_ledger(l)
...
def get(url):
    q=urllib.request.Request(url,headers={'accept':'application/json','user-agent':'meme-alpha-signer-v2'})
    with urllib.request.urlopen(q,timeout=10) as r:return json.loads(r.read())

def order(req,seed,pkey,address,p):
    if not root_file(ENABLE,'ARMED=YES'):raise PermissionError('SIGNING_LOCKED')
    a,b=mint(req.get('inputMint')),mint(req.get('outputMint'))
    ...
    ledger,recent=limits(p,buy)
    maximpact=min(float(req.get('maxPriceImpactPct',p['maxPriceImpactPct'])),float(p['maxPriceImpactPct']))
    qs=urllib.parse.urlencode({'inputMint':a,'outputMint':b,'amount':str(amount),'taker':address})
    j=get(str(p['jupiterBaseUrl']).rstrip('/')+'/swap/v2/order?'+qs)
    impact=float(j.get('priceImpactPct',0) or 0)
    if impact>maximpact:raise PermissionError('PRICE_IMPACT_LIMIT')
    ...
    signed=sign_tx(raw,seed,pkey);rid=j.get('requestId');record(ledger,recent,buy,rid)
    return {'ok':True,'signedTransaction':base64.b64encode(signed).decode(),'requestId':rid,'inputMint':a,'outputMint':b,'inAmount':str(j.get('inAmount',amount)),'outAmount':str(j.get('outAmount','0')),'priceImpactPct':impact,'routeHash':hashlib.sha256(json.dumps(j.get('routePlan',[]),sort_keys=True,separators=(',',':')).encode()).hexdigest()[:16]}
...
def serve(c,seed,pkey,address,p):
    ...
    op=r.get('op')
    if op=='health':return reply(c,{'ok':True,'service':'meme-alpha-signer','version':'2.0','mode':'READY' if seed else 'LOCKED','walletLoaded':bool(seed),'signingEnabled':bool(seed and root_file(ENABLE,'ARMED=YES')),'publicKey':address,'arbitraryRawSign':False})
...
def selftest():
    seed=os.urandom(32);p=pub(seed);msg=bytes([1,0,0,1])+p+b'\0'*32+bytes([0]);tx=bytes([1])+b'\0'*64+msg;s=sign_tx(tx,seed,p);assert s[1:65]!=b'\0'*64;assert b58d(b58e(p))==p
    print('READY_SIGNER_V2_SELF_TEST=PASS');print('RAW_TRANSACTION_SIGNING=PASS');print('ARBITRARY_RAW_SIGN_OP=NOT_IMPLEMENTED');print('PERSISTENT_SPEND_LIMITER=IMPLEMENTED')
