...
echo '=== V300 PAPER LOOP AUDIT ==='
echo '=== FILE OWNERSHIP / WRITABILITY ==='
for f in src/paper.js src/scanner.js src/safe-signal-export.js config/runtime.json runtime-status/signal-snapshot.json; do
  [ -e "$f" ] || continue
...
echo '=== PAPER LOOP TIMING ==='
if [ -r src/paper.js ]; then
  grep -nEi 'setInterval|setTimeout|sleep|delay|interval|cycle|scanner|safe-signal|signal|30000|15000|10000|5000|3000' src/paper.js | head -120 || true
fi
...
echo 'V300_PAPER_LOOP_AUDIT_PASS'
