...
function eligible(c){if(!c)return false;const impact=Number(c.sellImpactPct??c.priceImpactPct);return c.universeClass==='MEME_CONFIRMED'&&c.securityDecision==='PASS'&&c.decision==='PROBE_CANDIDATE'&&!c.token2022&&c.sellRoute===true&&hardRejectEmpty(c.hardReject)&&Number(c.score)>=82&&Number(c.liquidityUsd)>=50000&&Number.isFinite(impact)&&Math.abs(impact)<=1.25&&Number(c.consecutiveEligible||0)>=2}
