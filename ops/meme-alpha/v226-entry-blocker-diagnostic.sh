...
console.log('=== MEME ALPHA CURRENT LIVE/TREND DIAGNOSTIC ===');
console.log(`GATE_ALLOWED=${!!g.allowed} GATE_REASONS=${(g.reasons||[]).join(',')||'NONE'} EXEC=${g.executionMode||'-'} ARM=${!!g.armOk}`);
console.log(`RISK_ENTRY_ALLOWED=${!!risk.entryAllowed} REGIME=${risk.riskRegime||'-'} GLOBAL_BLOCK=${(risk.globalBlockReasons||[]).join(',')||'NONE'} OPEN_PAPER=${risk.openPositions??'-'} EXPOSURE_PCT=${risk.exposurePct??'-'}`);
