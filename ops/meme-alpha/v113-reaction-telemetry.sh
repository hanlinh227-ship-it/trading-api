...
BACKUP=/var/lib/meme-alpha/data/backups/v113-$STAMP

rollback(){
  rc=$?
  echo "ROLLBACK rc=$rc"
  if [ -f "$BACKUP/package.json" ]; then cp -f "$BACKUP/package.json" "$APP/package.json"; fi
  if [ -f "$BACKUP/reaction-telemetry.js" ]; then cp -f "$BACKUP/reaction-telemetry.js" "$APP/src/reaction-telemetry.js"; else rm -f "$APP/src/reaction-telemetry.js"; fi
  chown meme-alpha:meme-alpha "$APP/package.json" 2>/dev/null || true
  [ -f "$APP/src/reaction-telemetry.js" ] && chown meme-alpha:meme-alpha "$APP/src/reaction-telemetry.js" || true
...
[ -f src/reaction-telemetry.js ] && cp -a src/reaction-telemetry.js "$BACKUP/reaction-telemetry.js" || true
systemctl stop "$SERVICE"

cat > src/reaction-telemetry.js <<'JS'
import fs from "node:fs";
...
node --check src/reaction-telemetry.js

python3 - <<'PY'
import json
from pathlib import Path
p=Path('/opt/meme-alpha/app/package.json')
j=json.loads(p.read_text())
s=j.setdefault('scripts',{})
s['reaction']='node src/reaction-telemetry.js'
cycle=s.get('cycle5','')
needle='node src/persistence.js && node src/risk.js'
if needle not in cycle:
    raise SystemExit('CYCLE5_TARGET_NOT_FOUND')
s['cycle5']=cycle.replace(needle,'node src/persistence.js && node src/reaction-telemetry.js && node src/risk.js',1)
p.write_text(json.dumps(j,indent=2)+"\n")
print('PACKAGE_V113_PATCHED')
PY

chown meme-alpha:meme-alpha src/reaction-telemetry.js package.json

BEFORE=$(node - <<'NODE'
const fs=require('fs');const s=JSON.parse(fs.readFileSync('/var/lib/meme-alpha/data/paper/state.json','utf8'));console.log((s.trades||[]).length);
NODE
)
...
if [ "$BEFORE" != "$AFTER" ]; then echo 'TELEMETRY_CHANGED_TRADES'; exit 1; fi
echo "NO_TRADE_SIDE_EFFECT_PASS trades=$AFTER"

grep -n 'reaction-telemetry.js' package.json
cat "$DATA/reaction-telemetry.json" | head -80
...
tail -220 /var/log/meme-alpha/paper.log | grep -E 'REACTION TELEMETRY|REACTION_TELEMETRY_STATUS|SHADOW_FAST_TRACK|BEHAVIOR_CHANGE=false|FULL_CYCLE_COMPLETE|HTTP 429|HTTP429' || true

node - <<'NODE'
import fs from 'node:fs';
const cfg=JSON.parse(fs.readFileSync('/opt/meme-alpha/app/config/runtime.json','utf8'));
const t=JSON.parse(fs.readFileSync('/var/lib/meme-alpha/data/paper/reaction-telemetry.json','utf8'));
if(cfg.mode!=='PAPER') throw new Error('MODE_CHANGED');
if(t.version!=='1.1.3-shadow' || t.behaviorChange!==false) throw new Error('TELEMETRY_INVARIANT_BAD');
console.log('MODE=PAPER');
console.log('LIVE_EXECUTION=DISABLED');
console.log('TELEMETRY_VERSION='+t.version);
console.log('BEHAVIOR_CHANGE='+t.behaviorChange);
console.log('SOURCE_HEALTHY='+t.sourceHealthy);
console.log('SHADOW_FAST_TRACK_COUNT='+t.shadowFastTrackCount);
console.log('V113_INVARIANT_PASS');
NODE

free -h
uptime
echo "V113_DEPLOY_COMPLETE"
echo "BACKUP=$BACKUP"
trap - ERR
