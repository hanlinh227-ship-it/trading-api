...
const FEEDS={
  'jupiter-recent':{ttl:45000,url:`${JUP}/tokens/v2/recent`,provider:'jupiter'},
  ...
  'gecko-new-1':{ttl:30000,url:`${GECKO}/networks/solana/new_pools?page=1&include=base_token,quote_token,dex`,provider:'geckoterminal'},
  'gecko-new-2':{ttl:60000,url:`${GECKO}/networks/solana/new_pools?page=2&include=base_token,quote_token,dex`,provider:'geckoterminal'},
  'gecko-trending':{ttl:60000,url:`${GECKO}/networks/solana/trending_pools?page=1&include=base_token,quote_token,dex`,provider:'geckoterminal'},
...
async function main(){
 if(process.argv.includes('--self-test')){const m=new Map();parseGecko(m,{data:[{attributes:{address:'POOL',pool_created_at:new Date().toISOString(),reserve_in_usd:'20000',transactions:{m5:{buys:12,sells:4}},volume_usd:{m5:'9000'}},relationships:{base_token:{data:{id:'solana_TESTMINT'}},quote_token:{data:{id:`solana_${WSOL}`}},dex:{data:{id:'raydium'}}}}],included:[{id:'solana_TESTMINT',attributes:{symbol:'TEST',name:'Test'}}]},'gecko-new-1','geckoterminal');const x=m.get('TESTMINT');if(!x||x.liquidityUsd!==20000||x.buys5m!==12)throw new Error('SELF_TEST');console.log('V367_SOLANA_DEX_UNIVERSE_SELF_TEST=PASS');console.log('AGE_BUCKETS=ULTRA_NEW_1H,DAY_1,SHORT_DAY_7D,MATURE');console.log('DISCOVERY_ONLY_ENTRY_GATES_UNCHANGED=TRUE');return}
 ...
 try{
  ...
  current.sort((a,b)=>b.preScore-a.preScore||b.discoveryConfidence-a.discoveryConfidence);const chosen=current.slice(0,MAX_CURRENT);const curSet=new Set(chosen.map(x=>x.mint));const retained=[];for(const old of previous.candidates||[]){if(curSet.has(old.mint))continue;const t=Date.parse(old.lastSeenAt||old.firstSeenAt||0);if(Number.isFinite(t)&&Date.now()-t<=WATCH_KEEP_MS)retained.push({...old,currentFeed:false,staleWatch:true,entryEligible:false,purpose:'DISCOVERY_ONLY',preScore:Math.max(0,n(old.preScore)-2)})}
  retained.sort((a,b)=>n(b.preScore)-n(a.preScore));const candidates=[...chosen,...retained].slice(0,MAX_OUTPUT);const providerCount=freshProviders.size;const status=providerCount>=2&&chosen.length>0?'HEALTHY':providerCount>=1?'DEGRADED':'DEGRADED';const out={version:'3.67.0',updatedAt:new Date().toISOString(),status,policy:'BROAD_SOLANA_DEX_DISCOVERY_ONLY_NEVER_GRANTS_ENTRY',providerCount,providers:[...freshProviders],freshSources,failures,currentFeedMints:map.size,currentCandidates:chosen.length,watchlistCandidates:retained.length,candidates};writeRuntime(out);try{atomic(STATE,out)}catch{};console.log(`SOLANA_DEX_UNIVERSE v=3.67.0 status=${status} providers=${providerCount} feedMints=${map.size} current=${chosen.length} watch=${retained.length}`)
