...
print('MODE=PAPER');print('LIVE_EXECUTION=DISABLED')
PY
...
import json,socket
s=socket.socket(socket.AF_UNIX);s.settimeout(2);s.connect('/run/meme-alpha-signer/signer.sock');s.sendall(b'{"op":"health"}\n');r=json.loads(s.recv(4096));s.close();assert r['ok'] is True; assert r['signingEnabled'] is False; print('BOT_SIGNER_HEALTH=PASS');print('SIGNER_MODE='+str(r.get('mode')));print('WALLET_LOADED='+str(r.get('walletLoaded')).lower());print('SIGNING_ENABLED=false')
PY
