...
# Ready signer implementation is staged separately and may already be present; never overwrite with an older locked signer.
...
bash -n "$DST/v192-root-install-micro-executor.sh"
bash -n "$DST/v193-root-arm-micro-live.sh"
bash -n "$DST/v194-root-disarm-micro-live.sh"
...
2) create isolated wallet only with the separately staged v180 root wallet bootstrap after validation policy allows it
...
5) only after >=20 empirical lifecycle + validation PASS + stress PASS: bash /opt/meme-alpha/app/ops/security/v193-root-arm-micro-live.sh
