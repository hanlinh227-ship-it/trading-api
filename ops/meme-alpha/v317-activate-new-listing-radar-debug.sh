...
TMP="/tmp/meme-alpha-v319-activate.$$.sh"
[ -r "$BASE" ] || { echo V319_BASE_MISSING; exit 2; }
python3 - "$BASE" > "$TMP" <<'PY'
from pathlib import Path
import sys
s=Path(sys.argv[1]).read_text()
old0='''/usr/bin/node --check "$rtmp"\nmv -f "$rtmp" "$RADAR"'''
new0='''/usr/bin/node --check "$rtmp"\nmv -f "$rtmp" "$RADAR"\nchmod 644 "$RADAR"\necho "V319_RADAR_FILE_READY $(stat -c 'owner=%U group=%G mode=%a size=%s' "$RADAR")"'''
if s.count(old0)!=1: raise SystemExit('V319_RADAR_INSTALL_ANCHOR_MISMATCH')
s=s.replace(old0,new0)
old_sc='''/usr/bin/node --check "$stmp"\nmv -f "$stmp" "$SCANNER"'''
new_sc='''/usr/bin/node --check "$stmp"\nmv -f "$stmp" "$SCANNER"\nchmod 644 "$SCANNER"\necho "V319_SCANNER_FILE_READY $(stat -c 'owner=%U group=%G mode=%a size=%s' "$SCANNER")"'''
if s.count(old_sc)!=1: raise SystemExit('V319_SCANNER_INSTALL_ANCHOR_MISMATCH')
