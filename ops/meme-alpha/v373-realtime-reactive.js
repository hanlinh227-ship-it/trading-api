...
const LOCK=`${APP}/runtime-status/realtime-pulse-v373.lock`;
const SELF_TEST=process.argv.includes('--self-test');
const VERSION='3.73.0-reactive-fabric';
...
function seedPairMemory(){for(const r of read(OUT,{rows:[]}).rows||[]){if(r?.mint&&r?.pair)pairMemoryByMint.set(r.mint,{pair:r.pair,symbol:r.symbol||null,pairAgeSec:r.pairAgeSec??null,lastSeenAt:Date.now()})}}
function desiredPairs(){
  const held=heldSet(),radar=read(RADAR,{candidates:[]}),rows=Array.isArray(radar.candidates)?radar.candidates:[];
  for(const x of rows){if(x?.mint&&x?.pairAddress)pairMemoryByMint.set(x.mint,{pair:x.pairAddress,symbol:x.symbol||null,pairAgeSec:x.pairAgeSec??null,lastSeenAt:Date.now()})}
  const byPair=new Map();
  for(const x of rows){
    if(!x?.pairAddress||!x?.mint||!(x.currentFeed===true||held.has(x.mint)))continue;
    const p={mint:x.mint,symbol:x.symbol||null,pair:x.pairAddress,preScore:n(x.preScore),fastDiscoveryLane:x.fastDiscoveryLane===true,held:held.has(x.mint),priority:Number(candidatePriority(x,held).toFixed(3)),pairAgeSec:Number.isFinite(Number(x.pairAgeSec))?Number(x.pairAgeSec):null};
  ...
  for(const mint of held){
    ...
    const m=pairMemoryByMint.get(mint);if(m?.pair)byPair.set(m.pair,{mint,symbol:m.symbol||null,pair:m.pair,preScore:0,fastDiscoveryLane:false,held:true,priority:10000,pairAgeSec:m.pairAgeSec??null,memoryFallback:true});
  }
...
function snapshot(){
  const now=Date.now(),pairs=[...desiredByPair.values()],rows=pairs.map(p=>{const a=events.get(p.mint)||[];const c1=a.filter(t=>now-t<=1000).length,c5=a.filter(t=>now-t<=5000).length,c15=a.filter(t=>now-t<=15000).length,last=a.length?a[a.length-1]:0;const r5=c5/5,r15=Math.max(.05,c15/15),momentum=Math.min(6,r5/r15),active=activeByPair.get(p.pair);return {...p,subscribed:!!active,subscriptionAgeMs:active?now-active.subscribedAt:null,events1s:c1,events5s:c5,events15s:c15,eventRate5s:Number(r5.toFixed(3)),eventMomentum:Number(momentum.toFixed(3)),lastEventAgeMs:last?now-last:null}});
...
async function main(){
  if(SELF_TEST){
    ...
    console.log('V373_REACTIVE_FABRIC_SELF_TEST=PASS');
    console.log('INCREMENTAL_SUBSCRIPTIONS=TRUE');
    console.log('HOTSET_CHANGE_RECONNECT=FALSE');
    console.log('SOLANA_SLOT_HEARTBEAT=TRUE');
    console.log('BOUNDED_RECONCILE_OPS=TRUE');
    console.log('HELD_POSITIONS_PRIORITY=TRUE');
    console.log('FAST_DISCOVERY_PRIORITY=TRUE');
    console.log('ATOMIC_STATE_WRITES=TRUE');
    console.log('SINGLETON_GUARD=TRUE');
    return;
  }
...
function shutdown(sig){if(shuttingDown)return;shuttingDown=true;lastError=sig;try{snapshot()}catch{};try{ws?.close()}catch{};releaseLock();setTimeout(()=>process.exit(0),50)}
