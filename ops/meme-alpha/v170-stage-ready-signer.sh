...
mkdir -p "$DST"
# Replace through a runner-owned temp inode; app is setgid meme-alpha-deploy.
t="$DST/ready_signer.py.new-$$"
cat "$SRC_DIR/signer/ready_signer_v2.py" > "$t"; chmod 0755 "$t"; mv -f "$t" "$DST/ready_signer.py"
...
echo READY_SIGNER_V2_STAGED=PASS
