...
echo '=== MEME ALPHA v2.10 SMART PROFIT / ANTI-WHIPSAW EXIT STAGE v2 ==='
...
post_old="async function post(url,body){const r=await fetch(url,{method:'POST',headers:{'content-type':'application/json','accept':'application/json'},body:JSON.stringify(body),signal:AbortSignal.timeout(15000)});const txt=await r.text();let j;try{j=JSON.parse(txt)}catch{j={raw:txt}};if(!r.ok)throw new Error(`HTTP_${r.status}`);return j}\n"
post_new=post_old+"async function getJson(url){const r=await fetch(url,{headers:{'accept':'application/json','user-agent':'meme-alpha-v210-smart-exit'},signal:AbortSignal.timeout(12000)});const txt=await r.text();let j;try{j=JSON.parse(txt)}catch{j={raw:txt}};if(!r.ok)throw new Error(`HTTP_${r.status}`);return j}\n"
if post_old not in s:raise SystemExit('POST_PATTERN_NOT_FOUND')
s=s.replace(post_old,post_new,1)

hold_old="""function holdSafe(c){
...
"""
hold_new="""function hardSafetyBroken(c){
  if(!c)return false;
...
"""
if hold_old not in s:raise SystemExit('HOLD_PATTERN_NOT_FOUND')
s=s.replace(hold_old,hold_new,1)

norm_old="    if(!st.position.lastAddAt)st.position.lastAddAt=st.position.openedAt||null;\n"
norm_new=norm_old+"    if(!Number.isFinite(Number(st.position.weakExitCount)))st.position.weakExitCount=0;\n    if(!Number.isFinite(Number(st.position.gateClosedCount)))st.position.gateClosedCount=0;\n    if(!Number.isFinite(Number(st.position.peakReturnPct)))st.position.peakReturnPct=null;\n    if(!Number.isFinite(Number(st.position.lastReturnPct)))st.position.lastReturnPct=null;\n    st.position.tp1Done=st.position.tp1Done===true;st.position.tp2Done=st.position.tp2Done===true;st.position.tp3Done=st.position.tp3Done===true;st.position.profitProtectDone=st.position.profitProtectDone===true;st.position.scaleInLockedAfterProfit=st.position.scaleInLockedAfterProfit===true;\n"
if norm_old not in s:raise SystemExit('NORMALIZE_PATTERN_NOT_FOUND')
s=s.replace(norm_old,norm_new,1).replace("st.version='2.9.0'","st.version='2.10.0'",1)

buy_old="walletBeforeSolLamports:beforeSol,walletAfterSolLamports:afterSol};event({type:'MICRO_BUY'"
buy_new="walletBeforeSolLamports:beforeSol,walletAfterSolLamports:afterSol,weakExitCount:0,gateClosedCount:0,peakReturnPct:null,lastReturnPct:null,tp1Done:false,tp2Done:false,tp3Done:false,profitProtectDone:false,scaleInLockedAfterProfit:false};event({type:'MICRO_BUY'"
if buy_old not in s:raise SystemExit('BUY_STATE_PATTERN_NOT_FOUND')
s=s.replace(buy_old,buy_new,1)

sell_pat=r"async function sell\(st,reason\)\{.*?\n\}\n\nasync function observeCapital"
sell_new="""async function previewExitReturn(st,pub){
...
async function observeCapital"""
s,nsub=re.subn(sell_pat,sell_new,s,count=1,flags=re.S)
if nsub!=1:raise SystemExit('SELL_PATTERN_NOT_FOUND')

tick_old="""async function tick(){
...
"""
if tick_old not in s:raise SystemExit('TICK_PATTERN_NOT_FOUND')
...
grep -q 'ENTRY_GATE_CLOSED_HOLD' "$DST"
...
echo TP_NORMAL=22_50_100
echo TP_STRONG_TREND=30_70_130
echo TP_PARTIAL_FRACTIONS=15_20_15_PERCENT_OF_REMAINING
echo PROFIT_GIVEBACK_PROTECT=25_PERCENT_OF_REMAINING
echo RUNNER_REMAINS_WHILE_TREND_HEALTHY=TRUE
