...
B=/var/lib/meme-alpha/data/backups/v130-$(date -u +%Y%m%d-%H%M%S)
mkdir -p "$B"
cp -a src package.json "$B"/

cat > src/stress-validation.js <<'NODE'
import fs from 'node:fs';
...
node --check src/stress-validation.js
node src/stress-validation.js
...
echo "CYCLE_FAILURES=$(grep -cE 'CYCLE_FAILED|FULL_CYCLE_FAILED|FAST_POSITION_TICK_FAILED' "$TMP" || true)"
...
FAILS=$(grep -cE 'CYCLE_FAILED|FULL_CYCLE_FAILED|FAST_POSITION_TICK_FAILED' "$TMP" || true)
[ "$FAILS" -eq 0 ] || { echo 'V130_SOAK_FAILURE'; exit 1; }
rm -f "$TMP"
...
import fs from 'node:fs';
const c=JSON.parse(fs.readFileSync('config/runtime.json','utf8'));
const r=JSON.parse(fs.readFileSync('/var/lib/meme-alpha/data/paper/stress-validation.json','utf8'));
if(c.mode!=='PAPER'||r.behaviorChange!==false||r.summary.fail!==0) throw new Error('V130_INVARIANT_FAIL');
console.log('MODE=PAPER'); console.log('LIVE_EXECUTION=DISABLED'); console.log('BEHAVIOR_CHANGE=false'); console.log('V130_INVARIANT_PASS');
NODE
free -h
uptime
echo "V130_DEPLOY_COMPLETE"
echo "BACKUP=$B"
