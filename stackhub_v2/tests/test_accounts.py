    ...
    rows = account_status(
        {
            ...
            "MOLTJOBS_API_KEY": "fake_moltjobs_value",
            "TASKFORCE_API_KEY": "fake_taskforce_value",
            "GUMROAD_ACCESS_TOKEN": "fake_gumroad_value",
        }
    )
    text = repr(rows)
    assert "fake_taskbounty_value" not in text
    assert "fake_moltjobs_value" not in text
    assert "fake_taskforce_value" not in text
...
def test_account_status_reports_missing_env_names_only():
    ...
    assert rows["moltjobs"]["missing_secrets"] == ("MOLTJOBS_API_KEY",)
    assert rows["taskforce"]["missing_secrets"] == ("TASKFORCE_API_KEY",)
