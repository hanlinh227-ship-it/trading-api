...
async function sellability(candidate) {
  try {
    const decimals =
      Math.max(
        0,
        Math.min(12, Number(candidate.decimals ?? 6))
      );
    ...
    return {
      sellRoute: r.ok &&
        Boolean(body.outAmount) &&
        Number(body.outAmount) > 0,

      sellQuoteHttp: r.status,
      sellOutAmount: body.outAmount ?? null,
      sellPriceImpactPct:
        body.priceImpactPct ?? null
    };

  } catch (err) {
    return {
      sellRoute: false,
      sellQuoteError: err.message
    };
  }
}
...
// v2.0: keep the best 20, then add at most 6 high-signal meme/launchpad candidates.
