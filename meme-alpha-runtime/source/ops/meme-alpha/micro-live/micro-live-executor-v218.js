...
const ENTRY_SIGNAL_MAX_AGE_SEC=45,HARD_STALE_EXIT_SEC=180;
...
function tier(c,p){
  ...
  const score=opportunityScore(c),con=n(c.consecutiveEligible),net=n(c.netBuyers5m),avg=n(c.avgNetBuyersLast2),liq=n(c.liquidityUsd),imp=impact(c),chg=n(c.priceChange5m);
...
function rank(c){return opportunityScore(c)*100+n(c.netBuyers5m)*2+n(c.avgNetBuyersLast2)+n(c.organicRatio5m)*30-Math.max(0,n(c.priceChange5m)-10)*10}
...
function normalizeState(st){
  ...
  st.version='2.18.0';st.closed=n(st.closed);st.position=st.position||null;
  if(st.position){
    ...
    if(!Number.isFinite(Number(st.position.dataStaleCount)))st.position.dataStaleCount=0;
...
async function placeBuy(st,c,targetTier,isAdd=false){
  ...
  else{st.position={mint:c.mint,symbol:c.symbol,tokenRaw:delta.toString(),costBasisLamports:spent,entrySolLamports:spent,entrySignature:sig,openedAt:new Date().toISOString(),lastAddAt:new Date().toISOString(),addCount:0,targetUtilizationPct:targetTier.pct,tier:targetTier.name,walletBeforeSolLamports:beforeSol,walletAfterSolLamports:afterSol,weakExitCount:0,gateClosedCount:0,peakReturnPct:null,lastReturnPct:null,tp1Done:false,tp2Done:false,tp3Done:false,profitProtectDone:false,scaleInLockedAfterProfit:false,dataStaleCount:0};event({type:'MICRO_BUY',mint:c.mint,symbol:c.symbol,tier:targetTier.name,targetUtilizationPct:targetTier.pct,spentLamports:spent,spentSol:spent/1e9,signature:sig})}
...
async function previewExitReturn(st,pub){
...
function sellImpactLimit(p,reason){
...
async function sellFraction(st,fraction,reason){
  ...
  const allowedSellImpact=sellImpactLimit(p,reason),o=await signer({op:'order',inputMint:m,outputMint:WSOL,amount:amount.toString(),maxPriceImpactPct:allowedSellImpact});if(!o.ok){if(o.error==='PRICE_IMPACT_LIMIT'){event({type:'SELL_DEFERRED_IMPACT',reason,mint:m,allowedSellImpactPct:allowedSellImpact});return{closed:false,skipped:true,reason:'PRICE_IMPACT_LIMIT'}}throw new Error(`SIGNER_${o.error}`)};
...
async function tick(){
  ...
  if(st.position){
    const c=candidate(st.position.mint),pos=st.position,age=(Date.now()-Date.parse(pos.openedAt||0))/1000,sigAge=fileAgeSec(SIGNAL);
    if(!gate.allowed)pos.gateClosedCount=n(pos.gateClosedCount)+1;else pos.gateClosedCount=0;
    const dataStale=!c||sigAge>60;pos.dataStaleCount=dataStale?Math.min(120,n(pos.dataStaleCount)+1):Math.max(0,n(pos.dataStaleCount)-1);
    ...
    if(Number.isFinite(ret)){
      if(!pos.tp1Done&&ret>=th.tp1){const r=await sellFraction(st,0.15,'SMART_TP1');if(!r.closed&&!r.skipped&&st.position){st.position.tp1Done=true;st.position.scaleInLockedAfterProfit=true;atomic(statePath,st)}return{action:'PARTIAL_SELL',reason:'SMART_TP1'}}
      if(!pos.tp2Done&&ret>=th.tp2){const r=await sellFraction(st,0.20,'SMART_TP2');if(!r.closed&&!r.skipped&&st.position){st.position.tp2Done=true;st.position.scaleInLockedAfterProfit=true;atomic(statePath,st)}return{action:'PARTIAL_SELL',reason:'SMART_TP2'}}
      if(!pos.tp3Done&&ret>=th.tp3){const r=await sellFraction(st,0.15,'SMART_TP3_RUNNER_LOCK');if(!r.closed&&!r.skipped&&st.position){st.position.tp3Done=true;st.position.scaleInLockedAfterProfit=true;atomic(statePath,st)}return{action:'PARTIAL_SELL',reason:'SMART_TP3_RUNNER_LOCK'}}
      const protectGiveback=Math.max(10,peak*0.35);
      if(!pos.profitProtectDone&&peak>=25&&giveback>=protectGiveback&&pos.weakExitCount>=1&&ret>0){const r=await sellFraction(st,0.25,'SMART_PROFIT_GIVEBACK');if(!r.closed&&!r.skipped&&st.position){st.position.profitProtectDone=true;st.position.scaleInLockedAfterProfit=true;atomic(statePath,st)}return{action:'PARTIAL_SELL',reason:'SMART_PROFIT_GIVEBACK'}}
    }
    if(sigAge>=HARD_STALE_EXIT_SEC||pos.dataStaleCount>=36){await sell(st,'ANALYSIS_HARD_STALE');return{action:'SELL',reason:'ANALYSIS_HARD_STALE'}}
    if(age>=75&&pos.weakExitCount>=4){await sell(st,'CONFIRMED_TREND_BREAK');return{action:'SELL',reason:'CONFIRMED_TREND_BREAK'}}
    if(gate.allowed&&!pos.scaleInLockedAfterProfit&&pos.weakExitCount===0&&c){const t=tier(c,p),last=Date.parse(pos.lastAddAt||pos.openedAt||0),addAge=(Date.now()-last)/1000;if(t.pct>0&&addAge>=p.minAddIntervalSec){const r=await placeBuy(st,c,t,true);if(r.placed)return{action:'ADD',reason:t.name}}}
...
if(process.argv.includes('--self-test')){
  ...
  if(!trendEntryEligible(c)||tier(c,p).pct!==94||trendEntryEligible({...c,priceChange5m:25})||trendEntryEligible({...c,securityDecision:'REVIEW'})||trendEntryEligible({...c,holderClusterDecision:'REVIEW'})||trendEntryEligible({...c,sellRoute:false})||trendEntryEligible({...c,token2022:true})||!holdSafe({...c,score:60}))throw new Error('TREND_SELFTEST');
  ...
  console.log('MICRO_EXECUTOR_V218_SELF_TEST=PASS');console.log('STAGED_UTILIZATION=15_35_65_94 OPPORTUNITY_ENTRY=9_10_TARGET');console.log('ABSOLUTE_RESERVE_SOL=0.010');console.log('MIN_REAL_ORDER_SOL=0.010');console.log('EXTERNAL_DEPOSIT_WITHDRAWAL_AWARE=TRUE');console.log('NETWORK_EXECUTION=NOT_CALLED');
