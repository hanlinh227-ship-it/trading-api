...
function normalizeState(st){
  ...
  st.version='2.10.0';st.closed=n(st.closed);st.position=st.position||null;
  if(st.position){
...
async function placeBuy(st,c,targetTier,isAdd=false){
  ...
  else{st.position={mint:c.mint,symbol:c.symbol,tokenRaw:delta.toString(),costBasisLamports:spent,entrySolLamports:spent,entrySignature:sig,openedAt:new Date().toISOString(),lastAddAt:new Date().toISOString(),addCount:0,targetUtilizationPct:targetTier.pct,tier:targetTier.name,walletBeforeSolLamports:beforeSol,walletAfterSolLamports:afterSol,weakExitCount:0,gateClosedCount:0,peakReturnPct:null,lastReturnPct:null,tp1Done:false,tp2Done:false,tp3Done:false,profitProtectDone:false,scaleInLockedAfterProfit:false};event({type:'MICRO_BUY',mint:c.mint,symbol:c.symbol,tier:targetTier.name,targetUtilizationPct:targetTier.pct,spentLamports:spent,spentSol:spent/1e9,signature:sig})}
...
if(process.argv.includes('--self-test')){
  ...
  console.log('MICRO_EXECUTOR_V210_SELF_TEST=PASS');console.log('STAGED_UTILIZATION=15_35_65_94 OPPORTUNITY_ENTRY=9_10_TARGET');console.log('ABSOLUTE_RESERVE_SOL=0.010');console.log('MIN_REAL_ORDER_SOL=0.010');console.log('EXTERNAL_DEPOSIT_WITHDRAWAL_AWARE=TRUE');console.log('NETWORK_EXECUTION=NOT_CALLED');
