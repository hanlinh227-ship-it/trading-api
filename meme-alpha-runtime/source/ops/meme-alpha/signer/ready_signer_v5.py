...
def candidate_ok(mint_out,p):
 ...
 for c in s.get('candidates',[]) or []:
  ...
  try:
   impact=abs(float(impact));score=float(c.get('score',0));liq=float(c.get('liquidityUsd',0));chg=float(c.get('priceChange5m'));net=float(c.get('netBuyers5m'));avg=float(c.get('avgNetBuyersLast2'));slope=float(c.get('scoreSlopeLast2'))
  except:return False
  return c.get('universeClass')=='MEME_CONFIRMED' and c.get('securityDecision')=='PASS' and c.get('holderClusterDecision')=='PASS' and c.get('decision')=='PROBE_CANDIDATE' and not c.get('token2022') and c.get('sellRoute') is True and hard_empty(c.get('hardReject')) and score>=82 and liq>=50000 and impact<=float(p['maxBuyPriceImpactPct']) and int(c.get('consecutiveEligible',0))>=2 and chg>=0.30 and chg<=18 and net>=3 and avg>=3 and slope>=0 and c.get('liquidityStableLast2') is True
 return False
def rpc_balance(address,p):
 body=json.dumps({'jsonrpc':'2.0','id':1,'method':'getBalance','params':[address,{'commitment':'confirmed'}]}).encode();q=urllib.request.Request(str(p['rpcUrl']),data=body,headers={'content-type':'application/json','user-agent':'meme-alpha-signer-v5'})
...
def mint(v):
 ...
 return v
def get(url):
 q=urllib.request.Request(url,headers={'accept':'application/json','user-agent':'meme-alpha-signer-v5'});
...
def serve(c,seed,pkey,address,p):
 ...
 op=r.get('op')
 if op=='health':return reply(c,{'ok':True,'service':'meme-alpha-signer','version':'5.0','mode':'READY' if seed else 'LOCKED','walletLoaded':bool(seed),'signingEnabled':bool(seed and root_file(ENABLE,'ARMED=YES')),'publicKey':address,'arbitraryRawSign':False,'buyPolicy':'TREND_GATE_STAGED_FULL_CAPITAL','maxPortfolioUtilizationPct':p['maxPortfolioUtilizationPct']})
...
def selftest():
 ...
 print('READY_SIGNER_V5_SELF_TEST=PASS');print('ARBITRARY_RAW_SIGN_OP=NOT_IMPLEMENTED');print('BUY_REQUIRES_FRESH_TREND_GATE=TRUE');print('BUY_SIZE_SCALES_WITH_WALLET_BALANCE=TRUE');print('SELL_BYPASSES_BUY_FREQUENCY_QUOTA=TRUE');print('MAX_PORTFOLIO_UTILIZATION_PCT=94')
def main():
