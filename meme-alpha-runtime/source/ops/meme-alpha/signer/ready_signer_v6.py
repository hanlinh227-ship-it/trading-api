...
def rpc_balance(address,p):
 body=json.dumps({'jsonrpc':'2.0','id':1,'method':'getBalance','params':[address,{'commitment':'confirmed'}]}).encode();q=urllib.request.Request(str(p['rpcUrl']),data=body,headers={'content-type':'application/json','user-agent':'meme-alpha-signer-v6'})
...
def mint(v):
 ...
 return v
def get(url):
 q=urllib.request.Request(url,headers={'accept':'application/json','user-agent':'meme-alpha-signer-v6'});
...
def serve(c,seed,pkey,address,p):
 ...
 op=r.get('op')
 if op=='health':return reply(c,{'ok':True,'service':'meme-alpha-signer','version':'6.0','mode':'READY' if seed else 'LOCKED','walletLoaded':bool(seed),'signingEnabled':bool(seed and root_file(ENABLE,'ARMED=YES')),'publicKey':address,'arbitraryRawSign':False,'buyPolicy':'OPPORTUNITY_9_10_HARD_SAFETY_STAGED_CAPITAL','maxPortfolioUtilizationPct':p['maxPortfolioUtilizationPct']})
...
def selftest():
 ...
 print('READY_SIGNER_V6_SELF_TEST=PASS');print('ARBITRARY_RAW_SIGN_OP=NOT_IMPLEMENTED');print('BUY_REQUIRES_FRESH_HARD_SAFETY_TREND_GATE=TRUE');print('BUY_SIZE_SCALES_WITH_WALLET_BALANCE=TRUE');print('SELL_BYPASSES_BUY_FREQUENCY_QUOTA=TRUE');print('MAX_PORTFOLIO_UTILIZATION_PCT=94')
def main():
