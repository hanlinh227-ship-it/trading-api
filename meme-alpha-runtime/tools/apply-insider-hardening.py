...
EXP = Path('meme-alpha-runtime/source/src/safe-signal-export.js')
EXE = Path('meme-alpha-runtime/source/src/micro-live-executor.js')
SIG = Path('meme-alpha-runtime/source/external/signer/ready_signer.py')
...
"""
    ...
    text = text.replace(
        promotion,
        promotion + "  if(insiderRiskDecision!=='PASS'&&decision==='PROBE_CANDIDATE'){decision='INSIDER_RISK_BLOCK';entryGuardReasons.push(`V377_INSIDER_${insiderRiskDecision}`);}\n",
        1,
    )
    ...
    text = sub1(
        text,
        r"  const common=\{decision:'WATCH'.*?freezeAuthorityDisabled:true\};",
        ...
        'signal-selftest-common',
    )
    review = "  if(review.decision==='PROBE_CANDIDATE')throw new Error('SECURITY_REVIEW_PROMOTION_SELFTEST');\n"
    if review not in text:
        raise RuntimeError('signal-review-selftest-anchor')
    text = text.replace(
        review,
        review + "  const insider=guardCandidate({...common,mint:'I',score:85,liquidityUsd:700000,netBuyers5m:20,priceChange5m:2,buyVolume5m:300,sellVolume5m:100,holderClusterAudit:{decision:'REVIEW',maxAccountsSameOwner:3,devIdentityProven:false}},rt,wh,regime);\n  if(insider.decision==='PROBE_CANDIDATE'||insider.insiderRiskDecision==='PASS')throw new Error('INSIDER_RISK_PROMOTION_SELFTEST');\n",
        1,
    )
    text = text.replace(
...
def patch_executor(text):
    ...
    text = sub1(
        text,
        r"function coreSafe\(c\)\{return !!c&&c\.universeClass==='MEME_CONFIRMED'&&c\.securityDecision==='PASS'&&c\.holderClusterDecision==='PASS'&&",
        "function coreSafe(c){return !!c&&c.universeClass==='MEME_CONFIRMED'&&c.securityDecision==='PASS'&&c.holderClusterDecision==='PASS'&&insiderSafe(c)&&",
        'executor-core-safe',
    )
    text = sub1(
        text,
        r"  const c=\{mint:'C'.*?organicRatio5m:\.3\};",
    ...
    text = sub1(
        text,
        r"  if\(!trendEntryEligible\(c\)\|\|trendEntryEligible\(\{\.\.\.c,priceChange5m:25\}\).*?throw new Error\('ENTRY_SAFETY_SELFTEST'\);",
...
def main():
    ...
    print('INSIDER_HARDENING_PATCH=APPLIED')
