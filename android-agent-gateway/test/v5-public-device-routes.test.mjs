...
function fixture() {
  ...
  const env = {
    DEVICE_SESSIONS: {
      ...
      get(id) { return id === 'device:__latest_device_registry__' ? registryStub : deviceStub },
    },
  }
  return { env, seen, registrySeen }
}
...
for (const [publicOp, internalOp] of [
  ...
  test(`V5 ${publicOp} route preserves device authentication and proxies to durable object`, async () => {
    const { env, seen, registrySeen } = fixture()
    ...
    assert.equal(registrySeen.length, 1)
  })
}
...
test('V5 device task operation routes reject non-POST methods before proxying', async () => {
  ...
  assert.equal(registrySeen.length, 0)
})
